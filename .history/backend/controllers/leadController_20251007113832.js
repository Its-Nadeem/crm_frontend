     ...
     try {
         ...
         Object.keys(fieldNames).forEach(field => {
             if (updateData.hasOwnProperty(field)) {
                 ...
                 if (field === 'customFields' && typeof oldValue === 'object' && typeof newValue === 'object') {
                     // For custom fields, check if any values changed
                     const oldKeys = Object.keys(oldValue || {});
                     const newKeys = Object.keys(newValue || {});

                     // Check for changes in existing keys or new keys
                     ...
                     allKeys.forEach(key => {
                         ...
                         if (oldFieldValue !== newFieldValue) {
                             ...
                             newActivities.push({
                                 type: 'FIELD_UPDATE',
                                 content: `Updated custom field "${key}" from "${oldFieldValue}" to "${newFieldValue}"`,
                                 timestamp: new Date(),
                                 authorId: req.user.id
                             });
                         }
                     ...
                     if (!hasChanges) return; // No changes in custom fields
         ...
         if (!lead.id && req.params.id.match(/^[0-9a-fA-F]{24}$/)) {
