...
const updateLead = async (req, res) => {
     try {
         ...
         // Get current user name for activity attribution
         const currentUser = users.find(u => u.id === req.user.id);
         const currentUserName = currentUser ? currentUser.name : `User ${req.user.id}`;
         ...
         // Create a single activity for all field changes
         if (fieldChanges.length > 0) {
             ...
             // Determine the appropriate author based on what's being changed
             let activityAuthorId = lead.assignedToId;
             let activityAuthorName = getUserName(lead.assignedToId, users);

             // If assignedToId is being changed, use the current (old) owner for this activity
             const isAssignmentChange = fieldChanges.some(change => change.field === 'Assigned To');

             if (isAssignmentChange) {
                 // For assignment changes, use the current (old) owner
                 activityAuthorId = lead.assignedToId;
                 activityAuthorName = getUserName(lead.assignedToId, users);
             } else {
                 // For other changes, use the current owner
                 activityAuthorId = lead.assignedToId;
                 activityAuthorName = getUserName(lead.assignedToId, users);
             }

             if (!activityAuthorName || activityAuthorName.startsWith('User ')) {
                 activityAuthorName = 'Unassigned';
             }

             newActivities.push({
                 type: 'FIELD_UPDATE',
                 content: `${activityAuthorName} updated - ${changesText}`,
                 timestamp: new Date(),
                 authorId: activityAuthorId || req.user.id // Use lead owner as author, fallback to request user
             });
         }

         // Check for system fields that should not create activities
         Object.keys(updateData).forEach(field => {
             if (systemFields.includes(field)) {
                 // Remove any activities that might have been created for system fields
                 newActivities = newActivities.filter(activity =>
                     !activity.content.includes(field)
                 );
             }
         });

         // Handle custom fields separately with better logic
         if (updateData.hasOwnProperty('customFields')) {
             const oldCustomFields = lead.customFields || {};
             const newCustomFields = updateData.customFields || {};

             // Only track actual custom field definitions, not MongoDB internal fields
             const customFieldChanges = [];

             Object.keys(newCustomFields).forEach(key => {
                 // Skip MongoDB internal fields and system fields
                 if (key.startsWith('$__') || key.startsWith('_') || key.includes('ObjectId')) return;

                 const oldValue = oldCustomFields[key];
                 const newValue = newCustomFields[key];

                 // Skip if the new value contains MongoDB internal data
                 if (typeof newValue === 'string' && newValue.includes('new ObjectId')) return;
             ...
             // Create a single activity for all custom field changes
             if (customFieldChanges.length > 0) {
                 ...
                 const changesText = customFieldChanges
                     .map(change => {
                         const oldVal = formatValue(change.oldValue);
                         const newVal = formatValue(change.newValue);
                 ...
                 // Use the current lead owner as the author, not the request user
                 const leadOwnerName = lead.assignedToId ? getUserName(lead.assignedToId, users) : 'Unassigned';

                 newActivities.push({
                     type: 'FIELD_UPDATE',
                     content: `${leadOwnerName} updated custom fields - ${changesText}`,
                     timestamp: new Date(),
                     authorId: lead.assignedToId || req.user.id // Use lead owner as author, fallback to request user
                 });
             }
         }
         ...
         if (newActivities.length > 0) {
             ...
             updateData.activities = filteredActivities;
         }
