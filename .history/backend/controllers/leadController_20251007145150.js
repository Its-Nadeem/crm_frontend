...
const updateLead = async (req, res) => {
     try {
         ...
         if (fieldChanges.length > 0) {
             ...
             // Use the actual user who made the request as the author
             const requestUser = users.find(u => u.id === req.user.id);
             const requestUserName = requestUser ? requestUser.name : `User ${req.user.id}`;

             newActivities.push({
                 type: 'FIELD_UPDATE',
                 content: `${requestUserName} updated - ${changesText}`,
                 timestamp: new Date(),
