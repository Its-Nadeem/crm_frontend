...
const updateLead = async (req, res) => {
     try {
         ...
         if (fieldChanges.length > 0) {
             ...
             const requestUser = users.find(u => u.id === req.user.id) || users.find(u => u._id.toString() === req.user._id.toString());
             const requestUserName = requestUser ? requestUser.name : `User ${req.user.id || req.user._id}`;
