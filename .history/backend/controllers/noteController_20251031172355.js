...
// @desc    Delete a note (from lead activities)
...
const deleteNote = async (req, res) => {
    try {
        ...
        // Find the lead that contains this note activity
        const lead = await Lead.findOne({
            organizationId: req.user.organizationId,
            'activities._id': noteId
        });
        ...
        // Remove the note activity from the lead's activities array
        lead.activities = lead.activities.filter(activity =>
            activity._id.toString() !== noteId && activity.type === 'NOTE'
        );
