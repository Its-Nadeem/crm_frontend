...
export const createOrUpdateUser = async (req, res) => {
    try {
        ...
        if (!id) {
            ...
            const lastUser = await User.findOne({ organizationId: req.user.organizationId }).sort({ id: -1 });
            data.id = lastUser ? lastUser.id + 1 : 1;
            console.log('DEBUG: Creating new user with ID:', data.id);
        } else {
            console.log('DEBUG: Updating existing user with ID:', id);
        }

        console.log('DEBUG: Final user data to save:', JSON.stringify(data, null, 2));

        const user = id
            ? await User.findOneAndUpdate({ id: parseInt(id), organizationId: req.user.organizationId }, data, { new: true, runValidators: true })
