...
// Specific team handlers to work with 'id' field instead of '_id'
export const createOrUpdateTeam = async (req, res) => {
    try {
        const { id } = req.params;
        const data = { ...req.body, organizationId: req.user.organizationId };
        ...
        // Treat string 'undefined' or 'null' as missing id (defensive against client bugs)
        const isNew = !id || id === 'undefined' || id === 'null';

        // Generate a unique ID for new teams
        if (isNew) {
            data.id = `team_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
            console.log('DEBUG: Creating new team with ID:', data.id);
        } else {
            console.log('DEBUG: Updating existing team with ID:', id);
        }

        console.log('DEBUG: Final data to save:', JSON.stringify(data, null, 2));

        // If new, create. If updating, try by string id first, then fall back to MongoDB _id if id looks like an ObjectId
        let team;
        if (isNew) {
            team = await Team.create(data);
        } else {
            team = await Team.findOneAndUpdate({ id: id, organizationId: req.user.organizationId }, data, { new: true, runValidators: true });

            if (!team && id.match(/^[0-9a-fA-F]{24}$/)) {
                // Fallback to updating by _id when clients accidentally pass MongoDB ids
                team = await Team.findOneAndUpdate({ _id: id, organizationId: req.user.organizationId }, data, { new: true, runValidators: true });
            }
        }

        if (!team) {
            console.error('Team not found or not authorized for ID:', id);
            return res.status(404).json({ message: 'Team not found or not authorized' });
        }

        console.log('Team operation successful:', team);
...
export const deleteTeam = async (req, res) => {
    try {
        ...
        console.log('DEBUG: Deleting team:', id);

        const team = await Team.findOne({ id: id, organizationId: req.user.organizationId });
        if (!team) {
            console.log('DEBUG: Team not found with ID:', id);
            return res.status(404).json({ message: 'Team not found or not authorized' });
        }

        await Team.deleteOne({ id: id, organizationId: req.user.organizationId });

        console.log('DEBUG: Team deleted successfully:', id);
        res.status(204).send();
    } catch (error) {
...
export const createOrUpdateUser = async (req, res) => {
    try {
        ...
        if (isNewUser) {
            ...
            // Get all existing user IDs from the entire database to avoid conflicts across organizations
            const allUsers = await User.find({}).sort({ id: 1 }).select('id');
            ...
            data.id = nextId;
            console.log('DEBUG: Creating new user with ID:', data.id, 'Next available ID after checking all users');
