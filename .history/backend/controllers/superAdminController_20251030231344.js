...
export const createOrUpdatePaymentGatewaySetting = async (req, res) => {
    try {
        const { id, provider: providerParam } = req.params;

        let item;
        if (providerParam) {
            // Update by provider name (from /payment-gateways/provider/:provider route)
        ...
        if (!item) return res.status(404).json({ message: 'Payment gateway setting not found' });
        res.status(id || providerParam ? 200 : 201).json(item);
    } catch (error) {
        res.status(500).json({ message: `Server Error: ${error.message}` });
    }
};
