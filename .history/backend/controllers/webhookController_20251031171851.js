...
// @desc    Get all webhooks for organization with enhanced data
...
const receiveWebhook = async (req, res) => {
    try {
        ...
        // First try to find webhook config by API key (for webhook-specific keys)
        let webhook = await WebhookConfig.findOne({ apiKey });

        // If not found, try to find organization by API key (for organization API keys)
        let organization = null;
        if (!webhook) {
            organization = await Organization.findOne({ apiKey, apiKeyStatus: 'active' });
            if (organization) {
                // Update last used timestamp
                organization.apiKeyLastUsed = new Date();
                await organization.save();

                // For organization API keys, we need to handle the webhook differently
                // This could be used for organization-level webhook endpoints
                console.log('Organization API key used for webhook:', organization.id);
            }
        }
        ...
        if (!webhook && !organization) {
            return res.status(404).json({
                success: false,
                message: 'Invalid API key or webhook not found'
            });
        }

        // If using organization API key, we might need different handling
        if (organization && !webhook) {
            return res.status(400).json({
                success: false,
                message: 'Organization API keys require webhook specification. Use x-webhook-id header.'
            });
        }
...
const getWebhookLogs = async (req, res) => {
    try {
        console.log('getWebhookLogs called with params:', req.params);
        console.log('getWebhookLogs called with query:', req.query);
...
const testWebhookEnhanced = async (req, res) => {
    try {
        ...
        // Block localhost/ngrok in production (but allow Cloudflare tunnels)
        if (process.env.NODE_ENV === 'production') {
            if (url.includes('localhost') || url.includes('ngrok') || url.includes('127.0.0.1')) {
                return res.status(400).json({
                    success: false,
                    message: 'Localhost and ngrok URLs are not allowed in production. Use proper domain or Cloudflare tunnel.'
                });
            }
        }
...
// @desc    Receive organization-level webhook (using organization API key)
// @route   POST /api/webhooks/organization/receive
// @access  Public (but requires organization API key)
const receiveOrganizationWebhook = async (req, res) => {
    try {
        const apiKey = req.headers['x-api-key'] || req.query.api_key;
        ...
        const organization = await Organization.findOne({ apiKey, apiKeyStatus: 'active' });
        ...
        const event = req.headers['x-event'] || req.body.event || 'lead.created';

        // Process the webhook data at organization level
        const result = await processOrganizationWebhookData(organization, event, req.body);
        ...
        console.error('Error receiving organization webhook:', error);
        res.status(500).json({
            success: false,
            message: 'Error processing organization webhook',
            error: error.message
        });
    }
};

// @desc    Process webhook data at organization level
// @access  Private
const processOrganizationWebhookData = async (organization, event, payload) => {
    try {
        const { data } = payload;
        ...
        // Handle different event types at organization level
        switch (event) {
            case 'lead.created':
            case 'lead.updated':
                return await handleOrganizationLeadWebhook(organization, data, event === 'lead.created' ? 'create' : 'update');
        ...
        console.error('Error processing organization webhook data:', error);
        throw error;
    }
};

// @desc    Handle lead-related webhook events at organization level
// @access  Private
const handleOrganizationLeadWebhook = async (organization, leadData, action) => {
    try {
        ...
        // Trigger organization-level webhooks for lead events
        await triggerOrganizationLeadWebhooks(organizationId, action === 'create' ? 'lead.created' : 'lead.updated', {
            lead: {
                id: lead.id,
                name: lead.name,
                email: lead.email,
        ...
        return {
            ...
            action,
            leadId: lead.id,
            message: `Lead ${action}d successfully via organization webhook`
        };

    } catch (error) {
        console.error('Error handling organization lead webhook:', error);
        throw error;
    }
};

// @desc    Trigger organization-level webhooks for lead events
// @access  Private
const triggerOrganizationLeadWebhooks = async (organizationId, event, leadData) => {
    try {
        const webhooks = await WebhookConfig.find({
            organizationId,
            isEnabled: true,
        ...
        console.error('Error triggering organization lead webhooks:', error);
        throw error;
    }
};
