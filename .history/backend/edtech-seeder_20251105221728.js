...
export const runEdtechSeeder = async () => {
    try {
        ...
        if (process.env.NODE_ENV === 'production') {
            console.log('🔄 Skipping edtech seeder in production environment');
            return;
        }
        ...
        if (existingOrg) {
            console.log('✅ Organization org-1 already exists, ensuring subscription plans and settings...');

            // Always ensure subscription plans exist
            const plansCount = await SubscriptionPlan.countDocuments({});
            if (plansCount === 0) {
                console.log('💳 Creating subscription plans...');
                const plans = [
            ...
            // Always ensure organization has enterprise plan
            if (existingOrg.subscriptionPlanId !== 'enterprise') {
                ...
                console.log('✅ Updated org-1 to enterprise plan with all features');
            } else {
