...
router.put('/stages/:id', async (req, res) => {
  try {
    const stage = await Stage.findOneAndUpdate({ id: req.params.id }, req.body, { new: true });
...
router.put('/custom-fields/:id', async (req, res) => {
  try {
    const customField = await CustomField.findOneAndUpdate({ id: req.params.id }, req.body, { new: true });
...
router.delete('/custom-fields/:id', async (req, res) => {
  try {
    const customField = await CustomField.findOneAndDelete({ id: req.params.id });
    if (!customField) return res.status(404).json({ success: false, message: 'Custom field not found' });
...
router.put('/automation-rules/:id', async (req, res) => {
  try {
    const rule = await AutomationRule.findOneAndUpdate({ id: req.params.id }, req.body, { new: true });
...
router.put('/organizations/:id', async (req, res) => {
  try {
    const organization = await Organization.findOneAndUpdate({ id: req.params.id }, req.body, { new: true });
