...
export const runSeeder = async () => {
    try {
        ...
        if (userCount > 0) {
            console.log('Data already exists. Seeder skipped.');
            return;
        }
        ...
        const leadIdMap = LEADS.reduce((acc, mockLead, index) => {
            acc[mockLead.id] = createdLeads[index]._id;
            return acc;
        }, {});

        const tasksWithMongoLeadIds = TASKS.map(task => ({
            ...task,
            leadId: leadIdMap[task.leadId] || null,
        }));
