...
// Emergency throttling middleware for infinite loop protection
const emergencyThrottle = (req, res, next) => {
  const clientKey = `${req.ip}:${req.user?.id || 'anonymous'}:${req.path}`;
  const now = Date.now();
  const windowMs = 1000; // 1 second window
  const maxRequests = 5; // Max 5 requests per second per client per endpoint
  ...
  if (clientData.requests.length >= maxRequests) {
    // Block the client for 30 seconds
    clientData.blocked = true;
    clientData.blockUntil = now + 30000;
    ...
    return res.status(429).json({
      success: false,
      message: 'Emergency rate limit exceeded. Client blocked for 30 seconds.',
      retryAfter: 30
    });
  }
...
const requestDeduplication = (req, res, next) => {
  const requestKey = `${req.ip}:${req.method}:${req.path}:${req.user?.id || 'anonymous'}:${JSON.stringify(req.body)}`;
  const requestHash = require('crypto').createHash('md5').update(requestKey).digest('hex');
  const now = Date.now();
  ...
  const existingRequest = store.get(requestHash);
  if (existingRequest && (now - existingRequest.timestamp) < 500) { // 500ms deduplication window
  ...
  // Store this request
  store.set(requestHash, { timestamp: now });
...
// Emergency rate limiting for infinite loop protection
const emergencyLimiter = rateLimit({
  ...
  legacyHeaders: false,
  skip: (req) => {
    // Only skip for health checks and static content
    return req.path === '/api/health' || req.path.startsWith('/privacy') || req.path.startsWith('/facebook/data-deletion');
  },
  keyGenerator: (req) => {
    // Use the built-in IP key generator for IPv6 compatibility
    const ip = rateLimit.helpers.ipKeyGenerator(req);
    return `${ip}:${req.user?.id || 'anonymous'}:${req.path}`;
  }
});
...
app.use('/api/leads/:id', (req, res, next) => {
