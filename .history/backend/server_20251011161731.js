...
// Request deduplication middleware - more permissive for normal usage
const requestDeduplication = (req, res, next) => {
  // Skip deduplication for authentication and essential API calls
  ...
  // Check if we have a recent identical request - increased window to 2 seconds
  const existingRequest = store.get(requestHash);
  if (existingRequest && (now - existingRequest.timestamp) < 2000) { // 2 second deduplication window
...
const emergencyLimiter = rateLimit({
  ...
  legacyHeaders: false,
  skip: (req) => {
    // Skip for health checks, static content, and common API endpoints
