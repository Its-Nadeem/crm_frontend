...
Clienn CRM('init', {
    ...
    /**
     * Update website form statistics after submission
     * @param {string} organizationId - Organization ID
     * @param {string} formId - Form ID
     * @param {Object} submissionData - Submission data
     * @private
     */
    async updateFormStatistics(organizationId, formId, submissionData) {
        try {
            ...
            if (form) {
                ...
                console.log('Form statistics updated for form:', formId);
            }
        } catch (error) {
            console.error('Failed to update form statistics:', error);
        }
    }
    ...
    async triggerLeadWorkflows(leadData) {
        try {
            // Import AutomationRule model dynamically
            const { default: AutomationRule } = await import('../models/AutomationRule.js');

            // Find active automation rules for website form submissions
            ...
            for (const rule of automationRules) {
                ...
                if (this.evaluateAutomationConditions(leadData, rule.conditions)) {
                    // Execute actions
                    await this.executeAutomationActions(leadData, rule.action);
                }
            }
     ...
     * @private
     */
    evaluateAutomationConditions(leadData, conditions) {
        // Simple condition evaluation - can be expanded
        return conditions.every(condition => {
            const fieldValue = leadData[condition.field] || leadData.customFields?.[condition.field];
     ...
     * @param {Object} leadData - Lead data
     * @param {Object} action - Automation action
     * @private
     */
    async executeAutomationActions(leadData, action) {
        try {
            switch (action.type) {
                case 'update_lead':
                    // Update lead properties
                    if (action.updates) {
                        await leadData.updateOne(action.updates);
                    }
                    break;
                case 'send_email':
                    // Would integrate with email service
                    console.log('Sending automation email to:', leadData.email);
                    break;
                case 'assign_user':
                    // Assign to specific user
                    if (action.userId) {
                        await leadData.updateOne({ assignedToId: action.userId });
                    }
                    break;
                case 'add_tag':
                    // Add tags to lead
                    if (action.tags) {
                        ...
                        await leadData.updateOne({ tags: newTags });
                    }
                    break;
                default:
    ...
    async getRealTimeStats(organizationId) {
        try {
            ...
            // Get recent submissions from last hour
            const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000);

            const forms = await WebsiteForm.find({
                organizationId,
                status: 'Active',
                'recentSubmissions.timestamp': { $gte: oneHourAgo }
            });

            const recentSubmissions = forms.reduce((total, form) => {
                return total + form.recentSubmissions.filter(sub =>
                    sub.timestamp >= oneHourAgo
                ).length;
            }, 0);

            // Get top performing forms by recent activity
            const topPages = forms
                .sort((a, b) => b.totalSubmissions - a.totalSubmissions)
                .slice(0, 5)
                .map(form => ({
                    url: form.url,
                    name: form.name,
                    views: form.totalSubmissions,
                    submissions: form.recentSubmissions.filter(sub =>
                        sub.timestamp >= oneHourAgo
                    ).length
                }));

            return {
                activeVisitors: 0, // Would need session tracking implementation
                currentPageViews: 0, // Would need real-time page view tracking
                recentSubmissions,
                topPages,
                topReferrers: [] // Would need referrer tracking implementation
            };
        } catch (error) {
