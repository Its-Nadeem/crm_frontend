...
const testApiEndpoints = async () => {
    try {
        ...
        // Test 5: Get Stages
        console.log('\n5. Testing GET /api/stages');
        try {
            const stagesResponse = await fetch(`${API_BASE_URL}/stages?organizationId=org-1`, { headers });
            if (stagesResponse.ok) {
                const stagesData = await stagesResponse.json();
                console.log(`   ✅ Stages endpoint working - Found ${stagesData.data?.length || 0} stages`);
            } else {
                console.log(`   ❌ Stages endpoint failed: ${statsResponse.status}`);
            }
        } catch (error) {
