...
import { HOMEPAGE_CONTENT as initialHomepageContent, BLOG_POSTS as initialBlogPosts, PRICING_COMPARISON_DATA as initialPricingData, INQUIRIES as initialInquiries, ADDONS as initialAddons, SUBSCRIPTION_PLANS as initialSubPlans, COUPONS as initialCoupons, OFFER_STRIP as initialOfferStrip } from './data';
...
const App: React.FC = () => {
    ...
    const [subscriptionPlans, setSubscriptionPlans] = useState<SubscriptionPlan[]>(initialSubPlans);
    ...
    const [offerStrip, setOfferStrip] = useState<OfferStrip>(initialOfferStrip);
    const [pricingComparisonData, setPricingComparisonData] = useState<PricingCategory[]>(initialPricingData);
        ...
        const fetchData = async () => {
            if (currentUser && authToken) {
                try {
                    ...
                    setCustomDomains(appData.customDomains || []);
                    ...
                    if (currentUser.role === UserRole.SUPER_ADMIN) {
                        ...
                        setOfferStrip(appData.offerStrip || {isEnabled: false, text: ''});
...
// FIX: Made handleLogin async to match the prop type required by LoginPage.
    const handleLogin = useCallback(async (email: string, pass: string): Promise<string | undefined> => {
        try {
            const data = await apiRequest('/api/users/login', 'POST', { email, password: pass });
    ...
    const hasFeature = useCallback((feature: SubscriptionPlan['features'][0]): boolean => {
        ...
        if (currentOrganization.manuallyAssignedFeatures?.includes(feature)) return true;
    ...
    const createOrUpdateItem = async <T extends { _id?: string }>(endpoint: string, item: Partial<T>, setItems: React.Dispatch<React.SetStateAction<T[]>>) => {
        const method = item._id ? 'PUT' : 'POST';
        const url = item._id ? `${endpoint}/${item._id}` : endpoint;
    ...
    const onUpdateLead = (updatedLead: Lead) => createOrUpdateItem('/api/leads', updatedLead, setLeads);
    ...
    const onAddStage = async () => {
        const newStage = { name: 'New Stage', color: '#cccccc' };
    ...
    const onDeleteStage = (id: string) => deleteItem('/api/settings/stages', id, setStages);
    ...
    const mainContent = (
        <Routes>
            ...
            <Route path="/settings" element={<SettingsLayout />}>
                ...
                <Route path="domains" element={<CustomDomainsPage domains={customDomains} onAddDomain={(domain) => createOrUpdateItem('/api/settings/domains', { domain, status: 'pending' }, setCustomDomains)} onDeleteDomain={(id) => deleteItem('/api/settings/domains', id, setCustomDomains)} onVerifyDomain={(id) => createOrUpdateItem('/api/settings/domains', { _id: id, status: 'verified' }, setCustomDomains)} />} />
            </Route>
