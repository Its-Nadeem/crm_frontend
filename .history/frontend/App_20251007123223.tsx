...
// FIX: Made handleLogin async to match the prop type required by LoginPage.
    ...
    const createOrUpdateItem = async <T extends { _id?: string; id?: string | number }>(endpoint: string, item: Partial<T>, setItems: React.Dispatch<React.SetStateAction<T[]>>) => {
        ...
        console.log('Creating/Updating item:', { endpoint, method, item: item });
            ...
            if (method === 'PUT') {
                ...
                return prev.map(i => {
                    if (i._id === savedItem._id || (i as any).id === (savedItem as any).id) {
                        console.log('Updating existing item in state:', savedItem);
                        return savedItem;
                    }
                    return i;
                });
    ...
    const onUpdateLead = async (updatedLead: Lead, oldLead?: Lead) => {
        console.log('onUpdateLead called with:', updatedLead);
        const result = await createOrUpdateItem('/api/leads', updatedLead, setLeads);
        console.log('onUpdateLead result:', result);
        return result;
    };
    ...
    const refreshLeadById = async (leadId: string) => {
        if (currentUser && authToken) {
            try {
                console.log('Refreshing lead by ID:', leadId);
                const updatedLead = await apiRequest(`/api/leads/${leadId}`);
                console.log('Refreshed lead data:', updatedLead);
                setLeads(prev => {
                    const newLeads = prev.map(lead =>
                        lead.id === leadId ? updatedLead : lead
                    );
                    console.log('Updated leads state:', newLeads.find(l => l.id === leadId));
                    return newLeads;
                });
                return updatedLead;
            } catch (error) {
