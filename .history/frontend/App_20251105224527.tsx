        ...
        const fetchData = async () => {
            if (currentUser && authToken) {
                try {
                    ...
                    console.log('DEBUG: Raw stages data from backend:', appData.stages?.length || 0, appData.stages?.map((s: any) => ({ id: s.id, name: s.name })));
...
// FIX: Made handleLogin async to match the prop type required by LoginPage.
    ...
    // Track if we've already logged the subscription plan warning to avoid spam
    const hasLoggedPlanWarning = useRef(false);
    ...
    const hasFeature = useCallback((feature: SubscriptionPlan['features'][0]): boolean => {
        ...
        // Find the subscription plan - handle both formats (with and without 'plan_' prefix)
        let plan = subscriptionPlans.find(p => p.id === currentOrganization.subscriptionPlanId);

        // If not found, try the alternative format
        if (!plan) {
            const alternativeId = currentOrganization.subscriptionPlanId.startsWith('plan_')
                ? currentOrganization.subscriptionPlanId.substring(5) // Remove 'plan_' prefix
                : `plan_${currentOrganization.subscriptionPlanId}`; // Add 'plan_' prefix

            plan = subscriptionPlans.find(p => p.id === alternativeId);
        }

        if (!plan) {
            // Only log the warning once to avoid spam
            if (!hasLoggedPlanWarning.current) {
                ...
                hasLoggedPlanWarning.current = true;
            }
            return false;
        }
    ...
    const createOrUpdateItem = async <T extends { _id?: string; id?: string | number }>(endpoint: string, item: Partial<T>, setItems: React.Dispatch<React.SetStateAction<T[]>>) => {
        ...
        // For teams, we need to be more careful about the id field
        const itemId = item.id;
        const item_id = item._id;

        // More robust check for valid ID
        const hasValidId = !!(item_id || (itemId && typeof itemId === 'string' && itemId.trim() !== '' && itemId !== 'undefined' && itemId !== 'null'));
        ...
        console.log('createOrUpdateItem Debug:', {
            ...
            itemIdLength: typeof itemId === 'string' ? itemId.length : 'N/A'
        });

        const isUpdate = hasValidId;
        const method = isUpdate ? 'PUT' : 'POST';
        const identifier = hasValidId ? (item_id || itemId) : undefined;
        const url = isUpdate && identifier ? `${endpoint}/${identifier}` : endpoint;
    ...
    const deleteItem = async <T extends { _id?: string; id?: string | number }>(endpoint: string, itemId: string, setItems: React.Dispatch<React.SetStateAction<T[]>>) => {
        await apiRequest(`${endpoint}/${itemId}`, 'DELETE');
        setItems(prev => prev.filter(i => i._id !== itemId && (i as any).id !== itemId));
    };
    
    const addLead = async (leadData: Omit<Lead, 'id' | '_id' | 'createdAt' | 'updatedAt' | 'organizationId' | 'activities' | 'score' | 'closeDate'>) => {
        await createOrUpdateItem('/api/leads', leadData, setLeads);
    };
    const onUpdateLead = async (updatedLead: Lead, oldLead?: Lead) => {
        ...
        setLeads(prev => {
            if (!Array.isArray(prev)) {
                console.error('Leads state is not an array:', prev);
                return prev;
            }
            return prev.map(lead => {
                if (lead.id === updatedLead.id || lead._id === updatedLead._id) {
                    return updatedLead;
                }
                return lead;
            });
    ...
    const onUpdateTask = async (updatedTask: Task) => {
        await createOrUpdateItem('/api/tasks', updatedTask, setTasks);
    };
    ...
    const onBulkDelete = async (leadIds: string[]) => {
        await apiRequest('/api/leads/bulk', 'DELETE', { leadIds });
    ...
    if (currentUser.role === UserRole.SUPER_ADMIN) {
        return <SuperAdminDashboard
            ...
            errorLogs={errorLogs}
            globalAutomationRules={globalAutomationRules}
            globalEmailTemplates={globalEmailTemplates}
            homepageContent={homepageContent}
            blogPosts={blogPosts}
            paymentGatewaySettings={paymentGatewaySettings}
            inquiries={inquiries}
            pricingComparisonData={pricingComparisonData}
            setInquiries={setInquiries}
            setLeads={setLeads}
            onLogout={handleLogout}
            onAddOrganization={(org) => createOrUpdateItem('/api/super-admin/organizations', org, setOrganizations)}
            onUpdateOrganization={(org) => createOrUpdateItem('/api/super-admin/organizations', org, setOrganizations)}
            onDeleteOrganization={(id) => deleteItem('/api/super-admin/organizations', id, setOrganizations)}
            onSaveSubscriptionPlan={onSaveSubscriptionPlan}
            onDeleteSubscriptionPlan={onDeleteSubscriptionPlan}
            onUpdateTicketStatus={(id, status) => createOrUpdateItem('/api/super-admin/support-tickets', { _id: id, status }, setSupportTickets)}
            ...
            onSaveGlobalRule={onSaveGlobalRule}
            onDeleteGlobalRule={onDeleteGlobalRule}
            onUpdatePricingComparisonData={onUpdatePricingComparisonData}
            ...
            onUpdateOfferStrip={onUpdateOfferStrip}
            fetchAnalyticsData={async () => ({ totalOrganizations: organizations.length, activeOrganizations: organizations.filter(o => new Date(o.subscriptionExpiresAt) > new Date()).length, mrrData: [] })}
        />
    }
    ...
    const mainContent = (
        <Routes>
            ...
            <Route path="/leads" element={<LeadListPage leads={leads} users={users} pipelineStages={stages} customFieldDefs={customFieldDefs} tasks={tasks} whatsAppTemplates={whatsAppTemplates} smsTemplates={smsTemplates} addLead={addLead} deleteLead={deleteLead} currentUser={currentUser} hasPermission={hasPermission} savedFilters={savedFilters} onSaveFilter={async (f) => { await createOrUpdateItem('/api/settings/filters', f, setSavedFilters); }} onDeleteFilter={(id) => deleteItem('/api/settings/filters', id, setSavedFilters)} onBulkAssign={onBulkAssign} onBulkDelete={onBulkDelete} onUpdateLead={onUpdateLead} onUpdateTask={onUpdateTask} onAddTask={onAddTask} onScheduleMessage={onScheduleMessage} onImportLeads={async () => {}} />} />
            ...
            <Route path="/tasks" element={<TasksPage tasks={tasks} users={users} leads={leads} teams={teams} currentUser={currentUser} updateTask={onUpdateTask} onAddTask={onAddTask} onDeleteTask={onDeleteTask} pipelineStages={stages} customFieldDefs={customFieldDefs} whatsAppTemplates={whatsAppTemplates} smsTemplates={smsTemplates} onUpdateLead={onUpdateLead} onScheduleMessage={onScheduleMessage} />} />
            ...
            <Route path="/marketing" element={<MarketingLayout />}>
                <Route path="email" element={<EmailPage users={users} pipelineStages={stages} customFieldDefs={customFieldDefs} />} />
                <Route path="sms" element={<SMSPage users={users} pipelineStages={stages} customFieldDefs={customFieldDefs} />} />
                <Route path="whatsapp" element={<WhatsAppPage customFieldDefs={customFieldDefs} users={users} pipelineStages={stages} />} />
                <Route path="call" element={<CallPage users={users} pipelineStages={stages} customFieldDefs={customFieldDefs} />} />
            </Route>
            <Route path="/blog" element={<BlogListPage blogPosts={blogPosts} users={users} />} />
            <Route path="/blog/:slug" element={<BlogPostPage blogPosts={blogPosts} users={users} />} />
            <Route path="/api-docs" element={<ApiDocsPage />} />
            <Route path="/settings" element={<SettingsLayout />}>
                <Route index element={<SettingsPage pipelineStages={stages} onAddStage={onAddStage} onUpdateStage={onUpdateStage} onDeleteStage={onDeleteStage} customFieldDefs={customFieldDefs} onAddCustomField={(type) => createOrUpdateItem('/api/settings/custom-fields', {name: 'New Field', type, isMappable: true, isRequired: false}, setCustomFieldDefs)} onUpdateCustomField={(id, data) => createOrUpdateItem('/api/settings/custom-fields', {_id: id, ...data}, setCustomFieldDefs)} onDeleteCustomField={(id) => deleteItem('/api/settings/custom-fields', id, setCustomFieldDefs)} leadScoreRules={leadScoreRules} onSaveLeadScoreRule={(r) => createOrUpdateItem('/api/settings/score-rules', r, setLeadScoreRules)} onDeleteLeadScoreRule={(id) => deleteItem('/api/settings/score-rules', id, setLeadScoreRules)} theme={theme} toggleTheme={toggleTheme} currentUser={currentUser} hasPermission={hasPermission} currentOrganization={currentOrganization} subscriptionPlans={subscriptionPlans} onUpdateProfile={(data) => createOrUpdateItem('/api/users/profile', { ...currentUser, ...data }, (updatedUsers) => setUsers(prev => prev.map(u => u.id === currentUser.id ? updatedUsers[0] : u))).then(() => setCurrentUser(p => p ? {...p, ...data} : null))} billingHistory={billingHistory} addons={addons} onUpdateOrganization={(data) => apiRequest(`/api/organizations/${currentOrganization._id}`, 'PUT', data).then(setOrganizations)} onUpgradeSubscription={(planId, billingCycle) => {}} onRefreshData={() => {}} />} />
                ...
                <Route path="integrations/facebook" element={<FacebookIntegrationPage settings={integrationSettings} setSettings={setIntegrationSettings} customFieldDefs={customFieldDefs} currentOrganization={currentOrganization} onSendTestLead={() => {}} />} />
                <Route path="integrations/google-ads" element={<GoogleAdsIntegrationPage settings={integrationSettings} setSettings={setIntegrationSettings} customFieldDefs={customFieldDefs} currentOrganization={currentOrganization} onSendTestLead={() => {}} />} />
                <Route path="integrations/website" element={<WebsiteIntegrationPage settings={integrationSettings} setSettings={setIntegrationSettings} customFieldDefs={customFieldDefs} currentOrganization={currentOrganization} onSendTestLead={() => {}} />} />
                <Route path="integrations/cloud-telephony" element={<CloudTelephonyIntegrationPage settings={integrationSettings} setSettings={setIntegrationSettings} customFieldDefs={customFieldDefs} currentOrganization={currentOrganization} onSendTestLead={() => {}} />} />
                <Route path="integrations/email-marketing" element={<EmailMarketingIntegrationPage settings={integrationSettings} setSettings={setIntegrationSettings} customFieldDefs={customFieldDefs} currentOrganization={currentOrganization} onSendTestLead={() => {}} />} />
                <Route path="integrations/sms-marketing" element={<SMSMarketingIntegrationPage settings={integrationSettings} setSettings={setIntegrationSettings} customFieldDefs={customFieldDefs} currentOrganization={currentOrganization} onSendTestLead={() => {}} />} />
                <Route path="integrations/api-webhooks" element={<ApiWebhooksIntegrationPage settings={integrationSettings} setSettings={setIntegrationSettings} webhooks={webhooks} onAddWebhook={(webhookData) => createOrUpdateItem('/api/webhooks', webhookData, setWebhooks)} onDeleteWebhook={(id) => deleteItem('/api/webhooks', id, setWebhooks)} onUpdateWebhook={(wh) => createOrUpdateItem('/api/webhooks', wh, setWebhooks)} onTestWebhook={async (id) => ({ success: true, status: 200, responseTime: 150 })} currentOrganization={currentOrganization} />} />
