...
const PublicLayout: React.FC<PublicLayoutProps> = ({ theme, toggleTheme, content, offerStrip }) => {
    ...
    const isOfferActive = offerStrip.isEnabled && !isOfferDismissed && (!offerStrip.autoDisableAt || new Date() < new Date(offerStrip.autoDisableAt));
                                ...
                                <ul className="mt-4 space-y-3 text-sm">
                                    {col.links.map(link => {
                                        ...
                                        if (url.startsWith('#') && url.length > 1) {
                                            const path = `/${url}`;
                                            return <li key={text}><Link to={path} className="text-subtle hover:text-on-surface transition-colors">{text}</a></li>;
                                        }
