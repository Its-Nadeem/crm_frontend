...
const MOCK_CSV_HEADERS = ['full_name', 'email_address', 'phone', 'company', 'city', 'inquiry_details', 'source', 'budget'];
const MOCK_CSV_ROWS = Array.from({ length: 3 }, () => ({
    full_name: faker.person.fullName(),
    email_address: faker.internet.email().toLowerCase(),
    phone: faker.phone.number(),
    company: faker.company.name(),
    city: faker.location.city(),
    inquiry_details: faker.lorem.sentence(),
    source: faker.helpers.arrayElement(Object.values(LeadSource)),
    budget: faker.number.int({ min: 1000, max: 50000 }).toString(),
}));

// Step 1: Upload
const UploadStep: React.FC<{ onNext: () => void }> = ({ onNext }) => (
    <div className="text-center">
        <AppIcons.Upload className="h-12 w-12 mx-auto text-subtle mb-4" />
        <h3 className="text-lg font-semibold text-on-surface">Upload your file</h3>
        <p className="text-sm text-subtle mt-1">Supported formats: CSV, XLS, XLSX. (This is a simulation)</p>
        <button onClick={onNext} className="mt-6 bg-primary-600 hover:bg-primary-700 text-white font-bold py-2 px-6 rounded-lg">
            Simulate Upload & Continue
        </button>
    </div>
);
