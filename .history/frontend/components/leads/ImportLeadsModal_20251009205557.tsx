...
const ImportLeadsModal: React.FC<ImportLeadsModalProps> = ({
    ...
    const handleImport = async () => {
        if (!fileData || !modernMappings) return;
        ...
        try {
            // Convert modern mappings to old format for compatibility
            ...
            fileData.rows.forEach((row, index) => {
                // Convert modern mappings to old format for validation
                const oldFormatMappings = mappedFields.map(m => ({
            ...
            fileData.rows.forEach((row, index) => {
                try {
                    ...
                    mappedFields.forEach(mapping => {
                        const value = row[mapping.csvColumnName];
                        if (value && mapping.crmFieldId) {
                            if (mapping.crmFieldId.startsWith('customFields.')) {
                                const fieldId = mapping.crmFieldId.replace('customFields.', '');
                                leadData.customFields = leadData.customFields || {};
                                leadData.customFields[fieldId] = value;
                            } else {
                                (leadData as any)[mapping.crmFieldId] = value;
                            }
                        }
                    });
