                                ...
                                >
                                    ...
                                    {fileData.headers.map(header => (
                                        <option key={header} value={header}>
                                            {header} {sampleValue && currentMapping === header ? `(${sampleValue})` : ''}
                                        </option>
                                    ))}
                                </select>
                                ...
                                <div className="text-xs text-subtle">
                                    {crmField.id === 'course' && (
                                        <div className="space-y-1">
                                            ...
                                            <div className="text-xs">MBA, BBA, BCA, MCA, etc.</div>
                                        </div>
                                    )}
                                    {crmField.id === 'stage' && (
                                        <div className="space-y-1">
                                            <div className="font-medium">Pipeline Stages:</div>
                                            <div className="text-xs">New Lead, Qualified, Proposal Sent, etc.</div>
                                        </div>
                                    )}
                                    {crmField.id === 'source' && (
                                        <div className="space-y-1">
                                            <div className="font-medium">Lead Sources:</div>
                                            <div className="text-xs">Facebook, Google, Website, Referral, etc.</div>
                                        </div>
                                    )}
                                    {crmField.id.startsWith('customFields.') && (
                                        <div className="space-y-1">
                                            <div className="font-medium">Custom Field:</div>
                                            <div className="text-xs">User-defined field values</div>
                                        </div>
                                    )}
                                    {!['course', 'stage', 'source'].includes(crmField.id) && !crmField.id.startsWith('customFields.') && (
                                        <div className="text-xs text-muted">Text input field</div>
                                    )}
                                </div>
                            </div>
                        );
