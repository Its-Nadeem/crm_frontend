                ...
                <div className="lg:col-span-2 flex flex-col overflow-hidden bg-background rounded-lg border border-muted">
                    ...
                    <div className="flex-grow overflow-y-auto p-4 space-y-6">
                        {activeTab === 'activity' && (
                            <div>
                                {isAddingNote && (
                                    <div className="p-2 space-y-2 border-b border-muted mb-4">
                                        <textarea
                                        ...
                                        />
                                        <div className="flex justify-end gap-2">
                                            <button onClick={() => setIsAddingNote(false)} className="bg-muted hover:bg-subtle/20 text-on-surface font-semibold py-1.5 px-3 rounded-lg text-sm">Cancel</button>
                                            <button onClick={(e) => {
                                                const noteContent = (e.currentTarget.previousElementSibling?.previousElementSibling?.querySelector('textarea') as HTMLTextAreaElement)?.value.trim();
                                                if (noteContent) {
                                                    handleAddNote(noteContent);
                                                }
                                            }} className="bg-primary-600 hover:bg-primary-700 text-white font-semibold py-1.5 px-3 rounded-lg text-sm">Add Note</button>
                                        </div>
                                    </div>
                                )}
