    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (changeLog.length > 0) {
                ...
                // Update with activity in local state only (no additional API call)
                setHasUnsavedChanges(false);
                setChangeLog([]);
                setShowSaveSuccess(true);
                console.log('Save process completed successfully - no additional API call needed');
            } else {
                // Update in local state only (no additional API call)
                setHasUnsavedChanges(false);
                setChangeLog([]);
                setShowSaveSuccess(true);
                console.log('Save process completed successfully - no changes to sync');
            }
