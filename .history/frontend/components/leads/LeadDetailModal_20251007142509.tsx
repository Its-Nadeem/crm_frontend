...
const WhatsAppModal: React.FC<{
    ...
    const handleSend = () => {
        ...
        alert(`WhatsApp message sent to ${lead.name} at ${lead.phone}`);
        onSend(message);
    };
...
const LeadDetailModal: React.FC<LeadDetailModalProps> = ({ lead, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onScheduleMessage, currentUser, onClose }) => {
    ...
    // Load notes and calls for the lead
    const loadNotesAndCalls = useCallback(async () => {
        try {
            const [notesData, callsData] = await Promise.all([
            ...
            console.error('Error loading notes and calls:', error);
        }
    }, [lead.id]);
    ...
    const handleSendWhatsApp = async (message: string) => {
        try {
            ...
            alert(`Failed to log WhatsApp message: ${error.message || 'Please try again.'}`);
        }
    };
    ...
    const handleSendSMS = async (message: string) => {
        try {
            ...
            alert(`Failed to log SMS message: ${error.message || 'Please try again.'}`);
        }
    };
   ...
   const handleLogCall = async (outcome: string, notes: string) => {
       try {
           ...
           alert(`Failed to log call: ${error.message || 'Please try again.'}`);
       }
   };
   ...
   const handleAddNote = async (noteContent: string) => {
       try {
           ...
           alert(`Failed to add note: ${error.message || 'Please try again.'}`);
       }
   };
                    ...
                    <div className="flex-grow overflow-y-auto p-4 space-y-6">
                        {activeTab === 'activity' && (
                            <div>
                                ...
                                {/* Display lead activities and notes/calls */}
                                {(editableLead.activities || []).map((activity, index) => (
                                    <ActivityItem key={`activity-${activity.id}-${index}`} activity={activity} author={users.find(u => u.id === activity.authorId)} isFirst={index === (editableLead.activities || []).length -1} />
                                ))}

                                {/* Display notes */}
                                {notes.map((note, index) => (
                                    <ActivityItem
                                        key={`note-${note._id || note.id}-${index}`}
                                        activity={{
                                            id: note._id || note.id,
                                            type: 'NOTE',
                                ...
                                {calls.map((call, index) => (
                                    <ActivityItem
                                        key={`call-${call._id || call.id}-${index}`}
                                        activity={{
                                            id: call._id || call.id,
                                            type: 'CALL',
                                        ...
                                        author={users.find(u => u.id === call.authorId)}
                                        isFirst={false}
                                    />
                                ))}
                            </div>
