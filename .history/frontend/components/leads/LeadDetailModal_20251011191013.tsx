...
const ActivityItem: React.FC<{ activity: Activity; author?: User; isFirst: boolean; users: User[] }> = ({ activity, author, isFirst, users }) => {
    const getIcon = () => {
    ...
    const getAuthorName = (authorId: number, users: User[]) => {
        ...
        <div className="flex items-start space-x-4 relative">
            ...
            <div className="flex-grow pt-1.5">
                ...
                <p className="text-xs text-subtle mt-1">
                    {getAuthorName(activity.authorId, users)} • {formatDate(activity.timestamp)}
                </p>
            </div>
        </div>
    );
