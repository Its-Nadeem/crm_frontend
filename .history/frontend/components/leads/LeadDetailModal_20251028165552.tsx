...
import { ActivityTimeline } from '../../src/components/leads/ActivityTimeline';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Call the callback to refresh data
             ...
             // Trigger timeline refresh for new task
             if (window.dispatchEvent) {
                 window.dispatchEvent(new CustomEvent('timelineRefresh', { detail: { leadId: lead.id } }));
             }
...
const LeadDetailModal: React.FC<LeadDetailModalProps> = ({ lead, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onScheduleMessage, currentUser, onClose }) => {
    const [activeTab, setActiveTab] = useState<'timeline' | 'tasks' | 'scheduled'>('timeline');
    ...
    // Load notes, calls, and tasks for the lead
     const loadNotesAndCalls = useCallback(async () => {
         try {
             ...
             // Trigger timeline refresh if available
             if (window.dispatchEvent) {
                 window.dispatchEvent(new CustomEvent('timelineRefresh', { detail: { leadId: lead.id } }));
             }
         } catch (error) {
    ...
    const handleFieldChange = (field: keyof Lead, value: any) => {
        ...
        if (oldValue !== newValue) {
            ...
            // Create change description and immediate activity for ALL field changes
            ...
            switch (field) {
                case 'assignedToId':
                    ...
                    activityContent = `Owner changed from ${oldUser?.name || 'Unassigned'} to ${newUser?.name || 'Unassigned'}`;
                    break;
                case 'stage':
                    ...
                    activityContent = `Pipeline stage changed from ${oldStage?.name || 'Unknown'} to ${newStage?.name || 'Unknown'}`;
                    break;
                case 'name':
                    changeDescription = `Name updated`;
                    activityContent = `Name changed from "${oldValue}" to "${newValue}"`;
                    break;
                case 'email':
                    changeDescription = `Email updated`;
                    activityContent = `Email changed from "${oldValue}" to "${newValue}"`;
                    break;
                case 'phone':
                    changeDescription = `Phone updated`;
                    activityContent = `Phone changed from "${oldValue}" to "${newValue}"`;
                    break;
                case 'dealValue':
                    changeDescription = `Deal value updated`;
                    activityContent = `Deal value changed from $${oldValue || 0} to $${newValue || 0}`;
                    break;
                case 'course':
                    changeDescription = `Course interest updated`;
                    activityContent = `Course interest changed from "${oldValue || ''}" to "${newValue || ''}"`;
                    break;
                case 'company':
                    changeDescription = `Company updated`;
                    activityContent = `Company changed from "${oldValue || ''}" to "${newValue || ''}"`;
                    break;
                case 'city':
                    changeDescription = `City updated`;
                    activityContent = `City changed from "${oldValue || ''}" to "${newValue || ''}"`;
                    break;
                case 'alternatePhone':
                    changeDescription = `Alternate phone updated`;
                    activityContent = `Alternate phone changed from "${oldValue || ''}" to "${newValue || ''}"`;
                    break;
                case 'campaign':
                    changeDescription = `Campaign updated`;
                    activityContent = `Campaign changed from "${oldValue || ''}" to "${newValue || ''}"`;
                    break;
                case 'tags':
                    changeDescription = `Tags updated`;
                    activityContent = `Tags changed from "${Array.isArray(oldValue) ? oldValue.join(', ') : oldValue || ''}" to "${Array.isArray(newValue) ? newValue.join(', ') : newValue || ''}"`;
                    break;
                case 'facebookCampaign':
                    changeDescription = `Facebook campaign updated`;
                    activityContent = `Facebook campaign changed from "${oldValue || ''}" to "${newValue || ''}"`;
                    break;
                case 'facebookAdset':
                    changeDescription = `Facebook adset updated`;
                    activityContent = `Facebook adset changed from "${oldValue || ''}" to "${newValue || ''}"`;
                    break;
                case 'facebookAd':
                    changeDescription = `Facebook ad updated`;
                    activityContent = `Facebook ad changed from "${oldValue || ''}" to "${newValue || ''}"`;
                    break;
                default:
                    changeDescription = `${field} updated`;
                    activityContent = `${field} changed from "${oldValue}" to "${newValue}"`;
            }

            // Create immediate activity for timeline display
            const fieldUpdateActivity: Activity = {
                id: `field-update-${field}-${Date.now()}`,
                type: 'FIELD_UPDATE',
            ...
            // Add to editable lead activities immediately for timeline display
            setEditableLead(prev => ({
                ...prev,
                activities: [fieldUpdateActivity, ...(prev.activities || [])]
            }));
    ...
    const handleCustomFieldChange = (id: string, value: any) => {
        ...
        if (oldValue !== value) {
            ...
            // Create custom field update activity for immediate timeline display
            const customFieldActivity: Activity = {
                id: `custom-field-update-${id}-${Date.now()}`,
                type: 'FIELD_UPDATE',
                content: `Custom field "${customFieldDef?.name || id}" updated from "${oldValue}" to "${value}"`,
                timestamp: new Date().toISOString(),
                authorId: currentUser.id
            };
            ...
            setEditableLead(prev => ({
                ...prev,
                activities: [customFieldActivity, ...(prev.activities || [])]
            }));
        }
    };
    ...
    const handleSendWhatsApp = async (message: string) => {
        try {
            ...
            const newActivity: Activity = {
                ...
                type: 'WHATSAPP',
                content: `WhatsApp message sent: <i class="whitespace-pre-wrap">"${message}"</i>`,
                timestamp: new Date().toISOString(),
                authorId: currentUser.id
            };
            ...
            // Trigger instant timeline refresh
            if (window.dispatchEvent) {
                window.dispatchEvent(new CustomEvent('activityUpdate', { detail: { leadId: lead.id } }));
            }
    ...
    const handleSendSMS = async (message: string) => {
        try {
            ...
            const newActivity: Activity = {
                ...
                type: 'SMS',
                content: `SMS message sent: <i class="whitespace-pre-wrap">"${message}"</i>`,
                timestamp: new Date().toISOString(),
                authorId: currentUser.id
            };
   ...
   const handleLogCall = async (outcome: string, notes: string) => {
       try {
           ...
           const callActivity: Activity = {
               ...
               type: 'CALL',
               content: `Call logged. Outcome: <strong>${outcome}</strong>. Notes: <i class="whitespace-pre-wrap">"${notes || ''}"</i>`,
               timestamp: new Date().toISOString(),
               authorId: currentUser.id
           };
           ...
           setEditableLead(prev => ({
               ...prev,
               activities: [callActivity, ...(prev.activities || [])]
           }));
                ...
                <div className="lg:col-span-2 flex flex-col overflow-hidden bg-background rounded-lg border border-muted">
                    <div className="flex-shrink-0 p-2 flex flex-wrap items-center justify-between gap-2 border-b border-muted">
                        <div className="flex items-center">
                             <button onClick={() => setActiveTab('timeline')} className={`relative px-3 py-1.5 text-sm font-semibold rounded-md ${activeTab === 'timeline' ? 'bg-primary-600 text-white' : 'text-subtle hover:bg-muted'}`}>
                                Timeline
                             </button>
