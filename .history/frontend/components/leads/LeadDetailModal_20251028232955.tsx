   ...
   const handleAddNote = async (noteContent: string) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Trigger timeline refresh for field updates immediately after save
            console.log('LeadDetailModal: Dispatching timeline refresh event after save');
            if (window.dispatchEvent) {
                window.dispatchEvent(new CustomEvent('timelineRefresh', { detail: { leadId: editableLead.id } }));
            }

             // Hide success message after 2 seconds (but don't auto-close modal)
             setTimeout(() => {
