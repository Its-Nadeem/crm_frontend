...
const RichTextEditor: React.FC<{ value: string; onChange: (value: string) => void }> = ({ value, onChange }) => {
    ...
    const handleLinkSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (linkUrl.trim()) {
            handleFormat('createLink', linkUrl.trim());
            setLinkUrl('');
            setShowLinkModal(false);
            addToast({
                type: 'success',
                title: 'Link Added',
                message: 'Link has been successfully added to the selected text.'
            });
        }
    };
        ...
        <div className="bg-surface rounded-lg border border-muted flex flex-col flex-grow min-h-[600px]">
            <div className="p-2 border-b border-muted flex gap-1 flex-wrap flex-shrink-0">
                ...
                <button type="button" onClick={() => handleFormat('formatBlock', '<blockquote>')} className="p-1 rounded hover:bg-muted w-8 h-8 flex items-center justify-center"><AppIcons.Quote className="h-4 w-4"/></button>
                 <div className="w-px bg-muted mx-1"></div>
                <button type="button" onClick={handleCreateLink} className="p-1 rounded hover:bg-muted w-8 h-8 flex items-center justify-center"><AppIcons.Link className="h-4 w-4"/></button>
            </div>
            <div
                ref={editorRef}
                contentEditable
            ...
            {showLinkModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-surface rounded-lg p-6 w-96 max-w-full mx-4">
                        <h3 className="text-lg font-semibold text-on-surface mb-4">Add Link</h3>
                        <form onSubmit={handleLinkSubmit}>
                            <div className="mb-4">
                                <label htmlFor="linkUrl" className="block text-sm font-medium text-subtle mb-2">
                                    URL
                                </label>
                                <input
                                    type="url"
                            ...
                            <div className="flex justify-end gap-3">
                                <button
                                    ...
                                    className="px-4 py-2 text-sm font-medium text-subtle hover:text-on-surface hover:bg-muted border border-muted rounded-lg transition-colors"
                                >
                                    Cancel
                                </button>
                                <button
