...
const AddWidgetModal: React.FC<{
    ...
    const handleSave = () => {
        ...
        onSave({
            id: existingWidget?.id || faker.string.uuid(),
            ...widget
                            ...
                            <div className={`transition-[max-height] duration-500 ease-in-out overflow-hidden ${chartsOpen.source ? 'max-h-[350px]' : 'max-h-0'}`}>
                                <div className="p-6 pt-4 h-[300px]">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            <Pie data={leadsBySource} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} fill="#8884d8" labelLine={false} label={({ cx, cy, midAngle, innerRadius, outerRadius, percent }) => {
                                            ...
                                            <Tooltip formatter={(value) => `${value} leads`} contentStyle={{backgroundColor: 'var(--surface)', border: '1px solid var(--muted)', borderRadius: '0.75rem'}} />
