...
const EmailPage: React.FC<EmailPageProps> = ({ users, pipelineStages, customFieldDefs }) => {
        ...
        const fetchData = async () => {
            try {
                ...
                const [templatesData, campaignsData] = await Promise.all([
                    apiService.request<any[]>('/marketing/templates/email'),
                    apiService.request<any[]>('/marketing/campaigns/email')
                ]);
    ...
    const handleSaveTemplate = async (template: EmailTemplate) => {
        try {
            if (template.id) {
                ...
                await apiService.request(`/marketing/templates/email/${template.id}`, {
                    method: 'PUT',
                ...
                // Create new template
                const newTemplate = await apiService.request('/marketing/templates/email', {
                    method: 'POST',
                    body: JSON.stringify(template)
                });
    ...
    const handleDeleteTemplate = async (templateId: string) => {
        ...
        try {
            await apiService.request(`/marketing/templates/email/${templateId}`, {
                method: 'DELETE'
            });
    ...
    const handleSaveCampaign = async (campaign: EmailCampaign) => {
        try {
            if (campaign.id) {
                ...
                await apiService.request(`/marketing/campaigns/email/${campaign.id}`, {
                    method: 'PUT',
                ...
                // Create new campaign
                const newCampaign = await apiService.request('/marketing/campaigns/email', {
                    method: 'POST',
                    body: JSON.stringify(campaign)
                });
    ...
    const handleDeleteCampaign = async (campaignId: string) => {
        ...
        try {
            await apiService.request(`/marketing/campaigns/email/${campaignId}`, {
                method: 'DELETE'
            });
