...
export const FacebookIntegrationPage: React.FC<FacebookIntegrationPageProps> = ({
        ...
        try {
            ...
            if (data.success) {
                const formattedAccounts = [{
                    ...
                    accessToken: 'fb-access-token', // This would come from the API in a real implementation
    ...
    const fetchMappings = async () => {
        ...
        try {
            const response = await fetch('/api/fb/mappings', {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const fetchForms = async (pageId: string) => {
        ...
        try {
            const response = await fetch(`/api/fb/forms?pageId=${pageId}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const fetchFacebookFields = async (formId: string) => {
        ...
        try {
            // For now, we'll use mock field data structure
            ...
            const mockFields = [
                ...
                { name: 'job_title', values: ['Software Engineer'] },
                { name: 'city', values: ['New York'] },
                { name: 'country', values: ['United States'] }
            ];

            setFacebookFields(mockFields);
        } catch (err) {
    ...
    const handleCreateMapping = async (mappingData: any) => {
        ...
        try {
            ...
            const response = await fetch(`/api/fb/forms/${formId}/map`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const handleDeleteMapping = async (mappingId: string) => {
        ...
        try {
            ...
            const response = await fetch(`/api/fb/forms/${mapping.formId}/map`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const handleSyncLeads = async () => {
        ...
        try {
            const response = await fetch('/api/fb/sync/backfill', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <div className="space-y-8 h-full flex flex-col min-h-0 bg-surface">
            {/* Header */}
                            ...
                            <div className="bg-background rounded-lg border border-muted p-6 shadow-sm">
                                <div className="flex items-center gap-4 mb-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h4 className="font-semibold text-on-surface">{accounts[0]?.name || 'Connected Facebook Account'}</h4>
                                    </div>
                                </div>

                                <div className="space-y-4 ml-11">
                                    {/* My Awesome Product Page */}
                                    <div className="flex items-center justify-between py-3 border-t border-muted/50">
                                        <div>
                                            <div className="font-medium text-on-surface">My Awesome Product Page</div>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <span className={`text-xs px-2 py-1 rounded-full ${accounts.find(acc => acc.id === '1')?.pages.find(p => p.id === 'page1')?.subscribed ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
                                                {accounts.find(acc => acc.id === '1')?.pages.find(p => p.id === 'page1')?.subscribed ? 'Subscribed' : 'Not Subscribed'}
                                            </span>
                                            <label className="relative inline-flex items-center cursor-pointer">
                                                <input
                                                    type="checkbox"
                                                    checked={accounts.find(acc => acc.id === '1')?.pages.find(p => p.id === 'page1')?.subscribed || false}
                                                    onChange={(e) => handleSubscriptionToggle('1', 'page1', e.target.checked)}
                                                    disabled={loading}
                                                    className="sr-only peer"
                                                />
                                                <div className={`w-10 h-5 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border after:rounded-full after:h-4 after:w-4 ${accounts.find(acc => acc.id === '1')?.pages.find(p => p.id === 'page1')?.subscribed ? 'bg-primary-600' : 'bg-muted'}`}></div>
                                            </label>
                                        </div>
                                    </div>

                                    {/* Secondary Business Page */}
                                    <div className="flex items-center justify-between py-3 border-t border-muted/50">
                                        <div>
                                            <div className="font-medium text-on-surface">Secondary Business Page</div>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <span className={`text-xs px-2 py-1 rounded-full ${accounts.find(acc => acc.id === '1')?.pages.find(p => p.id === 'page2')?.subscribed ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
                                                {accounts.find(acc => acc.id === '1')?.pages.find(p => p.id === 'page2')?.subscribed ? 'Subscribed' : 'Not Subscribed'}
                                            </span>
                                            <label className="relative inline-flex items-center cursor-pointer">
                                                <input
                                                    type="checkbox"
                                                    checked={accounts.find(acc => acc.id === '1')?.pages.find(p => p.id === 'page2')?.subscribed || false}
                                                    onChange={(e) => handleSubscriptionToggle('1', 'page2', e.target.checked)}
                                                    disabled={loading}
                                                    className="sr-only peer"
                                                />
                                                <div className={`w-10 h-5 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border after:rounded-full after:h-4 after:w-4 ${accounts.find(acc => acc.id === '1')?.pages.find(p => p.id === 'page2')?.subscribed ? 'bg-primary-600' : 'bg-muted'}`}></div>
                                            </label>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="bg-background rounded-lg border border-muted p-6 shadow-sm">
                                <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h4 className="font-semibold text-on-surface">{accounts[1]?.name || 'Second Facebook Account'}</h4>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs bg-red-100 text-red-700 px-2 py-1 rounded-full">Token Expired</span>
                                        <button
                                            onClick={() => handleReconnect('2')}
                                ...
                                <div className="space-y-4 ml-11">
                                    ...
                                    <div className="flex items-center justify-between py-3 border-t border-muted/50">
                                        <div>
                                            <div className="font-medium text-on-surface">Consulting Services</div>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <span className={`text-xs px-2 py-1 rounded-full ${accounts.find(acc => acc.id === '2')?.pages.find(p => p.id === 'page3')?.subscribed ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
                                                {accounts.find(acc => acc.id === '2')?.pages.find(p => p.id === 'page3')?.subscribed ? 'Subscribed' : 'Not Subscribed'}
                                            </span>
                                            <label className="relative inline-flex items-center cursor-pointer">
                                                <input
                                                    type="checkbox"
                                                    checked={accounts.find(acc => acc.id === '2')?.pages.find(p => p.id === 'page3')?.subscribed || false}
                                                    onChange={(e) => handleSubscriptionToggle('2', 'page3', e.target.checked)}
                                                    disabled={loading}
                                                    className="sr-only peer"
                                                />
                                                <div className={`w-10 h-5 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border after:rounded-full after:h-4 after:w-4 ${accounts.find(acc => acc.id === '2')?.pages.find(p => p.id === 'page3')?.subscribed ? 'bg-primary-600' : 'bg-muted'}`}></div>
                                            </label>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        ...
                                        >
                                            ...
                                            {accounts.filter(account => account.name === 'Connected Facebook Account').map(account => (
