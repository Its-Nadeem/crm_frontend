...
export const FacebookIntegrationPage: React.FC<FacebookIntegrationPageProps> = ({
    ...
    // CRM fields for mapping - only pipeline stage as requested
    const [crmFields] = useState([
        { key: 'stage', label: 'Sales Pipeline Stage', type: 'select', required: true }
    ]);
    ...
    const fetchFacebookFields = async (formId: string) => {
        ...
        try {
            ...
            if (form && form.fields && form.fields.length > 0) {
                const fields = form.fields.map(f => ({ name: f.name, values: ['Sample value'] }));
                setFacebookFields(fields);
            } else {
                // Use mock fields if no actual fields available
                const mockFields = [
