...
const IntegrationsPage: React.FC<IntegrationsPageProps> = ({ logs, settings, setSettings, customFieldDefs, onSendTestLead, syncFacebookLeads, currentOrganization, webhooks, onAddWebhook, onDeleteWebhook, onUpdateWebhook, onRefreshApiKey }) => {
    ...
    useEffect(() => {
        if (selectedSource === 'API') {
            console.log('API tab selected, fetching API key...');
    ...
    const fetchOrganizationApiKey = async (unmasked = false) => {
        try {
            ...
            console.log('API Key fetch - Token:', token ? 'Token exists' : 'No token found');
            ...
            const url = unmasked
                ? 'http://localhost:5000/api/settings/api-key?unmasked=true'
                : 'http://localhost:5000/api/settings/api-key';
    ...
    const generateApiKey = async () => {
        try {
            ...
            console.log('Generating API key with token:', token ? 'Token exists' : 'No token');

            const response = await fetch('http://localhost:5000/api/settings/api-key/generate', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
    ...
    const regenerateApiKey = async () => {
        ...
        try {
            ...
            console.log('Regenerating API key with token:', token ? 'Token exists' : 'No token');

            const response = await fetch('http://localhost:5000/api/settings/api-key/regenerate', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
    ...
    const revokeApiKey = async () => {
        ...
        try {
            ...
            console.log('Revoking API key with token:', token ? 'Token exists' : 'No token');

            const response = await fetch('http://localhost:5000/api/settings/api-key/revoke', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center gap-2">
                                        <input
                                            ...
                                            readOnly
                                            value={
                                                showFullApiKey && apiKeyData.fullApiKey
                                                    ? apiKeyData.fullApiKey
                                                    : (apiKeyVisible ? (apiKeyData.apiKey || 'No API key generated') : '••••••••••••••••••••••••••••••••')
                                            }
                                            className={`w-full bg-surface border rounded-md py-2 px-3 font-mono ${
                                                showFullApiKey ? 'text-on-surface border-green-300 bg-green-50' : 'text-subtle border-muted'
                                            }`}
                                            placeholder="No API key generated"
                                        />
                                        <button
                                            onClick={handleToggleApiKeyVisibility}
                                            className={`font-bold py-2 px-3 rounded-lg text-sm ${
                                                showFullApiKey
                                                    ? 'bg-green-100 text-green-700 hover:bg-green-200'
                                                    : 'bg-muted hover:bg-subtle/80 text-on-surface'
                                            }`}
                                            title={showFullApiKey ? "Hide Full API Key" : "Show Full API Key"}
                                        >
                                            {showFullApiKey ? <AppIcons.EyeOff className="w-4 h-4" /> : <AppIcons.Eye className="w-4 h-4" />}
                                        </button>
                                        {(apiKeyData.apiKey || (showFullApiKey && apiKeyData.fullApiKey)) && (
                                            <button
                                                onClick={() => navigator.clipboard.writeText(showFullApiKey && apiKeyData.fullApiKey ? apiKeyData.fullApiKey : apiKeyData.apiKey!)}
                                    ...
                                    {showFullApiKey && (
                                        <div className="text-xs text-green-600 bg-green-50 p-2 rounded border border-green-200">
                                            🔓 Full API key is visible for 5 seconds for security
                                        </div>
                                    )}
