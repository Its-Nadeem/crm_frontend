...
export const AccountsTab: React.FC<AccountsTabProps> = ({ accounts, onAccountsChange, onAddAccount, mappings, logs }) => {
    ...
    const [displayAccounts, setDisplayAccounts] = useState<FacebookAccount[]>(mockAccounts);
    ...
    const totalMappings = mockMappings.length; // Using static mappings for now
    const totalLeads = mockLogs.length;
    const successfulLeads = mockLogs.filter(l => l.status === LogStatus.SUCCESS).length;
