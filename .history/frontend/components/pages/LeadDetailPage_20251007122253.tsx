...
// FIX: Corrected onAddTask prop type to be more precise and avoid potential errors.
    ...
    onRefreshLeads?: () => Promise<void>; // Add refresh function
    currentUser: User;
}
...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ leads, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onScheduleMessage, onRefreshLeads, currentUser }) => {
    ...
    const handleFieldChange = async (field: keyof Lead, value: any) => {
        ...
        if (oldValue !== newValue) {
            ...
            // Save to backend - only if this is not an API response (no _id yet) and lead has valid ID
            if (!leadWithActivity._id && leadWithActivity.id) {
    ...
    const handleCustomFieldChange = async (id: string, value: any) => {
        ...
        if (oldValue !== newValue) {
            ...
            // Save to backend - only if this is not an API response (no _id yet)
            if (!leadWithActivity._id) {
                setSaveStatus('saving');
                onUpdateLead(leadWithActivity, editableLead!);
                setSaveStatus('saved');
                // Reset save status after 2 seconds
                setTimeout(() => setSaveStatus('idle'), 2000);
            }
