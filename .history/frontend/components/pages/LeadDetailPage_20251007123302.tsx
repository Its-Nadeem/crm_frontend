...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ leads, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onScheduleMessage, onRefreshLeadById, currentUser }) => {
    ...
    useEffect(() => {
        if (lead) {
            console.log('Updating editableLead with fresh data:', lead);
            setEditableLead(lead);
        }
    }, [lead]);

    // Refresh lead data after updates
    const refreshLeadData = useCallback(async () => {
        if (onRefreshLeadById && editableLead?.id) {
            try {
                console.log('Refreshing lead data for ID:', editableLead.id);
                const refreshedLead = await onRefreshLeadById(editableLead.id);
                console.log('Lead data refreshed, updating editableLead:', refreshedLead);
                if (refreshedLead) {
                    setEditableLead(refreshedLead);
                }
                console.log('editableLead updated with fresh data');
            } catch (error) {
                console.error('Failed to refresh lead data:', error);
            }
        }
    }, [onRefreshLeadById, editableLead?.id]);
    ...
    const handleFieldChange = async (field: keyof Lead, value: any) => {
        ...
        if (oldValue !== newValue) {
            ...
            if (leadWithActivity.id) {
                ...
                try {
                    await onUpdateLead(leadWithActivity, editableLead!);
                    setSaveStatus('saved');
    ...
    const handleCustomFieldChange = async (id: string, value: any) => {
        ...
        if (oldValue !== newValue) {
            ...
            if (leadWithActivity.id) {
                setSaveStatus('saving');
                try {
                    console.log('Saving lead with activity:', leadWithActivity);
                    await onUpdateLead(leadWithActivity, editableLead!);
                    ...
                    await refreshLeadData();
                    console.log('Lead data refreshed');
    ...
    const handleSendWhatsApp = async (message: string) => {
        ...
        if (updatedLead.id) {
            setSaveStatus('saving');
            try {
                await onUpdateLead(updatedLead, editableLead!);
                setSaveStatus('saved');
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
                ...
                <div className="lg:col-span-2 flex flex-col overflow-hidden bg-surface rounded-xl shadow-sm border border-muted">
                    ...
                    <div className="flex-shrink-0 p-2 grid grid-cols-2 sm:grid-cols-4 items-center gap-2 border-b border-muted">
