...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ leads, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onScheduleMessage, onRefreshLeadById, currentUser }) => {
        ...
        if (oldValue !== newValue) {
            ...
            if (leadWithActivity.id) {
                ...
                try {
                    ...
                    const { _id, __v, createdAt, updatedAt, ...cleanLeadData } = leadWithActivity as any;
                    await onUpdateLead(cleanLeadData as Lead, editableLead!);
                    setSaveStatus('saved');
        ...
        if (oldValue !== newValue) {
            ...
            if (leadWithActivity.id) {
                ...
                try {
                    ...
                    const { _id, __v, createdAt, updatedAt, ...cleanLeadData } = leadWithActivity;
                    await onUpdateLead(cleanLeadData as Lead, editableLead!);
                    setSaveStatus('saved');
    ...
    const handleSendWhatsApp = async (message: string) => {
        ...
        if (updatedLead.id) {
            ...
            try {
                // Remove system fields before sending to backend
                const { _id, __v, createdAt, updatedAt, ...cleanLeadData } = updatedLead as any;
                await onUpdateLead(cleanLeadData as Lead, editableLead!);
                setSaveStatus('saved');
