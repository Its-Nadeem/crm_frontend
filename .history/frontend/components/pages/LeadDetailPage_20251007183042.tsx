...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ leads, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onScheduleMessage, onRefreshLeadById, currentUser }) => {
    ...
    // If lead not found in current leads array, try to refresh it from backend
    useEffect(() => {
        if (!lead && leadId && onRefreshLeadById && !isLoadingLead) {
            console.log('Lead not found in array, attempting to refresh from backend:', leadId);
            ...
            onRefreshLeadById(leadId)
                .then(refreshedLead => {
                    if (refreshedLead) {
                        ...
                        // The parent component should update the leads array
                        // We can force a re-render by updating a state that triggers useEffect
    ...
    }, [lead, leadId, onRefreshLeadById, isLoadingLead]);
