...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ leads, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onScheduleMessage, onRefreshLeadById, currentUser, isStandalone = false }) => {
    ...
    // Create a debounced version of handleFieldChange to prevent excessive API calls while typing
    const debouncedHandleFieldChange = useMemo(
        () => {
            let timeoutId: NodeJS.Timeout;
            return (field: keyof Lead, value: any) => {
                clearTimeout(timeoutId);
                timeoutId = setTimeout(() => {
                    // Call the actual handleFieldChange function (defined below)
                    if (timeoutId) {
    ...
    // Update the debounced function dependency after handleFieldChange is defined
    useEffect(() => {
        // This effect runs after handleFieldChange is defined, ensuring proper dependency
    }, []);
