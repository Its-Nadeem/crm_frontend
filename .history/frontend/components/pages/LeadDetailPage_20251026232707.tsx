...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ leads, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onAddNote, onScheduleMessage, onRefreshLeadById, currentUser, isStandalone = false, isDedicatedPage = false }) => {
    ...
    const handleNoteSubmit = async () => {
        ...
        // Check for duplicate activities before adding (prevent WebSocket duplicates)
        const isDuplicate = editableLead.activities?.some(activity =>
            activity.type === 'NOTE' &&
            activity.content.includes(noteText) &&
            Math.abs(new Date(activity.timestamp).getTime() - new Date(currentTimestamp).getTime()) < 10000 // 10 second window
        );
        ...
        if (!isDuplicate) {
            ...
            const newActivity: Activity = {
                id: faker.string.uuid(),
                type: 'NOTE',
                content: `Note added: <i class="whitespace-pre-wrap">"${noteText}"</i>`,
        ...
        if (editableLead.id) {
            ...
            try {
                ...
                // Rollback optimistic update on error - remove the added activity
                if (!isDuplicate) {
                    setEditableLead(prev => ({
                        ...
                        activities: prev.activities?.filter(activity =>
                            !(activity.type === 'NOTE' &&
                              activity.content.includes(noteText) &&
                              Math.abs(new Date(activity.timestamp).getTime() - new Date(currentTimestamp).getTime()) < 10000)
                        ) || []
                    }));
                }
    ...
    const handleManualSave = async () => {
        ...
        try {
            ...
            // No need to refresh immediately after successful save
                        ...
                        <div className="lg:col-span-2">
                            <div className="bg-surface rounded-xl shadow-sm border border-muted overflow-hidden">
                                ...
                                <div className="border-b border-muted">
                                    <div className="flex">
                                        <button onClick={() => setActiveTab('activity')} className={`px-4 py-3 text-sm font-medium ${activeTab === 'activity' ? 'border-b-2 border-primary-500 text-primary-600' : 'text-subtle hover:text-on-surface'}`}>Activity</button>
