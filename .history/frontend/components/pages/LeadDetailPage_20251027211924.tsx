...
const AddTaskForm: React.FC<{ lead: Lead; onAddTask: (task: Omit<Task, 'id' | 'createdAt' | 'organizationId' | '_id'>) => void; onCancel: () => void; currentUser: User; }> = ({ lead, onAddTask, onCancel, currentUser }) => {
    ...
    const showCustomReminder = (task: any) => {
        // Create a custom reminder popup
            ...
            <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div class="bg-white rounded-lg p-6 max-w-md mx-4 shadow-xl">
                    <div class="flex items-center mb-4">
                        ...
                        <h3 class="text-lg font-semibold text-gray-900">Task Reminder</h3>
                    </div>
                    <p class="text-gray-700 mb-2"><strong>${task.title}</strong></p>
                    <p class="text-gray-600 mb-4">Task for lead "${lead.name}" is due now</p>
                    <div class="flex justify-end">
                        <button onclick="document.getElementById('task-reminder-popup').remove()" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg">
                            OK
                        </button>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="relative">
                        <input
                            ...
                            className="w-full bg-background border border-muted p-3 pr-10 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                            min={new Date().toISOString().split('T')[0]}
                            required
                        />
                        <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                            <AppIcons.Calendar className="h-5 w-5 text-subtle" />
                        </div>
                    </div>
                </div>
                <div>
                    ...
                    <div className="relative">
                        ...
                        />
                        <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                            <AppIcons.Activity className="h-5 w-5 text-subtle" />
                        </div>
                    </div>
                </div>
            </div>
