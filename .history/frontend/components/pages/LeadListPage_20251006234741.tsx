...
const LeadListPage: React.FC<LeadListPageProps> = ({
    ...
    const handleBulkDelete = async () => {
        try {
            await apiService.bulkDeleteLeads(selectedLeadIds);
            // Refresh leads data
            window.location.reload();
        } catch (error) {
    ...
    return (
        <div className="flex flex-col h-full space-y-6">
            ...
            {isImportModalOpen && <ImportLeadsModal customFieldDefs={customFieldDefs} onClose={() => setImportModalOpen(false)} onImport={onImportLeads} />}
                            ...
                            {currentLeads.map(lead => (
                                <tr key={lead.id} className="hover:bg-muted/50 transition-colors duration-150">
                                    ...
                                    <td data-label="Actions" className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <button onClick={() => setViewingLead(lead)} className="text-subtle hover:text-primary-500 p-1 rounded-full hover:bg-muted" title="Quick View">
