...
const LeadListPage: React.FC<LeadListPageProps> = ({
    ...
    const renderCellContent = (lead: Lead, columnId: string) => {
        ...
        switch (columnId) {
            case 'name':
                return (
                    <Link to={`/leads/${lead.id}`} target="_blank" rel="noopener noreferrer" className="font-semibold text-on-surface hover:text-primary-500">
                        {lead.name}
                    </Link>
                );
            case 'email':
    ...
    return (
        <div className="flex flex-col h-full space-y-6">
            ...
            {isImportModalOpen && <ImportLeadsModal customFieldDefs={customFieldDefs} onClose={() => setImportModalOpen(false)} onImport={onImportLeads} currentOrganization={{ id: 'org-1' }} existingLeads={leads} />}
                            ...
                            {currentLeads.map(lead => (
                                <tr key={lead.id} className="hover:bg-muted/50 transition-colors duration-150">
                                    ...
                                    <td data-label="Actions" className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <Link to={`/leads/${lead.id}`} target="_blank" rel="noopener noreferrer" className="text-subtle hover:text-primary-500 p-1 rounded-full hover:bg-muted" title="View Lead Details">
