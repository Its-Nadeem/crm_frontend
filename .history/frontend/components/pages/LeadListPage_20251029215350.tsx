...
const LeadListPage: React.FC<LeadListPageProps> = ({
    ...
    const handleResizeMove = useCallback((e: MouseEvent) => {
        ...
        if (newWidth > 60) { // Minimum width of 60px
            setColumnWidths(prev => {
                ...
                // Calculate total width to ensure table doesn't exceed container
                const totalWidth = Object.values(updated).reduce((sum, width) => sum + width, 0);
                // If total width exceeds reasonable bounds, scale down proportionally
                const maxTotalWidth = 2000; // Reasonable max width
                if (totalWidth > maxTotalWidth) {
                    const scaleFactor = maxTotalWidth / totalWidth;
                    return Object.fromEntries(
                        Object.entries(updated).map(([key, width]) => [key, Math.max(60, Math.round(width * scaleFactor))])
                    );
                }
                return updated;
            });
    ...
    const handleBulkDelete = () => {
        if (window.confirm(`Are you sure you want to delete ${selectedLeadIds.length} selected leads? This action cannot be undone.`)) {
            console.log('Bulk deleting leads:', selectedLeadIds);
    ...
    const handleSingleDelete = async (leadId: string) => {
        if (window.confirm('Are you sure you want to delete this lead? This action cannot be undone.')) {
            try {
                ...
                // The deleteLead function from props should handle the state update
            } catch (error) {
    ...
    const handleBulkUpdate = async (updates: any) => {
        try {
            console.log('Bulk updating leads:', selectedLeadIds, updates);
            await apiService.bulkUpdateLeads(selectedLeadIds, updates);

            // Refresh leads data from the parent component instead of full page reload
            // The parent component should handle refreshing the leads
            ...
            case 'name':
                return (
                    <button
                        ...
                        className="font-semibold text-on-surface hover:text-primary-500 text-left"
    ...
    return (
        <div className="flex flex-col h-full space-y-6">
            ...
            {isLeadDetailModalOpen && selectedLeadForModal && (
                <LeadDetailModal
                        ...
                        <thead className="bg-surface">
                            <tr>
                                ...
                                >
                                    ...
                                    >
                                        <div className="w-px h-4 bg-primary-400 transition-colors"></div>
                                    </div>
                                </th>
                            ...
                            {currentLeads.map(lead => (
                                <tr key={lead.id} className="hover:bg-muted/50 transition-colors duration-150">
                                    ...
                                    <td data-label="Actions" className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <button onClick={() => handleOpenLeadModal(lead)} className="text-subtle hover:text-primary-500 p-1 rounded-full hover:bg-muted" title="View Lead Details">
                                            <AppIcons.Eye className="h-5 w-5"/>
                                        </button>
                                        {hasPermission(Permission.DELETE_LEADS) && (
                                            <button onClick={() => handleSingleDelete(lead.id)} className="text-subtle hover:text-red-500 p-1 rounded-full hover:bg-muted ml-1" title="Delete Lead">
