...
const LeadListPage: React.FC<{
    ...
    const filteredAndSortedLeads = useMemo(() => {
        ...
        filtered.sort((a, b) => {
            ...
            if (sortBy === 'assignedToId') {
                const aUser = users.find(u => u.id === aValue);
                const bUser = users.find(u => u.id === bValue);
                aValue = aUser?.name || '';
                bValue = bUser?.name || '';
            }
        ...
        return filtered;
    }, [leads, searchTerm, filters, sortBy, sortOrder, pipelineStages, users]);
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                    <div className="flex-1">
                        <div className="relative">
                            <AppIcons.Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-subtle" />
                            <input
                                type="text"
                                placeholder="Search leads..."
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                                className="w-full pl-10 pr-4 py-2 bg-background border border-muted rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                            />
                        </div>
                    </div>
                    <div className="flex gap-2">
                        <FilterBuilder
                            fields={[
                                ...
                                { key: 'stage', label: 'Stage', type: 'select', options: pipelineStages.map(s => ({ value: s.id, label: s.name })) },
                                { key: 'assignedToId', label: 'Assigned To', type: 'select', options: users.map(u => ({ value: u.id, label: u.name })) },
                                { key: 'source', label: 'Source', type: 'select', options: Object.entries(LEAD_SOURCES).map(([value, label]) => ({ value, label })) },
                            ...
                            <tr>
                                ...
                                >
                                    <div className="flex items-center gap-1">
                                        Name
                                        {sortBy === 'name' && (
                                            <AppIcons.Sort className={`h-3 w-3 ${sortOrder === 'desc' ? 'rotate-180' : ''}`} />
                                        )}
                                    </div>
                                </th>
                                <th
                                ...
                                <tr>
                                    <td colSpan={8} className="px-6 py-12 text-center text-subtle">
                                        {leads.length === 0 ? (
                                            ...
                                            <div>
                                                <AppIcons.Search className="mx-auto h-12 w-12 text-muted mb-4" />
