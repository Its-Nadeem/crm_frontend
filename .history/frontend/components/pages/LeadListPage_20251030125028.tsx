...
const LeadListPage: React.FC<{
    ...
    const basicFilteredLeads = useMemo(() => {
        ...
        return filtered;
    }, [leads, searchTerm, basicFilters.campaign, basicFilters.stage, basicFilters.assignedToId]);
    ...
    const handleClearFilters = () => {
        ...
        setBasicFilters({ campaign: '', stage: '', assignedToId: 0 });
        setCurrentPage(1);
    };
            ...
            <div className="bg-surface rounded-lg p-4 space-y-4">
                <div className="flex flex-col lg:flex-row gap-4">
                    <div className="flex flex-wrap gap-2">
                        ...
                        >
                            ...
                            {(basicFilters.campaign || basicFilters.stage || basicFilters.assignedToId > 0) && (
                                <span className="bg-red-500 text-white text-xs px-2 py-1 rounded-full">
                                    {(basicFilters.campaign ? 1 : 0) + (basicFilters.stage ? 1 : 0) + (basicFilters.assignedToId > 0 ? 1 : 0)}
                                </span>
                            )}
                        </button>
                        <button
                        ...
                        {(filters.length > 0 || basicFilters.campaign || basicFilters.stage || basicFilters.assignedToId > 0) && (
                            <button
            ...
            {showBulkEditModal && (
                <Modal isOpen={showBulkEditModal} onClose={() => setShowBulkEditModal(false)} title={`Bulk Edit ${selectedLeads.size} Leads`}>
                    <div className="space-y-4">
                        ...
                        <BulkEditForm
                            selectedLeads={Array.from(selectedLeads).map(id => leads.find(l => l.id === id)).filter(Boolean) as Lead[]}
                            pipelineStages={pipelineStages}
                            users={users}
                            onUpdateLeads={async (updates) => {
                                try {
