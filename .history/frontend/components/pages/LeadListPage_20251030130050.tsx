    ...
    const filteredAndSortedLeads = useMemo(() => {
        ...
        filtered.sort((a, b) => {
            ...
            if (sortBy === 'stage') {
                ...
                // Source is already a string, no special handling needed
                    ...
                    >
                        ...
                        <thead className="bg-muted/50">
                            <tr>
                                {columnOrder.map(columnId => {
                                    if (columnId === 'checkbox' && visibleColumns.name) {
                                        return (
                                            <th key={columnId} className="px-6 py-3 text-left" style={{ width: columnWidths[columnId] || 50 }}>
                                                <input
                                                    type="checkbox"
                                    ...
                                    if (columnId === 'name' && visibleColumns.name) {
                                        return (
                                            <th
                                                key={columnId}
                                                className="px-6 py-3 text-left text-xs font-medium text-subtle uppercase tracking-wider cursor-pointer hover:bg-muted/70 relative group"
                                                style={{ width: columnWidths[columnId] || 200 }}
                                                draggable
                                            ...
                                            >
                                                <div className="flex items-center gap-1">
                                                    <span className="cursor-move opacity-50 group-hover:opacity-100">⋮⋮</span>
                                                    Name
                                                    {sortBy === 'name' && (
                                                ...
                                                <div
                                                    className="absolute right-0 top-0 h-full w-1 cursor-col-resize bg-gray-300 hover:bg-gray-400 opacity-0 group-hover:opacity-100"
                                                    onMouseDown={(e) => handleMouseDown(e, columnId)}
                                                />
                                            </th>
                                        );
                                    }
                                    if (columnId === 'source' && visibleColumns.source) {
                                        return (
                                            <th
                                                key={columnId}
                                                className="px-6 py-3 text-left text-xs font-medium text-subtle uppercase tracking-wider cursor-pointer hover:bg-muted/70 relative group"
                                                style={{ width: columnWidths[columnId] || 120 }}
                                                draggable
                                                onDragStart={(e) => handleDragStart(e, columnId)}
                                                onDragOver={(e) => handleDragOver(e, columnId)}
                                                onDrop={(e) => handleDrop(e, columnId)}
                                    ...
                                    if (columnId === 'email' && visibleColumns.email) {
                                        return (
                                            <th
                                                key={columnId}
                                                className="px-6 py-3 text-left text-xs font-medium text-subtle uppercase tracking-wider cursor-pointer hover:bg-muted/70 relative group"
                                                style={{ width: columnWidths[columnId] || 180 }}
                                                draggable
                                                onDragStart={(e) => handleDragStart(e, columnId)}
                                                onDragOver={(e) => handleDragOver(e, columnId)}
                                                onDrop={(e) => handleDrop(e, columnId)}
                                    ...
                                    if (columnId === 'phone' && visibleColumns.phone) {
                                        return (
                                            <th
                                                key={columnId}
                                                className="px-6 py-3 text-left text-xs font-medium text-subtle uppercase tracking-wider cursor-pointer hover:bg-muted/70 relative group"
                                                style={{ width: columnWidths[columnId] || 140 }}
                                                draggable
                                                onDragStart={(e) => handleDragStart(e, columnId)}
                                                onDragOver={(e) => handleDragOver(e, columnId)}
                                                onDrop={(e) => handleDrop(e, columnId)}
                                    ...
                                    if (columnId === 'stage' && visibleColumns.stage) {
                                        return (
                                            <th
                                                key={columnId}
                                                className="px-6 py-3 text-left text-xs font-medium text-subtle uppercase tracking-wider cursor-pointer hover:bg-muted/70 relative group"
                                    ...
                                    if (columnId === 'assignedToId' && visibleColumns.assignedToId) {
                                        return (
                                            <th
                                                key={columnId}
                                                className="px-6 py-3 text-left text-xs font-medium text-subtle uppercase tracking-wider relative group"
                                                style={{ width: columnWidths[columnId] || 140 }}
                                                draggable
                                                onDragStart={(e) => handleDragStart(e, columnId)}
                                                onDragOver={(e) => handleDragOver(e, columnId)}
                                    ...
                                    if (columnId === 'updatedAt' && visibleColumns.updatedAt) {
                                        return (
                                            <th
                                                key={columnId}
                                                className="px-6 py-3 text-left text-xs font-medium text-subtle uppercase tracking-wider cursor-pointer hover:bg-muted/70 relative group"
                                    ...
                                    if (columnId === 'actions') {
                                        return (
                                            <th
                                                key={columnId}
                                                className="px-6 py-3 text-right text-xs font-medium text-subtle uppercase tracking-wider relative group"
                                                style={{ width: columnWidths[columnId] || 100 }}
                                                draggable
                                                onDragStart={(e) => handleDragStart(e, columnId)}
                                                onDragOver={(e) => handleDragOver(e, columnId)}
                                                onDrop={(e) => handleDrop(e, columnId)}
                                ...
                                {visibleColumns.facebookCampaign && (
                                    <th className="px-6 py-3 text-left text-xs font-medium text-subtle uppercase tracking-wider">
                                        Facebook Campaign
                                    </th>
                                )}
                                {visibleColumns.facebookAdset && (
                                    <th className="px-6 py-3 text-left text-xs font-medium text-subtle uppercase tracking-wider">
                                        Facebook Adset
                                    </th>
                                )}
                                {visibleColumns.facebookAd && (
                                    <th className="px-6 py-3 text-left text-xs font-medium text-subtle uppercase tracking-wider">
                                        Facebook Ad
                                    </th>
                                )}
                                ...
                                {customFieldDefs.map(field => (
                                    visibleColumns[`custom_${field.id}`] && (
                                        <th key={field.id} className="px-6 py-3 text-left text-xs font-medium text-subtle uppercase tracking-wider">
                                            {field.name}
                                        </th>
                                    )
                                ))}
                        ...
                        <tbody className="bg-surface divide-y divide-muted">
                            {paginatedLeads.length === 0 ? (
                                <tr>
                                    <td colSpan={Object.values(visibleColumns).filter(Boolean).length + customFieldDefs.filter(f => visibleColumns[`custom_${f.id}`]).length + 1} className="px-6 py-12 text-center text-subtle">
                                        {leads.length === 0 ? (
                                            <div>
                                ...
                                paginatedLeads.map((lead) => (
                                    <tr key={lead.id} className="hover:bg-muted/30">
                                        {columnOrder.map(columnId => {
                                            if (columnId === 'checkbox' && visibleColumns.name) {
                                                return (
                                                    <td key={columnId} className="px-6 py-4 whitespace-nowrap" style={{ width: columnWidths[columnId] || 50 }} onClick={(e) => e.stopPropagation()}>
                                                        <input
                                                            type="checkbox"
                                            ...
                                            if (columnId === 'name' && visibleColumns.name) {
                                                return (
                                                    <td key={columnId} className="px-6 py-4 whitespace-nowrap cursor-pointer" style={{ width: columnWidths[columnId] || 200 }} onClick={() => {
                                                        ...
                                                        <div className="flex items-center">
                                                            ...
                                                            <div className="ml-4">
                                                                <div className="text-sm font-medium text-on-surface hover:text-primary-600 hover:scale-105 transition-all duration-200 transform">
                                            ...
                                            if (columnId === 'source' && visibleColumns.source) {
                                                return (
                                                    <td key={columnId} className="px-6 py-4 whitespace-nowrap text-sm text-on-surface" style={{ width: columnWidths[columnId] || 120 }}>
                                            ...
                                            if (columnId === 'email' && visibleColumns.email) {
                                                return (
                                                    <td key={columnId} className="px-6 py-4 whitespace-nowrap text-sm text-on-surface" style={{ width: columnWidths[columnId] || 180 }}>
                                                        {lead.email || '-'}
                                                    </td>
                                                );
                                            }
                                            if (columnId === 'phone' && visibleColumns.phone) {
                                                return (
                                                    <td key={columnId} className="px-6 py-4 whitespace-nowrap text-sm text-on-surface" style={{ width: columnWidths[columnId] || 140 }}>
                                                        {lead.phone || '-'}
                                                    </td>
                                                );
                                            }
                                            if (columnId === 'stage' && visibleColumns.stage) {
                                                return (
                                                    <td key={columnId} className="px-6 py-4 whitespace-nowrap" style={{ width: columnWidths[columnId] || 120 }}>
                                                        <span
                                            ...
                                            if (columnId === 'updatedAt' && visibleColumns.updatedAt) {
                                                return (
                                                    <td key={columnId} className="px-6 py-4 whitespace-nowrap text-sm text-subtle" style={{ width: columnWidths[columnId] || 120 }}>
                                            ...
                                            if (columnId === 'actions') {
                                                return (
                                                    <td key={columnId} className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium" style={{ width: columnWidths[columnId] || 100 }} onClick={(e) => e.stopPropagation()}>
                            ...
                            {paginatedLeads.length === 0 ? (
                                ...
                                paginatedLeads.map((lead) => (
                                    <tr key={lead.id} className="hover:bg-muted/30">
                                        ...
                                        {visibleColumns.closeDate && (
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-on-surface">
                                                {lead.closeDate ? new Date(lead.closeDate).toLocaleDateString() : '-'}
                                            </td>
                                        )}
                                        {visibleColumns.campaign && (
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-on-surface">
                                        ...
                                        {customFieldDefs.map(field => (
                                            visibleColumns[`custom_${field.id}`] && (
                                                <td key={field.id} className="px-6 py-4 whitespace-nowrap text-sm text-on-surface">
                                                    {lead.customFields?.[field.id] || '-'}
                                                </td>
                                            )
                                        ))}
