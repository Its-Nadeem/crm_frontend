    ...
    const handleExportLeads = async (exportType: 'filtered' | 'all' | 'selected' | 'currentPage') => {
        try {
            ...
            const exportData = dataToExport.map(lead => ({
                ...
                'Alternate Phone': lead.alternatePhone || '', City: lead.city || '',
                Course: lead.course || '', Company: lead.company || '',
                Source: typeof lead.source === 'string' ? lead.source : '',
                Stage: getStageName(lead.stage), 'Follow Up Status': lead.followUpStatus || '',
                Score: lead.score || 0, Tags: lead.tags?.join(', ') || '',
                'Assigned To': getUserName(lead.assignedToId), 'Deal Value': lead.dealValue || 0,
                'Close Date': lead.closeDate ? new Date(lead.closeDate).toLocaleDateString() : '',
                Campaign: lead.campaign || '', 'Facebook Campaign': lead.facebookCampaign || '',
                'Facebook Adset': lead.facebookAdset || '', 'Facebook Ad': lead.facebookAd || '',
