...
const AccountBillingSettings: React.FC<{ organization: Organization, plans: SubscriptionPlan[], history: BillingHistory[] }> = ({ organization, plans, history }) => {
                    ...
                    {plans.filter(p => p.price > 0).map(plan => (
                        <div key={plan.id} className={`p-6 rounded-lg border-2 ${currentPlan?.id === plan.id ? 'border-primary-500 bg-primary-500/5' : 'border-muted'}`}>
                            ...
                            <ul className="text-xs space-y-1 mt-4">
                             ...
                             <button disabled={currentPlan?.id === plan.id} className="w-full mt-6 py-2 rounded-lg font-semibold text-sm disabled:bg-primary-600 disabled:text-white disabled:cursor-not-allowed bg-muted hover:bg-primary-500/20 text-on-surface hover:text-primary-500">
                                {currentPlan?.id === plan.id ? 'Current Plan' : (currentPlan && plan.price > currentPlan.price ? 'Upgrade' : 'Downgrade')}
                            </button>
                        </div>
                    ))}
                </div>
    ...
    <div className="space-y-3">
        {stages.map(stage => (
            <div key={stage.id} className="flex items-center gap-2 p-2 bg-background rounded-lg">
                <input type="color" value={stage.color} onChange={e => onUpdate(stage.id, 'color', e.target.value)} className="w-8 h-8 p-0 border-none rounded cursor-pointer bg-transparent" style={{backgroundColor: stage.color}} />
                <input type="text" value={stage.name} onChange={e => onUpdate(stage.id, 'name', e.target.value)} className="flex-grow bg-transparent focus:bg-muted p-1 rounded-md" />
                <button onClick={() => onDelete(stage.id)} className="text-red-400 hover:text-red-300"><AppIcons.Delete className="w-4 h-4"/></button>
            </div>
        ))}
        <button onClick={onAdd} className="text-sm text-primary-500 font-semibold hover:text-primary-600 mt-2">+ Add Stage</button>
    </div>
);
...
const SettingsPage: React.FC<SettingsPageProps> = (props) => {
    const { currentUser, hasPermission, currentOrganization, subscriptionPlans, billingHistory, onUpdateProfile } = props;
        ...
        <div className="space-y-6">
            ...
            <div className="border-b border-muted">
             ...
             <div className="pt-6">
                ...
                {activeTab === 'account' && <AccountBillingSettings organization={currentOrganization} plans={subscriptionPlans} history={billingHistory} />}
