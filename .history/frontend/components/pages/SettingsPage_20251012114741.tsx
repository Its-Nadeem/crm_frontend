...
const LeadScoringSettings: React.FC<{
    ...
    const handleDeleteRule = (rule: LeadScoreRule) => {
        console.log('Delete rule clicked:', rule);
        console.log('Rule ID:', rule.id);
        console.log('Rule has ID:', !!rule.id);
    ...
    const confirmDeleteRule = async () => {
        ...
        console.log('ruleToDelete ID:', ruleToDelete?.id);

        if (!ruleToDelete || !ruleToDelete.id) {
            console.error('No rule selected for deletion or rule ID is missing');
            console.error('ruleToDelete:', ruleToDelete);
            return;
        }

        setIsLoading('delete');
        try {
            console.log('Calling onDelete with ID:', ruleToDelete.id);
                    ...
                    {rules.map((rule, index) => (
                        ...
                        >
                            ...
                            <div className="text-xs text-subtle mb-2">
                                Debug: Rule ID: {rule.id} | Has ID: {rule.id ? 'YES' : 'NO'}
                            </div>
                ...
                {activeTab === 'appearance' && (
                 ...
                 {activeTab === 'scoring' && (
                     <SettingCard title="Lead Scoring" description="Automate lead prioritization by setting up scoring rules." permission={Permission.MANAGE_SETTINGS} hasPermission={hasPermission}>
                       <LeadScoringSettings rules={props.leadScoreRules} onSave={props.onSaveLeadScoreRule} onDelete={props.onDeleteLeadScoreRule} customFieldDefs={props.customFieldDefs} onRefreshRules={onRefreshLeadScoreRules} />
                    </SettingCard>
                 )}
            </div>
        </div>
