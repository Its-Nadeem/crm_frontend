...
}> = ({ customDomains, organizations, onDelete, onVerify }) => {
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
                {customDomains.map(domain => {
                    const org = organizations.find(o => o.id === domain.organizationId);
                    return (
                        <div key={domain.id} className="bg-surface rounded-xl shadow-lg border border-muted/50 p-5">
                            <div className="flex flex-wrap justify-between items-center gap-4">
                                <div>
                                    <h3 className="font-semibold text-lg text-on-surface font-mono">{domain.domain}</h3>
                                    <p className="text-sm text-subtle">Organization: {org?.name}</p>
                                </div>
                                 <div className="flex items-center gap-4">
                                    <span className={`px-2.5 py-1 text-xs font-semibold rounded-full ${domain.status === 'verified' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-300'}`}>{domain.status.toUpperCase()}</span>
                                    {domain.status === 'pending' && <button onClick={() => handleVerify(domain.id)} disabled={!!verifyingId} className="text-sm bg-blue-500/20 text-blue-400 font-semibold py-1 px-2 rounded-md hover:bg-blue-500/30 disabled:opacity-50 disabled:cursor-wait">{verifyingId === domain.id ? 'Verifying...' : 'Verify'}</button>}
                                    <button onClick={() => onDelete(domain.id)} className="p-2 text-subtle hover:text-red-500"><AppIcons.Delete className="h-5 w-5"/></button>
                                </div>
                            </div>
                            {domain.status === 'pending' && (
                                 <details className="mt-4">
                                     <summary className="text-sm font-semibold text-primary-500 cursor-pointer">Show DNS Configuration</summary>
                                    <div className="mt-2 bg-background p-4 rounded-lg">
                                        <p className="text-sm text-subtle mt-1">Create a CNAME record in your domain's DNS settings.</p>
                                        <div className="mt-2 text-sm font-mono bg-muted/50 p-2 rounded-md space-y-1">
                                            <p><strong>Type:</strong> CNAME</p>
                                            <p><strong>Host:</strong> {domain.domain.split('.')[0]}</p>
                                            <p><strong>Value:</strong> crm.Clienn CRM.app</p>
                                        </div>
                                    </div>
                                </details>
                            )}
...
const MonitoringTab: React.FC<{
    ...
    const serviceStatus = { API: 'Operational', Database: 'Operational', 'Facebook Sync': 'Degraded' };
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-6">
                    <div className="bg-surface p-4 rounded-xl shadow-lg border border-muted/50"><h3 className="text-sm font-semibold text-subtle text-center">CPU Load (%)</h3><ResponsiveContainer width="100%" height={100}><AreaChart data={healthData} margin={{top:10, right:0, left:0, bottom:0}}><Tooltip/><Area type="monotone" dataKey="cpuLoad" stroke="#8884d8" fill="#8884d8" /></AreaChart></ResponsiveContainer></div>
                    <div className="bg-surface p-4 rounded-xl shadow-lg border border-muted/50"><h3 className="text-sm font-semibold text-subtle text-center">Memory (%)</h3><ResponsiveContainer width="100%" height={100}><AreaChart data={healthData} margin={{top:10, right:0, left:0, bottom:0}}><Tooltip/><Area type="monotone" dataKey="memoryUsage" stroke="#82ca9d" fill="#82ca9d" /></AreaChart></ResponsiveContainer></div>
                    <div className="bg-surface p-4 rounded-xl shadow-lg border border-muted/50"><h3 className="text-sm font-semibold text-subtle text-center">Response (ms)</h3><ResponsiveContainer width="100%" height={100}><AreaChart data={healthData} margin={{top:10, right:0, left:0, bottom:0}}><Tooltip/><Area type="monotone" dataKey="responseTime" stroke="#ffc658" fill="#ffc658" /></AreaChart></ResponsiveContainer></div>
                </div>
                 <div className="bg-surface p-6 rounded-xl shadow-lg border border-muted/50">
            ...
            <div className="bg-surface p-6 rounded-xl shadow-lg border border-muted/50">
                ...
                <div className="max-h-80 overflow-y-auto mt-4 font-mono text-xs">
                    {(logTab === 'api' ? apiLogs : errorLogs).slice(0, 50).map(log => (
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                 ...
                 <div className="bg-surface p-6 rounded-xl shadow-lg border border-muted/50">
                    <h3 className="font-semibold text-lg mb-4">New Organizations (Last 6 Months)</h3>
                     {/* Another chart can go here */}
                </div>
            </div>
        </div>
...
interface SuperAdminDashboardProps {
    ...
    onDeleteCustomDomain: (id: string) => void;
    onVerifyCustomDomain: (id: string) => void;
...
const SuperAdminDashboard: React.FC<SuperAdminDashboardProps> = (props) => {
    const { 
        ...
        onSaveGlobalRule, onDeleteGlobalRule, homepageContent, customDomains,
        ...
        onUpdateHomepageContent, onDeleteCustomDomain, onVerifyCustomDomain, paymentGatewaySettings, onUpdatePaymentGatewaySettings,
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'site_management': return <SiteManagementTab customDomains={customDomains} organizations={organizations} onDelete={onDeleteCustomDomain} onVerify={onVerifyCustomDomain} />;
            case 'monitoring': return <MonitoringTab healthData={systemHealthData} apiLogs={apiUsageLogs} errorLogs={errorLogs} />;
            ...
            case 'analytics': return <AnalyticsTab organizations={organizations} plans={subscriptionPlans} />;
