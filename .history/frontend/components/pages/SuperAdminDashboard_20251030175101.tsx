...
const PaymentGatewayTab: React.FC<{
    ...
    const defaultSettings: PaymentGatewaySetting[] = [
        { provider: 'stripe', name: 'Stripe', apiKey: '', apiSecret: '', isEnabled: false },
        { provider: 'razorpay', name: 'Razorpay', apiKey: '', apiSecret: '', isEnabled: false },
        { provider: 'paypal', name: 'PayPal', apiKey: '', apiSecret: '', isEnabled: false },
        { provider: 'payu', name: 'PayU', apiKey: '', apiSecret: '', isEnabled: false },
        { provider: 'paytm', name: 'Paytm', apiKey: '', apiSecret: '', isEnabled: false },
        { provider: 'square', name: 'Square', apiKey: '', apiSecret: '', isEnabled: false },
        { provider: 'braintree', name: 'Braintree', apiKey: '', apiSecret: '', isEnabled: false },
        { provider: 'adyen', name: 'Adyen', apiKey: '', apiSecret: '', isEnabled: false },
        { provider: 'authorize.net', name: 'Authorize.net', apiKey: '', apiSecret: '', isEnabled: false }
    ];
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {localSettings.map(setting => (
                    <div key={setting.provider} className={`bg-surface p-6 rounded-xl shadow-lg border ${setting.isEnabled ? 'border-primary-500/50' : 'border-muted/50'}`}>
                        ...
                        <button onClick={() => setEditingProvider(setting.provider)} className="w-full bg-muted hover:bg-subtle/20 text-on-surface font-semibold py-2 px-4 rounded-lg text-sm">Configure</button>
                    </div>
                ))}
            </div>
        </div>
