...
const PaymentGatewayTab: React.FC<{
    ...
    const handleTestPayment = async (provider: PaymentGatewayProvider) => {
        try {
            ...
            const testAmount = 1.00; // $1.00 test payment
            const testData = {
                provider,
                amount: testAmount,
                currency: 'USD',
            ...
            const response: any = await apiService.requestWithRetry('/api/super-admin/test-payment', {
                method: 'POST',
            ...
            if (response.success) {
                ...
                alert(`❌ Test payment failed: ${response.error || 'Unknown error'}`);
            }
        } catch (error) {
            console.error('Test payment error:', error);
            alert('❌ Test payment failed. Please check your configuration.');
        }
    };
