...
}> = ({ settings, onSave, onUpdatePaymentGatewaySettings }) => {
    ...
    const handleToggleGateway = async (provider: PaymentGatewayProvider, checked: boolean) => {
        ...
        try {
            ...
            if (gatewayToUpdate) {
                ...
                onUpdatePaymentGatewaySettings(localSettings.map(s => s.provider === provider ? updatedGateway : s));
            }
        } catch (error) {
    ...
    const handleTestPayment = async (provider: PaymentGatewayProvider) => {
        try {
            ...
            if (response.success) {
                ...
                const isConfigError = response.error && (
                    response.error.includes('not configured') ||
                    response.error.includes('not set') ||
                    response.error.includes('missing')
                );
                ...
                setTestPaymentResult({
                    ...
                    message: isConfigError
                        ? `Configuration Required: ${response.error}. Please configure the ${provider} payment gateway credentials first.`
                        : response.error || 'Test payment failed. Please check your configuration.',
                    provider
                });
            }
        } catch (error) {
    ...
    return (
        <div className="space-y-8">
            ...
            {saveStatus && saveStatus.show && (
                <div className={`p-4 rounded-lg border ${saveStatus.success ? 'bg-green-50 border-green-200 text-green-800' : 'bg-red-50 border-red-200 text-red-800'}`}>
                    <div className="flex items-center gap-3">
                        {saveStatus.success ? (
                            <AppIcons.CheckCircle className="h-5 w-5 text-green-600" />
                        ) : (
                            <AppIcons.XCircle className="h-5 w-5 text-red-600" />
                        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-4 bg-surface rounded-lg border border-muted">
                <FormField label="Filter by Action Type">
                    <select value={actionFilter} onChange={e => setActionFilter(e.target.value)} className="w-full bg-background border border-muted p-2 rounded">
                        ...
                        {/* Also include any dynamic action types that aren't in our predefined list */}
                        {actionTypes
                            .filter(action => !actionTypeCategories.some(cat => cat.value === action))
                            .map(action => (
...
const MonitoringTab: React.FC<{
    ...
    const serviceStatus = (monitoringData?.serviceStatus as Record<string, string>) || { API: 'Unknown', Database: 'Unknown', 'Facebook Sync': 'Unknown' };
        ...
        <div className="space-y-8">
            ...
            {monitoringData && (
                <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">
                    Successfully loaded real system monitoring data from database!
                </div>
            )}
            {!monitoringData && !error && (
                <div className="bg-blue-100 border border-blue-400 text-blue-700 px-4 py-3 rounded mb-4">
                    Loading real system monitoring data...
                </div>
            )}
...
const SuperAdminDashboard: React.FC<SuperAdminDashboardProps> = (props) => {
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'payment_gateway': return <PaymentGatewayTab settings={paymentGatewaySettings} onSave={onUpdatePaymentGatewaySettings} onUpdatePaymentGatewaySettings={onUpdatePaymentGatewaySettings} />;
            case 'blog': return (
                <div className="space-y-6">
                    <div className="flex justify-between items-center">
