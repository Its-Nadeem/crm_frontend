...
const AddTaskModal: React.FC<{
    ...
    users: User[];
    teams: Team[];
}> = ({ onClose, onAddTask, leads, users, teams }) => {
        ...
        <Modal isOpen={true} onClose={onClose} title="Add New Task">
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    ...
                    <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required className="mt-1 block w-full bg-background border border-muted rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500" />
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} required className="mt-1 block w-full bg-background border border-muted rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500" />
                </div>
                <div className="flex justify-end gap-3 pt-4 border-t border-muted">
                    ...
                    <button type="submit" className="bg-primary-600 hover:bg-primary-700 text-white font-bold py-2 px-4 rounded-lg">Add Task</button>
                </div>
            </form>
        </Modal>
    );
...
const TasksPage: React.FC<TasksPageProps> = ({ tasks, users, leads, teams, currentUser, updateTask, onAddTask, onDeleteTask }) => {
    ...
    return (
        <div className="flex flex-col h-full space-y-6">
            {isAddModalOpen && <AddTaskModal onClose={() => setAddModalOpen(false)} onAddTask={onAddTask} leads={leads} users={users} teams={teams} />}
                            ...
                            {filteredTasks.map(task => {
                                ...
                                return (
                                    <tr key={task.id} className={`group ${task.isCompleted ? 'bg-surface/50 opacity-70' : 'bg-surface'} hover:bg-muted/30 transition-colors duration-150`}>
                                        <td className="px-6 py-4"><input type="checkbox" checked={task.isCompleted} onChange={() => updateTask({ ...task, isCompleted: !task.isCompleted })} className="h-5 w-5 rounded-md text-primary-600 focus:ring-primary-500 border-muted bg-background"/></td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium"><span className={task.isCompleted ? 'line-through text-subtle' : 'text-on-surface'}>{task.title}</span></td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm">{lead ? <Link to={`/leads/${lead.id}`} className="flex items-center gap-2 text-subtle hover:text-primary-500 hover:underline transition-colors"><AppIcons.Link className="h-4 w-4"/>{lead.name}</Link> : <span className="text-subtle">-</span>}</td>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium"><button onClick={() => window.confirm('Are you sure you want to delete this task?') && onDeleteTask(task.id)} className="text-subtle opacity-0 group-hover:opacity-100 hover:text-red-500 p-1 rounded-full hover:bg-muted"><AppIcons.Delete className="h-5 w-5"/></button></td>
                                    </tr>
                                );
                            })}
                             {filteredTasks.length === 0 && (
                                <tr><td colSpan={5} className="text-center p-16 text-subtle">
