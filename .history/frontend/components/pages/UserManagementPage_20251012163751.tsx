...
const UserManagementPage: React.FC<UserManagementPageProps> = ({ users, teams, onSaveUser, onDeleteUser }) => {
    ...
    // Use optimistic users if available, otherwise fall back to props
    const displayUsers = optimisticUsers.length > 0 ? optimisticUsers : users;

    // Debug: Log when component re-renders and receives new props
    console.log('🔄 UserManagementPage: Component re-rendered');
    console.log('📊 UserManagementPage: Props users count:', users.length);
    console.log('🚀 UserManagementPage: Optimistic users count:', optimisticUsers.length);
    console.log('👀 UserManagementPage: Display users count:', displayUsers.length);

    // Reset optimistic updates when props change
    useEffect(() => {
        if (optimisticUsers.length === 0 && users.length > 0) {
            console.log('🔄 UserManagementPage: Props updated, clearing optimistic state');
            setOptimisticUsers([]);
        }
    }, [users.length, optimisticUsers.length]);
    ...
    const handleDeleteUser = async (userId: number) => {
        // Find the user being deleted for potential rollback
        const userToDelete = users.find(u => u.id === userId);
        if (!userToDelete) {
            console.error('❌ UserManagementPage: User not found for deletion:', userId);
            return;
        }

        console.log('🗑️ UserManagementPage: STARTING DELETE PROCESS');
        console.log('📊 UserManagementPage: Current props users count:', users.length);
        console.log('🚀 UserManagementPage: Current optimistic users count:', optimisticUsers.length);
        console.log('👀 UserManagementPage: Current display users count:', displayUsers.length);
        console.log('🆔 UserManagementPage: Deleting user ID:', userId);

        setIsDeleting(true);

        // Optimistic update: Remove user from display immediately
        console.log('⚡ UserManagementPage: Applying optimistic update - removing user from state');
        setOptimisticUsers(prev => {
            const filtered = prev.filter(u => u.id !== userId);
            console.log('📊 UserManagementPage: Optimistic users after filter:', filtered.length);
            return filtered;
        });

        // Force a re-render check
        console.log('🔄 UserManagementPage: About to make API call');

        try {
            await onDeleteUser(userId);

            console.log('✅ UserManagementPage: User deletion completed successfully');
            addToast({
                type: 'success',
                title: 'Success',
            ...
            console.error('❌ UserManagementPage: Failed to delete user:', error);

            // Rollback optimistic update on error
            console.log('🔄 UserManagementPage: Rolling back optimistic update');
