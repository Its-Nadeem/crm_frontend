...
const UserManagementPage: React.FC<UserManagementPageProps> = ({ users, teams, onSaveUser, onDeleteUser }) => {
    ...
    // Filter out the user being deleted for immediate UI feedback
    const displayUsers = useMemo(() => {
        if (deletingUserId !== null) {
            const filtered = users.filter(u => u.id !== deletingUserId);
            console.log('🔄 UserManagementPage: Filtering out deleting user:', deletingUserId, 'from', users.length, 'to', filtered.length);
            return filtered;
        }
        return users;
    ...
    console.log('🗑️ UserManagementPage: Deleting user ID:', deletingUserId);
    console.log('👀 UserManagementPage: Display users count:', displayUsers.length);

    // Clear deleting user ID when users prop changes (parent state updated)
    useEffect(() => {
        if (deletingUserId !== null) {
            const userStillExists = users.some(u => u.id === deletingUserId);
            if (!userStillExists) {
                console.log('✅ UserManagementPage: User successfully deleted from parent state, clearing deleting ID');
    ...
    const handleDeleteUser = async (userId: number) => {
        ...
        // Optimistic update: Set the deleting user ID to filter it out immediately
        console.log('⚡ UserManagementPage: Setting deleting user ID for immediate UI update');
        ...
        try {
            ...
            // Clear the deleting user ID since deletion was successful
            setDeletingUserId(null);
            addToast({
                type: 'success',
                title: 'Success',
            ...
            console.log('🔄 UserManagementPage: Rolling back optimistic update - clearing deleting user ID');
            setDeletingUserId(null);
