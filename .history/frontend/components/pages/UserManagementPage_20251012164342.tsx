...
const UserManagementPage: React.FC<UserManagementPageProps> = ({ users, teams, onSaveUser, onDeleteUser }) => {
    ...
    const handleDeleteUser = async (userId: number) => {
        ...
        console.log('📊 Current localUsers count BEFORE delete:', localUsers.length);
        console.log('📋 Current localUsers:', localUsers.map(u => ({ id: u.id, name: u.name })));

        setIsDeleting(true);

        // Optimistic update: Remove user from local state immediately
        const originalUsers = [...localUsers]; // Create a copy
        ...
        setTimeout(() => {
            console.log('🔄 FORCE CHECK: Current localUsers count after timeout:', localUsers.length);
        }, 0);
