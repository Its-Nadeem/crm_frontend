...
const UserManagementPage: React.FC<UserManagementPageProps> = ({ users, teams, onSaveUser, onDeleteUser }) => {
    ...
    // Keep display users in sync with props
    useEffect(() => {
        console.log('🔄 UserManagementPage: Props updated, syncing display users');
    ...
    const handleDeleteUser = async (userId: number) => {
        console.log('🔥 DELETE BUTTON CLICKED - UserManagementPage');
        console.log('👤 Deleting user ID:', userId);
        console.log('📊 Current displayUsers count BEFORE delete:', displayUsers.length);
        console.log('📋 Current displayUsers:', displayUsers.map(u => ({ id: u.id, name: u.name })));

        setIsDeleting(true);

        // Optimistic update: Remove user from display state immediately
        const originalUsers = [...displayUsers]; // Create a copy
        console.log('💾 Saved original users count:', originalUsers.length);

        setDisplayUsers(prev => {
            const filtered = prev.filter(u => u.id !== userId);
            console.log('⚡ OPTIMISTIC UPDATE: Filtered users count:', filtered.length);
            console.log('✅ User should disappear NOW from UI');
            return filtered;
        });

        // Force immediate re-render check
        setTimeout(() => {
            console.log('🔄 FORCE CHECK: Current displayUsers count after timeout:', displayUsers.length);
        }, 0);
        ...
        try {
            ...
            console.log('🔄 ROLLING BACK: Restoring original users');
