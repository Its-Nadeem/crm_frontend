...
const UserManagementPage: React.FC<UserManagementPageProps> = ({ users, teams, onSaveUser, onDeleteUser }) => {
    ...
    useEffect(() => {
        console.log('🔄 UserManagementPage: PROPS CHANGED!');
        console.log('📊 New props users count:', users.length);
        console.log('👀 Current display users count:', displayUsers.length);
        console.log('📋 Props users:', users.map(u => ({ id: u.id, name: u.name })));
        console.log('📋 Display users:', displayUsers.map(u => ({ id: u.id, name: u.name })));

        // Check if the deleted user is no longer in props
        if (displayUsers.length > users.length) {
            console.log('✅ User was removed from props - syncing display users');
            setDisplayUsers(users);
        } else {
            console.log('ℹ️ No user removed from props yet');
        }
    }, [users, displayUsers]);
    ...
    const handleDeleteUser = async (userId: number) => {
        ...
        const originalUsers = [...displayUsers];
