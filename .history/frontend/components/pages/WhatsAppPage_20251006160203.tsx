...
const WhatsAppPage: React.FC<WhatsAppPageProps> = ({ 
    ...
    const handleBotConfigChange = (field: keyof ChatbotConfig, value: any) => {
        onSaveChatbotConfig({ ...chatbotConfig, [field]: value });
    }

    const handleQuestionChange = (id: string, field: keyof ChatbotQuestion, value: any) => {
        const newQuestions = chatbotConfig.questions.map(q => q.id === id ? {...q, [field]: value} : q);
        onSaveChatbotConfig({...chatbotConfig, questions: newQuestions});
    }

    const addQuestion = () => {
        const newQuestion: ChatbotQuestion = { id: faker.string.uuid(), question: '', crmField: 'name', type: 'text' };
        onSaveChatbotConfig({...chatbotConfig, questions: [...chatbotConfig.questions, newQuestion]});
    }
    const deleteQuestion = (id: string) => {
        onSaveChatbotConfig({...chatbotConfig, questions: chatbotConfig.questions.filter(q => q.id !== id)});
    }

    const handleKeywordRuleChange = (id: string, field: keyof ChatbotKeywordRule, value: any) => {
        const newRules = chatbotConfig.keywordRules.map(r => r.id === id ? {...r, [field]: value} : r);
        onSaveChatbotConfig({...chatbotConfig, keywordRules: newRules});
    }

    const addKeywordRule = () => {
        const newRule: ChatbotKeywordRule = { id: faker.string.uuid(), keyword: '', response: '' };
        onSaveChatbotConfig({...chatbotConfig, keywordRules: [...chatbotConfig.keywordRules, newRule]});
    }
    const deleteKeywordRule = (id: string) => {
        onSaveChatbotConfig({...chatbotConfig, keywordRules: chatbotConfig.keywordRules.filter(r => r.id !== id)});
    }
