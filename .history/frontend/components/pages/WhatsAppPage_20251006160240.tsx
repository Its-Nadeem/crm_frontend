...
// Show chatbot tab only if chatbotConfig exists or if we have a default config
if (activeTab === 'chatbot' && !chatbotConfig) {
    return (
        <div className="space-y-6">
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
                <div className="flex items-center">
                    <AppIcons.Warning className="h-5 w-5 text-yellow-400 mr-3" />
                    <div>
                        <h3 className="text-sm font-medium text-yellow-800 dark:text-yellow-200">Chatbot Configuration Needed</h3>
                        <p className="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
                            Please configure your chatbot settings first. You can set up the chatbot in the Settings section.
                        </p>
                    </div>
                </div>
            </div>
...
interface WhatsAppPageProps {
    ...
    onSaveChatbotConfig: (config: ChatbotConfig) => void;
    customFieldDefs: CustomFieldDefinition[];
    campaigns: WhatsAppCampaign[];
    onSaveCampaign: (campaign: WhatsAppCampaign) => void;
...
const WhatsAppPage: React.FC<WhatsAppPageProps> = ({ 
    ...
    chatbotConfig, onSaveChatbotConfig, customFieldDefs,
    campaigns, onSaveCampaign, onDeleteCampaign, 
    ...
    const handleBotConfigChange = (field: keyof ChatbotConfig, value: any) => {
        onSaveChatbotConfig({ ...currentChatbotConfig, [field]: value });
    }

    const handleQuestionChange = (id: string, field: keyof ChatbotQuestion, value: any) => {
        const newQuestions = currentChatbotConfig.questions.map(q => q.id === id ? {...q, [field]: value} : q);
        onSaveChatbotConfig({...currentChatbotConfig, questions: newQuestions});
    }

    const addQuestion = () => {
        const newQuestion: ChatbotQuestion = { id: faker.string.uuid(), question: '', crmField: 'name', type: 'text' };
        onSaveChatbotConfig({...currentChatbotConfig, questions: [...currentChatbotConfig.questions, newQuestion]});
    }
    const deleteQuestion = (id: string) => {
        onSaveChatbotConfig({...currentChatbotConfig, questions: currentChatbotConfig.questions.filter(q => q.id !== id)});
    }

    const handleKeywordRuleChange = (id: string, field: keyof ChatbotKeywordRule, value: any) => {
        const newRules = currentChatbotConfig.keywordRules.map(r => r.id === id ? {...r, [field]: value} : r);
        onSaveChatbotConfig({...currentChatbotConfig, keywordRules: newRules});
    }

    const addKeywordRule = () => {
        const newRule: ChatbotKeywordRule = { id: faker.string.uuid(), keyword: '', response: '' };
        onSaveChatbotConfig({...currentChatbotConfig, keywordRules: [...currentChatbotConfig.keywordRules, newRule]});
    }
    const deleteKeywordRule = (id: string) => {
        onSaveChatbotConfig({...currentChatbotConfig, keywordRules: currentChatbotConfig.keywordRules.filter(r => r.id !== id)});
    }
    ...
    return (
        <div className="space-y-6">
            ...
            {modal.type === 'campaign' && <CampaignFormModal campaign={modal.data} templates={templates} users={users} pipelineStages={pipelineStages} customFieldDefs={customFieldDefs} onClose={closeModal} onSave={onSaveCampaign} />}
                ...
                <div className="space-y-8">
                        ...
                        <div className="flex items-center justify-between p-4 bg-background rounded-lg">
                            <h4 className="font-semibold text-on-surface">Enable Website Chatbot</h4>
                             <label className="relative inline-flex items-center cursor-pointer">
                              <input type="checkbox" checked={chatbotConfig.isEnabled} onChange={(e) => handleBotConfigChange('isEnabled', e.target.checked)} className="sr-only peer" />
                         ...
                         <div>
                            <p className="text-sm text-subtle">Copy this code into your website's HTML before the closing `&lt;/body&gt;` tag.</p>
                             <pre className="bg-background p-4 rounded-lg text-xs text-on-surface overflow-auto mt-2 font-mono">{generateWidgetCode(chatbotConfig)}</pre>
                        </div>
                    </SettingCard>
                    ...
                    <SettingCard title="Widget Customization">
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                     <div>
                                        ...
                                        <input type="color" value={chatbotConfig.widgetColor} onChange={e => handleBotConfigChange('widgetColor', e.target.value)} className="mt-1 w-full h-10 p-1 bg-background border border-muted rounded-md cursor-pointer"/>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-subtle">Widget Style</label>
                                        <select value={chatbotConfig.widgetStyle} onChange={e => handleBotConfigChange('widgetStyle', e.target.value)} className="mt-1 block w-full bg-background border border-muted rounded-md py-2 px-3 h-10">
                                ...
                                <div>
                                    ...
                                    <select value={chatbotConfig.widgetPosition} onChange={e => handleBotConfigChange('widgetPosition', e.target.value)} className="mt-1 block w-full bg-background border border-muted rounded-md py-2 px-3">
                             ...
                             <div className="relative h-80 bg-slate-200 dark:bg-slate-800 rounded-lg overflow-hidden flex items-center justify-center p-4">
                                ...
                                <div className={`absolute bottom-5 ${chatbotConfig.widgetPosition === 'bottom-left' ? 'left-5' : 'right-5'}`}>
                                    <button
                                        className={`transition-all duration-300 ease-in-out text-white shadow-lg flex items-center gap-2`}
                                        style={{ backgroundColor: chatbotConfig.widgetColor, borderRadius: chatbotConfig.widgetStyle === 'icon' ? '50%' : '30px', padding: chatbotConfig.widgetStyle === 'icon' ? '1rem' : '1rem 1.5rem', }}
                                    >
                                        <AppIcons.ChatBubble className="w-6 h-6" />
                                        {chatbotConfig.widgetStyle === 'button' && <span className="font-semibold">Chat with us</span>}
                                    </button>
                                </div>
                            </div>
                        </div>
                     ...
                     <SettingCard title="Conversation Flow" description="Define how the chatbot interacts with visitors.">
                        <textarea value={chatbotConfig.welcomeMessage} onChange={(e) => handleBotConfigChange('welcomeMessage', e.target.value)} rows={3} className="w-full bg-background border border-muted rounded-md py-2 px-3" placeholder="Welcome message..."/>
                        <div className="space-y-2">
                            {chatbotConfig.questions.map((q, i) => (
                        ...
                        <textarea value={chatbotConfig.thankYouMessage} onChange={(e) => handleBotConfigChange('thankYouMessage', e.target.value)} rows={3} className="w-full bg-background border border-muted rounded-md py-2 px-3" placeholder="Thank you message..."/>
                    </SettingCard>
                </div>
            )}
        </div>
