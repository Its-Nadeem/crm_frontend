...
const App: React.FC = () => {
    ...
    const onUpdateLead = useCallback(async (updatedLead: Lead, oldLead?: Lead) => {
        try {
            // Only make API call if this is not already an API response
            if (updatedLead._id && !oldLead) {
                // This is likely already an API response, just update local state
                console.log('Updating lead in local state only (API response)');
                setLeads(prev => prev.map(l => l.id === updatedLead.id ? updatedLead : l));
            } else {
                // This is a local update, make API call
