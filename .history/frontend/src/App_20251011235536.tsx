...
const App: React.FC = () => {
    ...
    const onSaveSmsTemplate = useCallback(async (template: SMSTemplate) => {
        try {
            if (template.id) {
                await apiService.updateSmsTemplate(template.id, template);
                setSmsTemplates(prev => prev.map(t => t.id === template.id ? template : t));
            } else {
                const newTemplate = await apiService.createSmsTemplate(template);
    ...
    const onDeleteSmsTemplate = useCallback(async (id: string) => {
        try {
            await apiService.deleteSmsTemplate(id);
    ...
    const onSaveSmsCampaign = useCallback(async (campaign: SMSCampaign) => {
        try {
            if (campaign.id) {
                await apiService.updateSmsCampaign(campaign.id, campaign);
                setSmsCampaigns(prev => prev.map(c => c.id === campaign.id ? campaign : c));
            } else {
                const newCampaign = await apiService.createSmsCampaign(campaign);
    ...
    const onDeleteSmsCampaign = useCallback(async (id: string) => {
        try {
            await apiService.deleteSmsCampaign(id);
