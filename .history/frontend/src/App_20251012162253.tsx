    ...
    const handleDeleteUser = useCallback(async (userId: number) => {
        try {
            ...
            setUsers(prev => {
                ...
                console.log(`✅ User ${userId} removed from local state. Users before: ${prev.length}, after: ${updated.length}`);
            ...
            console.log('🔄 Making API call to delete user from backend...');
            // Then make API call to delete user from backend
            await apiService.requestWithRetry(`/settings/users/${userId}`, {
                method: 'DELETE'
            });

            console.log('✅ User deleted successfully from both backend and frontend state');
        } catch (error) {
            ...
            try {
                console.log('🔄 API call failed, refetching users to restore correct state');
                await fetchUsers();
            } catch (refetchError) {
                console.error('❌ Failed to refetch users after API error:', refetchError);
            }
