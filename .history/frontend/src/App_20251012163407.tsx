...
const App: React.FC = () => {
    ...
    }, [currentOrganization, currentUser, fetchLeads, fetchUsers, fetchStages, fetchTasks, fetchCustomFields, fetchLeadScoreRules]);
    ...
    const userUsers = useMemo(() => {
        ...
        console.log('🔄 App.tsx: Recomputing userUsers:', filtered.length, 'users for org:', currentOrganization?.id);
        console.log('📊 App.tsx: Total users in state:', users.length);
        console.log('🏢 App.tsx: Current organization ID:', currentOrganization?.id);
        return filtered;
    ...
    const onAddNote = useCallback(async (noteData: { content: string; leadId: string }) => {
        try {
            ...
            // Update leads array optimistically with note activity
            ...
            if (lead) {
                ...
                const noteActivity: Activity = {
                    id: crypto.randomUUID(),
                    type: 'NOTE',
                    content: `Note added: <i class="whitespace-pre-wrap">"${noteData.content}"</i>`,
                ...
                setLeads(prev => prev.map(l => l.id === noteData.leadId ? updatedLead : l));
                console.log('Updated lead with note activity in App.tsx');
            }
        } catch (error) {
    ...
    const handleDeleteUser = useCallback(async (userId: number) => {
        try {
            console.log('🚀 App.tsx: Starting user deletion for userId:', userId);
            console.log('📊 App.tsx: Current users count before deletion:', users.length);
            console.log('👥 App.tsx: Current userUsers count before deletion:', userUsers.length);

            // Update local state FIRST for immediate UI feedback
            setUsers(prev => {
                const updated = prev.filter(u => u.id !== userId);
                console.log(`✅ App.tsx: User ${userId} removed from local state. Users before: ${prev.length}, after: ${updated.length}`);
                return [...updated]; // Force new array reference
            });

            // Force React to process the state update immediately
            await new Promise(resolve => setTimeout(resolve, 0));

            console.log('📊 App.tsx: After state update, users count:', users.length);
            console.log('👥 App.tsx: After state update, userUsers count:', userUsers.length);

            console.log('🔄 App.tsx: Making API call to delete user from backend...');
            // Then make API call to delete user from backend
            await apiService.requestWithRetry(`/settings/users/${userId}`, {
                method: 'DELETE'
            });

            console.log('✅ App.tsx: User deleted successfully from both backend and frontend state');
            console.log('📊 App.tsx: Final users count:', users.length);
            console.log('👥 App.tsx: Final userUsers count:', userUsers.length);
        } catch (error) {
            console.error('❌ App.tsx: Failed to delete user from backend:', error);
            console.log('📊 App.tsx: Users count after error:', users.length);
            console.log('👥 App.tsx: userUsers count after error:', userUsers.length);

            // If API call fails, we need to refetch users to restore the correct state
            try {
                console.log('🔄 App.tsx: API call failed, refetching users to restore correct state');
                await fetchUsers();
                console.log('✅ App.tsx: Users refetched after error. New count:', users.length);
            } catch (refetchError) {
                console.error('❌ App.tsx: Failed to refetch users after API error:', refetchError);
            }

            throw error; // Re-throw to allow UI to handle the error
        }
    }, [fetchUsers, users, userUsers]);
    ...
    const mainContent = (
        <Routes>
            ...
            <Route path="/settings" element={<SettingsLayout />}>
                ...
                <Route path="users" element={<UserManagementPage key={`users-${userUsers.length}`} users={userUsers} teams={userTeams} onSaveUser={onSaveUser} onDeleteUser={handleDeleteUser} />} />
