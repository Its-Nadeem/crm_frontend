...
const App: React.FC = () => {
    ...
    const onSaveTeam = useCallback(async (team: Partial<Team>) => {
        try {
            ...
            const teamId = team.id || 'new';

            if(team.id && teams.some(t => t.id === team.id)) {
                // Update existing team
                setTeams(prev => prev.map(t => t.id === team.id ? { ...t, ...team } as Team : t));
            } else {
                // Add new team (ID will be generated by backend)
                const newTeam = { ...team, organizationId: currentOrganization!.id } as Team;
