...
const App: React.FC = () => {
    ...
    const onSaveTeam = useCallback(async (team: Partial<Team>) => {
        try {
            // For new teams, generate an ID before sending to backend
            ...
            if (isNewTeam) {
                // Generate a unique ID for new teams (similar to backend logic)
                teamData.id = `team_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
                console.log('Creating new team with generated ID:', teamData.id);
            }
