...
import { User, Lead, Task, UserRole, IntegrationLog, Activity, Stage, AutomationRule, CustomFieldDefinition, FollowUpStatus, Permission, Team, IntegrationSettings, IntegrationSource, WhatsAppTemplate, SavedFilter, ChatbotConfig, UserSessionLog, LeadSource, Organization, ScheduledMessage, EmailTemplate, EmailCampaign, ConnectedFacebookAccount, SMSTemplate, SMSCampaign, LeadScoreRule, SubscriptionPlan, SupportTicket, AuditLog, GlobalAnnouncement, Addon, SystemHealthMetric, ApiUsageLog, ErrorLog, GlobalAutomationRule, GlobalIntegrationStatus, LocalizationSettings, HomepageContent, BillingHistory, WhatsAppCampaign, CallScript, CallCampaign, CustomDashboardWidget, BlogPost, PaymentGatewaySetting, Inquiry, GlobalEmailTemplate, Coupon, OfferStrip, PricingCategory, WebhookConfig } from './types';
import { apiService } from './services/api';
import Dashboard from '../components/pages/Dashboard';
import LeadListPage from '../components/pages/LeadListPage';
import UserManagementPage from '../components/pages/UserManagementPage';
import IntegrationsPage from '../components/pages/IntegrationsPage';
import FacebookIntegrationPage from '../components/pages/FacebookIntegrationPage';
import GoogleAdsIntegrationPage from '../components/pages/GoogleAdsIntegrationPage';
import WebsiteIntegrationPage from '../components/pages/WebsiteIntegrationPage';
import CloudTelephonyIntegrationPage from '../components/pages/CloudTelephonyIntegrationPage';
import EmailMarketingIntegrationPage from '../components/pages/EmailMarketingIntegrationPage';
import SMSMarketingIntegrationPage from '../components/pages/SMSMarketingIntegrationPage';
import ApiWebhooksIntegrationPage from '../components/pages/ApiWebhooksIntegrationPage';
...
import { CustomFieldsTestPage } from './components/pages/CustomFieldsTestPage';
import { DebugCustomFields } from './components/pages/DebugCustomFields';
...
const App: React.FC = () => {
    ...
    const fetchLeadScoreRules = useCallback(async () => {
        if (currentOrganization) {
            try {
                const data = await apiService.getLeadScoreRules(currentOrganization.id);
                setLeadScoreRules(data);
            } catch (error) {
                console.error('Failed to fetch lead score rules:', error);
            }
        }
    }, [currentOrganization]);
    ...
    const fetchTeams = useCallback(async () => {
        if (currentOrganization) {
            try {
                const data = await apiService.getTeams(currentOrganization.id);
                setTeams(data);
            } catch (error) {
    ...
    useEffect(() => {
        if (currentOrganization && currentUser) {
            ...
            fetchLeadScoreRules();
            fetchTeams();
            fetchCustomDashboardWidgets();
        }
    }, [currentOrganization, currentUser, fetchLeads, fetchUsers, fetchStages, fetchTasks, fetchCustomFields, fetchLeadScoreRules, fetchTeams]);

    const fetchCustomDashboardWidgets = useCallback(async () => {
        if (currentOrganization && currentUser) {
            try {
                const widgets = await apiService.getCustomDashboardWidgets(currentOrganization.id, currentUser.id);
                setCustomDashboardWidgets(widgets);
            } catch (error) {
                console.error('Failed to fetch custom dashboard widgets:', error);
                setCustomDashboardWidgets([]);
            }
        }
    ...
    const fetchSuperAdminData = useCallback(async () => {
        if (currentUser?.role === UserRole.SUPER_ADMIN) {
            try {
                const data = await apiService.requestWithRetry('/api/super-admin/dashboard');
                setSuperAdminData(data as any);
                // Also update payment gateway settings state
                if ((data as any).paymentGatewaySettings) {
                    setPaymentGatewaySettings((data as any).paymentGatewaySettings);
                }
            } catch (error) {
    ...
    const refreshSuperAdminData = useCallback(async () => {
        await fetchSuperAdminData();
    }, [fetchSuperAdminData]);

    // Load Super Admin data when Super Admin logs in
    useEffect(() => {
        if (currentUser?.role === UserRole.SUPER_ADMIN) {
            fetchSuperAdminData();
        }
    ...
    // Data filtering based on current user - force re-computation when users change
    ...
    const userUsers = useMemo(() => {
        ...
        return [...filtered];
    }, [users, currentOrganization, users.length, users.map(u => u.id).join(',')]); // Added user IDs as dependency
    ...
    const onUpdateLead = useCallback(async (updatedLead: Lead, oldLead?: Lead) => {
        try {
            ...
            // Don't throw error - allow component to continue with local state
            return false; // Indicate failure but don't break the UI
        }
    }, []);
    ...
    const onSaveUser = useCallback(async (user: User) => {
        try {
            // Check if this is a new user (id is 0 or not found in current users)
            const isNewUser = user.id === 0 || !users.some(u => u.id === user.id);

            if (isNewUser) {
                // For new users, create via settings API endpoint and add to state
                const createdUser = await apiService.requestWithRetry<any>('/settings/users', {
                    method: 'POST',
    ...
    const onAddStage = useCallback(async (name: string, color: string) => {
        if (!currentOrganization) {
            console.error('Cannot create stage: No current organization');
            return;
        }
    ...
    const onUpdateStage = useCallback(async (id: string, field: 'name'|'color', value: string) => {
        if (!currentOrganization) {
            console.error('Cannot update stage: No current organization');
            return;
        }
    ...
    const onDeleteStage = useCallback(async (id: string) => {
        if (!currentOrganization) {
            console.error('Cannot delete stage: No current organization');
            return;
        }
    ...
    const onSaveLeadScoreRule = useCallback(async (rule: LeadScoreRule) => {
        try {
            if(rule.id.startsWith('new_') || !rule.id) { // It's a new rule
                ...
                await apiService.createLeadScoreRule(rule); // Using POST for both create and update as per backend API
    ...
    const onDeleteLeadScoreRule = useCallback(async (id: string) => {
        try {
            ...
            await apiService.deleteLeadScoreRule(id);
    ...
    const onSaveTeam = useCallback(async (team: Partial<Team>) => {
        try {
            // For new teams, don't generate ID - let backend handle it
            // For existing teams, use the provided ID
            ...
            if(team.id && teams.some(t => t.id === team.id)) {
                ...
                // Update local state with server response
                setTeams(prev => prev.map(t => t.id === team.id ? response as Team : t));
            } else {
                ...
                setTeams(prev => [...prev, response as Team]);
            }
        } catch (error) {
    ...
    const onDeleteTeam = useCallback(async (id: string) => {
        try {
            // Make API call to delete team from backend first
            await apiService.requestWithRetry(`/settings/teams/${id}`, {
                method: 'DELETE'
            });
    ...
    // SMS Marketing handlers
    const onSaveSmsTemplate = useCallback(async (template: SMSTemplate) => {
        try {
            let response;
            if (template.id) {
                response = await fetch(`/api/marketing/templates/sms/${template.id}`, {
                    method: 'PUT',
                ...
                response = await fetch('/api/marketing/templates/sms', {
                    method: 'POST',
                    headers: {
            ...
            if (!response.ok) throw new Error('Failed to save SMS template');

            const savedTemplate = await response.json();
            if (template.id) {
                setSmsTemplates(prev => prev.map(t => t.id === template.id ? savedTemplate : t));
            } else {
                setSmsTemplates(prev => [...prev, savedTemplate]);
            }
        } catch (error) {
    ...
    const onDeleteSmsTemplate = useCallback(async (id: string) => {
        try {
            const response = await fetch(`/api/marketing/templates/sms/${id}`, {
                method: 'DELETE'
            });
    ...
    const onSaveSmsCampaign = useCallback(async (campaign: SMSCampaign) => {
        try {
            let response;
            if (campaign.id) {
                response = await fetch(`/api/marketing/campaigns/sms/${campaign.id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                ...
                response = await fetch('/api/marketing/campaigns/sms', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`
            ...
            if (!response.ok) throw new Error('Failed to save SMS campaign');

            const savedCampaign = await response.json();
            if (campaign.id) {
                setSmsCampaigns(prev => prev.map(c => c.id === campaign.id ? savedCampaign : c));
            } else {
                setSmsCampaigns(prev => [...prev, savedCampaign]);
            }
        } catch (error) {
    ...
    const onDeleteSmsCampaign = useCallback(async (id: string) => {
        try {
            const response = await fetch(`/api/marketing/campaigns/sms/${id}`, {
                method: 'DELETE'
            });

            if (!response.ok) throw new Error('Failed to delete SMS campaign');
    ...
    const handleSaveCustomWidget = useCallback(async (widget: CustomDashboardWidget) => {
        try {
            if (!currentOrganization || !currentUser) {
                console.error('Cannot save widget: missing organization or user');
                return;
            }
            ...
            const existingWidget = customDashboardWidgets.find(w => w.id === widget.id);

            if (existingWidget) {
                // Update existing widget
                await apiService.updateCustomDashboardWidget(widget.id, widgetData);
                setCustomDashboardWidgets(prev => prev.map(w => w.id === widget.id ? widget : w));
            } else {
                // Create new widget
                await apiService.createCustomDashboardWidget(widgetData);
                setCustomDashboardWidgets(prev => [...prev, widget]);
            }
        } catch (error) {
            console.error('Failed to save custom widget:', error);
            throw error; // Re-throw to allow UI to handle the error
        }
    }, [currentOrganization, currentUser, customDashboardWidgets]);

    const handleDeleteCustomWidget = useCallback(async (widgetId: string) => {
        try {
            await apiService.deleteCustomDashboardWidget(widgetId);
    ...
    const onUpdateProfile = useCallback(async (data: Partial<User>) => {
        if (currentUser) {
            try {
                // Make API call to update user profile
                await apiService.updateUser(currentUser.id.toString(), data);
    ...
    const onUpgradeSubscription = useCallback(async (planId: string, billingCycle: string) => {
        try {
            await apiService.changeSubscription(planId, billingCycle);
        } catch (error) {
    ...
    const onRefreshData = useCallback(async () => {
        // Refresh organization and billing data
        if (currentOrganization) {
            try {
                // Refresh the app data which includes billing history
                // This will trigger a re-fetch of all data
    ...
    const onRefreshLeadScoreRules = useCallback(async () => {
        if (currentOrganization) {
            try {
                const rules = await apiService.getLeadScoreRules(currentOrganization.id);
                setLeadScoreRules(rules);
            } catch (error) {
                console.error('Failed to refresh lead score rules:', error);
            }
        }
    }, [currentOrganization]);
    ...
    const handleDeleteUser = useCallback(async (userId: number) => {
        try {
            ...
            // Also force a re-render of the component by triggering a state change
            setTimeout(() => {
                console.log('🔄 Triggering additional re-render');
                // This will cause the useMemo to re-evaluate
                setUsers(current => {
                    console.log('🔄 Additional re-render with current users:', current.length);
                    return [...current];
                });
            }, 10);

            // Final re-render trigger
            setTimeout(() => {
                console.log('🔄 Final re-render trigger');
    ...
    if (currentUser.role === UserRole.SUPER_ADMIN) {
        return <SuperAdminDashboard
            {...{
                organizations: superAdminData.organizations,
                users: superAdminData.users,
                leads: superAdminData.leads,
                tasks: superAdminData.tasks,
                subscriptionPlans: superAdminData.subscriptionPlans,
                supportTickets: superAdminData.supportTickets,
                auditLogs: [],
                globalAnnouncement,
                addons: [],
                systemHealthData: [],
                ...
                homepageContent,
                blogPosts: superAdminData.blogPosts,
                inquiries: superAdminData.inquiries,
                setInquiries: (inquiries: Inquiry[]) => setSuperAdminData(prev => ({ ...prev, inquiries })),
                setLeads: (leads: Lead[]) => setSuperAdminData(prev => ({ ...prev, leads })),
                onSaveBlogPost,
                onDeleteBlogPost,
                onLogout: handleLogout,
                onAddOrganization: async (org) => {
                    const newOrg = await apiService.requestWithRetry('/api/super-admin/organizations', {
                        method: 'POST',
                        body: JSON.stringify(org)
                    });
                    setSuperAdminData(prev => ({ ...prev, organizations: [...prev.organizations, newOrg] }));
                },
                onUpdateOrganization: async (org) => {
                    const updatedOrg = await apiService.requestWithRetry(`/api/super-admin/organizations/${org.id}`, {
                        method: 'PUT',
                        body: JSON.stringify(org)
                    });
                    setSuperAdminData(prev => ({
                        ...prev,
                        organizations: prev.organizations.map(o => o.id === org.id ? updatedOrg : o)
                    }));
                },
                onDeleteOrganization: async (id) => {
                    await apiService.requestWithRetry(`/api/super-admin/organizations/${id}`, {
                        method: 'DELETE'
                    });
                    setSuperAdminData(prev => ({
                        ...prev,
                ...
                onSaveSubscriptionPlan: async (plan) => {
                    const updatedPlan = await apiService.requestWithRetry(`/api/super-admin/plans/${plan.id}`, {
                        method: 'PUT',
                        body: JSON.stringify(plan)
                    });
                    setSuperAdminData(prev => ({
                        ...prev,
                        subscriptionPlans: prev.subscriptionPlans.map(p => p.id === plan.id ? updatedPlan : p)
                    }));
                },
                onDeleteSubscriptionPlan: async (id) => {
                    await apiService.requestWithRetry(`/api/super-admin/plans/${id}`, {
                        method: 'DELETE'
                    });
                    setSuperAdminData(prev => ({
                        ...prev,
                ...
                onUpdateTicketStatus: async (id, status) => {
                    const updatedTicket = await apiService.requestWithRetry(`/api/super-admin/support-tickets/${id}/status`, {
                        method: 'PUT',
                        body: JSON.stringify({ status })
                    });
                    setSuperAdminData(prev => ({
                        ...prev,
                        supportTickets: prev.supportTickets.map(t => t.id === id ? updatedTicket : t)
                    }));
                },
                onSaveAnnouncement: setGlobalAnnouncement,
                onSaveUser,
                globalIntegrationStatus,
                onUpdateGlobalIntegrationStatus: setGlobalIntegrationStatus,
                localizationSettings,
                onUpdateLocalizationSettings: setLocalizationSettings,
                onUpdateHomepageContent: setHomepageContent,
                paymentGatewaySettings,
                onUpdatePaymentGatewaySettings: setPaymentGatewaySettings,
                coupons: [],
                onSaveCoupon,
                onDeleteCoupon,
                offerStrip,
                onUpdateOfferStrip,
                onSaveAddon: async (addon) => {
                    const updatedAddon = await apiService.requestWithRetry(`/api/super-admin/addons/${addon.id}`, {
                        method: 'PUT',
                        body: JSON.stringify(addon)
                    });
                    setAddons(prev => prev.map(a => a.id === addon.id ? updatedAddon as Addon : a));
                },
                onDeleteAddon: async (id) => {
                    await apiService.requestWithRetry(`/api/super-admin/addons/${id}`, {
                        method: 'DELETE'
                    });
                    setAddons(prev => prev.filter(a => a.id !== id));
                },
                pricingComparisonData,
                onUpdatePricingComparisonData: setPricingComparisonData,
                fetchAnalyticsData: async () => {
                    try {
                        // Use real backend API for analytics data
                        const data = await apiService.requestWithRetry('/api/super-admin/analytics');
                        return data;
                    } catch (error) {
                        ...
                        return {
                            ...
                            totalOrganizations: superAdminData.organizations.length,
                            activeOrganizations: superAdminData.organizations.filter(org =>
    ...
    const mainContent = (
        <Routes>
            ...
            <Route path="/tasks" element={<TasksPage tasks={userTasks} users={userUsers} leads={userLeads} teams={userTeams} currentUser={currentUser} updateTask={onUpdateTask} onAddTask={onAddTask} onDeleteTask={onDeleteTask} pipelineStages={userStages} customFieldDefs={userCustomFieldDefs} whatsAppTemplates={userWhatsAppTemplates} smsTemplates={userSmsTemplates} onUpdateLead={onUpdateLead} onScheduleMessage={onScheduleMessage} />} />
            ...
            <Route path="/marketing" element={<MarketingLayout />}>
                 <Route path="email" element={<EmailPage users={userUsers} pipelineStages={userStages} customFieldDefs={userCustomFieldDefs} />} />
                 <Route path="sms" element={<SMSPage
                     users={userUsers}
                     pipelineStages={userStages}
                     customFieldDefs={userCustomFieldDefs}
                     templates={userSmsTemplates}
                     campaigns={userSmsCampaigns}
                     onSaveTemplate={onSaveSmsTemplate}
                     onDeleteTemplate={onDeleteSmsTemplate}
                     onSaveCampaign={onSaveSmsCampaign}
                     onDeleteCampaign={onDeleteSmsCampaign}
                 />} />
            ...
            <Route path="/test-custom-fields" element={<CustomFieldsTestPage />} />
            <Route path="/debug-custom-fields" element={<DebugCustomFields />} />
            <Route path="/settings" element={<SettingsLayout />}>
                <Route index element={<SettingsPage pipelineStages={userStages} onAddStage={onAddStage} onUpdateStage={onUpdateStage} onDeleteStage={onDeleteStage} customFieldDefs={userCustomFieldDefs} onAddCustomField={onAddCustomField} onUpdateCustomField={onUpdateCustomField} onDeleteCustomField={onDeleteCustomField} leadScoreRules={userLeadScoreRules} onSaveLeadScoreRule={onSaveLeadScoreRule} onDeleteLeadScoreRule={onDeleteLeadScoreRule} onRefreshLeadScoreRules={onRefreshLeadScoreRules} theme={theme} toggleTheme={toggleTheme} currentUser={currentUser} hasPermission={hasPermission} currentOrganization={currentOrganization} subscriptionPlans={subscriptionPlans} billingHistory={userBillingHistory} onUpdateProfile={onUpdateProfile} addons={addons} onUpdateOrganization={onUpdateCurrentOrganization} onUpgradeSubscription={onUpgradeSubscription} onRefreshData={onRefreshData} />} />
                <Route path="users" element={<UserManagementPage key={`users-${userUsers.length}-${userUsers.map(u => u.id).join(',')}`} users={userUsers} teams={userTeams} onSaveUser={onSaveUser} onDeleteUser={handleDeleteUser} />} />
                <Route path="teams" element={<TeamsPage teams={userTeams} users={userUsers} onSaveTeam={onSaveTeam} onDeleteTeam={onDeleteTeam} />} />
                <Route path="integrations" element={<IntegrationsPage logs={userIntegrationLogs} settings={userIntegrationSettings} setSettings={setIntegrationSettings} customFieldDefs={userCustomFieldDefs} currentOrganization={currentOrganization} webhooks={userWebhooks} onAddWebhook={async (webhookData: Partial<WebhookConfig>) => { const newWebhook: WebhookConfig = {id: crypto.randomUUID(), name: webhookData.name || 'New Webhook', url: `https://api.example.com/wh/${crypto.randomUUID()}`, isEnabled: true, organizationId: currentOrganization.id, events: ['lead.created' as const]}; setWebhooks(p => [...p, newWebhook]); }} onDeleteWebhook={(id) => setWebhooks(p => p.filter(w => w.id !== id))} onUpdateWebhook={(wh) => setWebhooks(p => p.map(w => w.id === wh.id ? wh as WebhookConfig : w))} onSendTestLead={(source, form) => alert(`Test lead sent from ${source}: ${form}`)} syncFacebookLeads={() => alert('Syncing recent leads...')} />} />
                <Route path="integrations/facebook" element={<FacebookIntegrationPage settings={userIntegrationSettings} setSettings={setIntegrationSettings} customFieldDefs={userCustomFieldDefs} currentOrganization={currentOrganization} onSendTestLead={(source, form) => alert(`Test lead sent from ${source}: ${form}`)} />} />
                <Route path="integrations/google-ads" element={<GoogleAdsIntegrationPage settings={userIntegrationSettings} setSettings={setIntegrationSettings} customFieldDefs={userCustomFieldDefs} currentOrganization={currentOrganization} onSendTestLead={(source, form) => alert(`Test lead sent from ${source}: ${form}`)} />} />
                <Route path="integrations/website" element={<WebsiteIntegrationPage settings={userIntegrationSettings} setSettings={setIntegrationSettings} customFieldDefs={userCustomFieldDefs} currentOrganization={currentOrganization} onSendTestLead={(source, form) => alert(`Test lead sent from ${source}: ${form}`)} />} />
                <Route path="integrations/cloud-telephony" element={<CloudTelephonyIntegrationPage settings={userIntegrationSettings} setSettings={setIntegrationSettings} customFieldDefs={userCustomFieldDefs} currentOrganization={currentOrganization} onSendTestLead={(source, form) => alert(`Test lead sent from ${source}: ${form}`)} />} />
                <Route path="integrations/email-marketing" element={<EmailMarketingIntegrationPage settings={userIntegrationSettings} setSettings={setIntegrationSettings} customFieldDefs={userCustomFieldDefs} currentOrganization={currentOrganization} onSendTestLead={(source, form) => alert(`Test lead sent from ${source}: ${form}`)} />} />
                <Route path="integrations/sms-marketing" element={<SMSMarketingIntegrationPage settings={userIntegrationSettings} setSettings={setIntegrationSettings} customFieldDefs={userCustomFieldDefs} currentOrganization={currentOrganization} onSendTestLead={(source, form) => alert(`Test lead sent from ${source}: ${form}`)} />} />
                <Route path="integrations/api-webhooks" element={<ApiWebhooksIntegrationPage settings={userIntegrationSettings} setSettings={setIntegrationSettings} webhooks={userWebhooks} onAddWebhook={async (webhookData: Partial<WebhookConfig>) => { const newWebhook: WebhookConfig = {id: crypto.randomUUID(), name: webhookData.name || 'New Webhook', url: `https://api.example.com/wh/${crypto.randomUUID()}`, isEnabled: true, organizationId: currentOrganization.id, events: ['lead.created' as const]}; setWebhooks(p => [...p, newWebhook]); }} onDeleteWebhook={(id) => setWebhooks(p => p.filter(w => w.id !== id))} onUpdateWebhook={(wh) => setWebhooks(p => p.map(w => w.id === wh.id ? wh as WebhookConfig : w))} onTestWebhook={async (id) => ({ success: true, status: 200, responseTime: 150 })} currentOrganization={currentOrganization} />} />
