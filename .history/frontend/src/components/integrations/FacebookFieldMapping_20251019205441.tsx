        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        {facebookFields.map(field => (
                            <div key={field.name} className="bg-background rounded-lg border border-muted p-4 shadow-sm">
                                <div className="flex items-center justify-between mb-2">
                                    <label className="font-medium text-on-surface">{field.name}</label>
                                ...
                                <div className="space-y-2">
                                    <select
                                        value={getCurrentMapping(field.name)?.crmField || ''}
                                    ...
                                    >
                                        ...
                                        {getAvailableCrmFields(field.name).map(crmField => (
                                    ...
                                    {getCurrentMapping(field.name)?.crmField === 'stage' && (
                                        <select
                                            value={getCurrentMapping(field.name)?.customValue || ''}
                                    ...
                                    {getCurrentMapping(field.name)?.crmField && getCurrentMapping(field.name)?.crmField !== 'stage' && (
                                        <input
                                            type="text"
                                            placeholder="Custom value (optional)"
                                            value={getCurrentMapping(field.name)?.customValue || ''}
                                            onChange={(e) => handleMappingChange(field.name, getCurrentMapping(field.name)?.crmField || '', e.target.value)}
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                        {crmFields.map(field => {
                            const mappedField = mappings.find(m => m.crmField === field.key);
                            ...
                            return (
                                <div
                                    ...
                                    className={`rounded-lg border p-4 shadow-sm transition-colors ${
                                        isMapped ? 'bg-green-50 border-green-200' : 'bg-background border-muted'
                                    }`}
                                >
                                    <div className="flex items-center justify-between mb-2">
                                    ...
                                    {isMapped && (
                                        <div className="text-sm text-subtle">
                                            <div className="flex items-center gap-2">
                                                <span className="text-blue-600">Facebook field:</span>
                                                <span className="font-medium">{mappedField.facebookField}</span>
                                            </div>
                                            {mappedField.customValue && (
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className="text-purple-600">Custom value:</span>
                                                    <span className="font-medium">"{mappedField.customValue}"</span>
                                                </div>
                                            )}
                                        </div>
                                    )}
