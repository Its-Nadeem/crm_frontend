    ...
    return (
        <div className="space-y-6">
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                        {getSortedFacebookFields().map(field => {
                            ...
                            return (
                                <div key={field.name} className={`bg-background rounded-lg border p-4 shadow-sm ${
                                    isMandatory && !currentMapping?.crmField ? 'border-red-300 bg-red-50' : 'border-muted'
                                }`}>
                                    <div className="mb-2">
                                        ...
                                        <select
                                            ...
                                            className={`w-full px-3 py-2 border rounded-lg bg-background text-on-surface focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 ${
                                                isMandatory && !currentMapping?.crmField ? 'border-red-500' : 'border-muted'
                                            }`}
                                        >
                                        ...
                                        {isMandatory && !currentMapping?.crmField && (
            ...
            {(() => {
                ...
                return (
                    <div className={`rounded-lg p-4 ${!validation.isValid ? 'bg-red-50 border border-red-200' : 'bg-green-50 border border-green-200'}`}>
                        <h4 className={`font-semibold mb-3 ${!validation.isValid ? 'text-red-900' : 'text-green-900'}`}>
                            {!validation.isValid ? '⚠️ Mapping Validation Required' : '✅ Mapping Validation Passed'}
                        </h4>

                        {!validation.isValid ? (
                            <div className="space-y-2">
                                <p className="text-sm text-red-800">
                                    The following mandatory fields must be mapped before saving:
                                </p>
                                <ul className="text-sm text-red-700 space-y-1">
                                    {validation.unmappedFields.map(fieldName => (
                                ...
                                <div className="mt-3 p-2 bg-red-100 rounded text-xs text-red-800">
                                    💡 Map all required fields (name, email, phone) to appropriate CRM fields to continue.
                                </div>
                            </div>
                        ) : (
                            <div className="space-y-2">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                    <div>
                                        <span className="text-green-700">Total Facebook fields:</span>
                                        <span className="font-medium text-green-900 ml-2">{facebookFields.length}</span>
                                    </div>
                                    <div>
                                        <span className="text-green-700">Mapped fields:</span>
                                        <span className="font-medium text-green-900 ml-2">{mappings.filter(m => m.crmField).length}</span>
                                    </div>
                                </div>
                            </div>
                        )}
