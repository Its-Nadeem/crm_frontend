...
const LeadDetailModal: React.FC<LeadDetailModalProps> = ({ lead, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onScheduleMessage, currentUser, onClose }) => {
    ...
    // Get mappable custom fields for this organization
    const { getLeadDisplayFields, fields: allCustomFields } = useCustomFields(lead.organizationId);
    ...
    const handleSendWhatsApp = (message: string) => {
         const newActivity: Activity = { id: crypto.randomUUID(), type: 'WHATSAPP', content: `Sent WhatsApp: <i class="whitespace-pre-wrap">"${message}"</i>`, timestamp: new Date().toISOString(), authorId: currentUser.id };
    ...
    const handleSendSMS = (message: string) => {
        const newActivity: Activity = { id: crypto.randomUUID(), type: 'SMS', content: `Sent SMS: <i class="whitespace-pre-wrap">"${message}"</i>`, timestamp: new Date().toISOString(), authorId: currentUser.id };
       const updatedLead = { ...editableLead, activities: [newActivity, ...(editableLead.activities || [])] };
       setEditableLead(updatedLead);
       onUpdateLead(updatedLead, originalLead);
   ...
   const handleLogCall = (outcome: string, notes: string) => {
       ...
       const newActivity: Activity = { id: crypto.randomUUID(), type: 'CALL', content, timestamp: new Date().toISOString(), authorId: currentUser.id };
       const updatedLead = { ...editableLead, activities: [newActivity, ...(editableLead.activities || [])] };
       setEditableLead(updatedLead);
       onUpdateLead(updatedLead, originalLead);
    ...
    const handleSave = () => {
        if (!editableLead.id) {
            console.error('Cannot save lead: missing lead ID');
            return;
        }
    ...
    return (
        <Modal isOpen={true} onClose={handleSave} title="">
            ...
            {isLogCallModalOpen && <LogCallModal leadId={editableLead.id} leadPhone={editableLead.phone} currentUser={currentUser} onClose={() => setLogCallModalOpen(false)} onCallLogged={(callData) => handleLogCall(callData.outcome, callData.notes)} />}
                ...
                <div className="lg:col-span-1 overflow-y-auto pr-4">
                    ...
                    <CollapsibleSection title="Deal Info">
                     ...
                     <CollapsibleSection title="Custom Fields">
                         {(() => {
                             // Get mappable fields for this organization that are enabled for imports
                             const { mappableFields } = useCustomFields(lead.organizationId);
                             const importFields = mappableFields.filter(field => field.usage?.inImports);

                             if (importFields.length === 0) {
                                 return (
                                     <div className="text-center py-4 text-subtle">
                                         <p>No custom fields configured for lead display.</p>
                                         <p className="text-xs mt-1">Enable fields for "Imports" in Custom Fields settings.</p>
                                     </div>
                                 );
                             }
                             ...
                             return (
                                 <div className="space-y-4">
                                     {importFields.map(field => (
                                         <div key={field.id}>
                                             <label className="text-xs font-medium text-subtle flex items-center gap-2">
                                                 {field.name}
                                             ...
                                             {field.type === 'dropdown' && field.options ? (
                                                 <select
                                                     value={editableLead.customFields?.[field.id] as string || ''}
                                                     onChange={e => handleCustomFieldChange(field.id, e.target.value)}
                                                     className="w-full bg-background border border-muted rounded-lg py-2 px-3 text-sm text-on-surface focus:outline-none focus:ring-2 focus:ring-primary-500 mt-1"
                                                 >
                        ...
                        {activeTab === 'activity' && (
                            <div>
                                {(editableLead.activities || []).map((activity, index) => (
                                    <ActivityItem key={activity.id || `activity-${index}`} activity={activity} author={users.find(u => u.id === activity.authorId)} isFirst={index === (editableLead.activities || []).length -1} />
                                ))}
                            </div>
                        )}
                        {activeTab === 'tasks' && (
