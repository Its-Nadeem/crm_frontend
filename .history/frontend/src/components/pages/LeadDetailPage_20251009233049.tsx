...
import { useLeadActivities, useCreateLeadNote, useCreateLeadTask, leadQueryKeys } from '../../hooks/useLeadActivities';
...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ leads, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onAddNote, onScheduleMessage, currentUser, onRefreshLeadById }) => {
    ...
    const lead = leads.find(l => l.id === leadId) || currentLead;
    ...
    // React Query hooks for lead activities
    ...
    // Load lead from backend if not found in local state
    useEffect(() => {
        // Only fetch if we have a leadId, no lead data, not currently loading, and haven't attempted this lead yet
        const shouldFetchLead = leadId &&
                               !currentLead &&
                               !isLoadingLead &&
                               fetchAttemptedRef.current !== leadId &&
                               onRefreshLeadById;

        if (shouldFetchLead) {
            console.log('Lead not found in local state, attempting to refresh from backend:', leadId);
            fetchAttemptedRef.current = leadId; // Mark this lead as attempted
            setIsLoadingLead(true);

            onRefreshLeadById(leadId)
                .then((refreshedLead) => {
                    if (refreshedLead) {
                        console.log('Successfully loaded lead from backend:', refreshedLead.id);
                ...
                .catch((error) => {
                    console.error('Failed to refresh lead from backend:', error);
                })
                .finally(() => {
                    setIsLoadingLead(false);
                });
        }
    }, [leadId, currentLead, isLoadingLead, onRefreshLeadById]);
...
// FIX: Rewrote object creation to help TypeScript compiler and avoid iterator error.
   ...
   const handleEmailSent = (emailData: any) => {
    ...
    const handleAddTask = async (taskData: Omit<Task, 'id' | 'createdAt' | 'organizationId' | '_id'>) => {
        try {
            ...
            // Create a new task activity for the timeline
            const newTaskActivity: Activity = {
                id: crypto.randomUUID(),
                type: 'TASK_CREATED',
                content: `Task created: <strong>${taskData.title}</strong>`,
                timestamp: new Date().toISOString(),
                authorId: currentUser.id
            };

            // Update local lead state with new activity
            const updatedLead = {
                ...editableLead,
                activities: [newTaskActivity, ...(editableLead?.activities || [])]
            };
            setEditableLead(updatedLead);
            onUpdateLead(updatedLead, originalLead);
    ...
    const handleAddNote = async (noteData: { content: string; leadId: string }) => {
        try {
            ...
            // Create a new note activity for the timeline
            const newNoteActivity: Activity = {
                id: crypto.randomUUID(),
                type: 'NOTE',
            ...
            const updatedLead = {
                ...editableLead,
                activities: [newNoteActivity, ...(editableLead?.activities || [])]
            };
            setEditableLead(updatedLead);
            onUpdateLead(updatedLead, originalLead);
                    ...
                    <div className="flex-grow overflow-y-auto p-4 space-y-6">
                        ...
                        {activeTab === 'tasks' && (
                            <div className="space-y-2">
                                {isAddingTask && <AddTaskForm lead={editableLead} onAddTask={handleAddTask} onCancel={() => setIsAddingTask(false)} currentUser={currentUser}/>}
                                {isAddingNote && <AddNoteForm lead={editableLead} onAddNote={handleAddNote} onCancel={() => setIsAddingNote(false)} currentUser={currentUser}/>}
                                {leadTasks.map(task => (
                                 ...
                                 {leadTasks.length === 0 && !isAddingTask && !isAddingNote && <p className="text-center text-subtle py-8">No tasks for this lead.</p>}
                            </div>
                        )}
                        {activeTab === 'scheduled' && (
