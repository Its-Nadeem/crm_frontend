...
// FIX: Rewrote object creation to help TypeScript compiler and avoid iterator error.
   ...
   const handleEmailSent = (emailData: any) => {
    ...
    const handleOptimisticTaskUpdate = (activity: Activity) => {
        if (!editableLead) return;

        // Add the optimistic activity to the lead's activities
        const updatedLead = {
            ...editableLead,
            activities: [activity, ...(editableLead.activities || [])]
        };
        setEditableLead(updatedLead);
        onUpdateLead(updatedLead, originalLead);
    };
    ...
    const handleOptimisticNoteUpdate = (activity: Activity) => {
        if (!editableLead) return;
        ...
        <div className="space-y-6">
            ...
            {isAddNoteModalOpen && editableLead && (
                <AddNoteModal
                    ...
                    onOptimisticUpdate={handleOptimisticNoteUpdate}
                    leadId={editableLead.id}
