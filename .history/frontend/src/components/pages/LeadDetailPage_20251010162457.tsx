...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ leads, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onAddNote, onScheduleMessage, currentUser, onRefreshLeadById }) => {
    ...
    // Use dedicated query for the current lead
    const {
        data: lead,
        isLoading: isLoadingLead,
        isFetching: isFetchingLead,
        isRefetching: isRefetchingLead,
    } = useLead(leadId || '', currentUser);

    // Add an isMutating ref to suppress loader during mutations
    const isMutatingRef = useRef(false);
    const [localMutationRunning, setLocalMutationRunning] = useState(false);
    ...
    const createNoteMutation = useCreateLeadNote(leadId || '', currentUser, isMutatingRef);
    const createTaskMutation = useCreateLeadTask(leadId || '', currentUser);

    // Keep isMutatingRef true while note/task mutations are pending to avoid showing the full-page loading skeleton
    useEffect(() => {
        isMutatingRef.current = !!(createNoteMutation?.isPending || createTaskMutation?.isPending || localMutationRunning);
    }, [createNoteMutation?.isPending, createTaskMutation?.isPending, localMutationRunning]);
...
// FIX: Rewrote object creation to help TypeScript compiler and avoid iterator error.
   ...
   const handleEmailSent = (emailData: any) => {
    ...
    const handleNoteSaveComplete = () => {
        // Stop any auto-refresh after note is saved
        console.log('Note saved successfully, auto-refresh stopped');
    ...
    // Keep refs to the latest lead state and only auto-save on unmount.
    const editableLeadRef = useRef<Lead | null>(editableLead);
    const originalLeadRef = useRef<Lead | null>(originalLead);
    const hasUnsavedRef = useRef<boolean>(false);

    // Sync refs and compute whether there are unsaved changes whenever editable/original change.
    useEffect(() => {
        editableLeadRef.current = editableLead;
        originalLeadRef.current = originalLead;
        try {
            hasUnsavedRef.current = !!(editableLead && originalLead) && JSON.stringify(editableLead) !== JSON.stringify(originalLead);
        } catch {
            // Fallback comparison if stringify fails for any reason
            hasUnsavedRef.current = !!(editableLead && originalLead) && editableLead !== originalLead;
        }
    }, [editableLead, originalLead]);

    // Run cleanup only on component unmount (empty deps). This prevents saves when deps change (e.g., adding a note).
    useEffect(() => {
        return () => {
            if (hasUnsavedRef.current && editableLeadRef.current && originalLeadRef.current) {
                try {
                    onUpdateLead(editableLeadRef.current, originalLeadRef.current);
                } catch (err) {
                    // swallow errors during unmount to avoid warnings
                }
            }
        };
    }, []);

    // Track whether we've ever had a lead to avoid showing the full-page loader on transient refetches
    const hadLeadRef = useRef<boolean>(!!lead);
    useEffect(() => { if (lead) hadLeadRef.current = true; }, [lead]);

    // Computed UI flags
    // showPageSkeleton should only be true on the very first load (never had a lead yet)
    const showPageSkeleton = !lead && isLoadingLead && !isMutatingRef.current && !hadLeadRef.current;
