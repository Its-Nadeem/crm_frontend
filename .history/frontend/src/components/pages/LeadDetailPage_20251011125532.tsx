...
// FIX: Rewrote object creation to help TypeScript compiler and avoid iterator error.
   ...
   const handleEmailSent = (emailData: any) => {
    ...
    // Computed UI flags - suppress loading during mutations
    const showPageSkeleton = !lead && isLoadingLead && !isMutatingRef.current;       // only first time, no data yet, not mutating
    const showInlineSync   = !!lead && (isFetchingLead || isRefetchingLead) && !isMutatingRef.current; // Don't show loading during mutations
    const showSyncingRibbon = !!lead && (isFetchingLead || isRefetchingLead) && !isLoadingLead && !isMutatingRef.current; // Don't show loading during mutations
