...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ leads, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onAddNote, onScheduleMessage, currentUser, onRefreshLeadById }) => {
    ...
    const {
        ...
        staleTime: 30 * 60 * 1000, // 30 minutes - keep data fresh much longer to prevent refetch after note save
        gcTime: 60 * 60 * 1000, // 60 minutes - keep data in cache very long
        retry: 1, // Only retry once on failure
        placeholderData: (prev) => prev, // Keep previous data while fetching new data
        // Add network mode to prevent background refetches
        networkMode: 'online',
    });
...
// FIX: Rewrote object creation to help TypeScript compiler and avoid iterator error.
   ...
   const handleEmailSent = (emailData: any) => {
    ...
    const handleAddNote = async (noteData: { content: string; leadId: string }) => {
        try {
            console.log('📝 Starting note save - no loading states should appear');
            // Save note silently to backend - no optimistic updates, no loading states
            await createNoteMutation.mutateAsync(noteData);
            console.log('✅ Note saved to backend successfully');
            setIsAddNoteModalOpen(false);
            // Force disable auto-refresh to prevent any auto-loading
    ...
    const handleNoteSaveComplete = () => {
        ...
        // Note: Mutation flag is already managed by the mutation itself
        // Just show visual feedback that manual refresh is required
        setTimeout(() => {
    ...
    // Computed UI flags - suppress loading during mutations and note operations
    const showPageSkeleton = !lead && isLoadingLead && !isMutatingRef.current;       // only first time, no data yet, not mutating
    const showInlineSync   = !!lead && (isFetchingLead || isRefetchingLead) && !isMutatingRef.current && !disableTimelineAutoRefresh; // Don't show loading during mutations or note save
    const showSyncingRibbon = !!lead && (isFetchingLead || isRefetchingLead) && !isLoadingLead && !isMutatingRef.current && !disableTimelineAutoRefresh; // Don't show loading during mutations or note save
    ...
    useEffect(() => {
        console.log('🔍 LeadDetailPage Debug:', {
            ...
            isFetchingLead,
            isRefetchingLead,
            isMutatingRef: isMutatingRef.current,
            disableTimelineAutoRefresh,
            showPageSkeleton,
            showInlineSync,
            showSyncingRibbon,
            hasLead: !!lead,
    ...
    }, [isLoadingLead, isFetchingLead, isRefetchingLead, disableTimelineAutoRefresh, lead, leadId]);
