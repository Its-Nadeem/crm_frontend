...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ leads, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onAddNote, onScheduleMessage, currentUser, onRefreshLeadById }) => {
    ...
    const {
        ...
        queryFn: async () => {
            ...
            try {
                ...
                console.log('✅ Lead data fetched successfully:', result?.id);
                return result;
            } catch (error) {
        ...
        networkMode: 'always', // Always use network
    });
    ...
    // Debug logging for leadId and activities - simplified
    useEffect(() => {
        if (leadId && !lead) {
            console.log('⚠️ Lead not found, checking leads array for:', leadId);
            const foundInArray = leads.find(l => l.id === leadId);
            console.log('🔍 Lead in array:', !!foundInArray, 'Total leads:', leads.length);
        }
    }, [leadId, lead, leads.length]);
   ...
   const handleEmailSent = (emailData: any) => {
    ...
    const handleAddNote = async (noteData: { content: string; leadId: string }) => {
        try {
            ...
            // Disable auto-refresh to prevent any auto-loading
    ...
    const handleNoteSaveComplete = () => {
        // Disable auto-refresh until user manually refreshes
        setDisableTimelineAutoRefresh(true);
        // Auto-refresh stays disabled until manual refresh
    };
    ...
    // Computed UI flags - ABSOLUTELY NO LOADING after note save
    const showPageSkeleton = !lead && isLoadingLead && !isMutatingRef.current && !disableTimelineAutoRefresh && !isFetchingLead;       // only first time, no data yet, not mutating, not after note save, not fetching
    const showInlineSync   = false; // DISABLE ALL inline loading indicators after note operations
    const showSyncingRibbon = false; // DISABLE ALL syncing ribbons after note operations

    // Additional safety check - if we're in a bad state, reset everything
    useEffect(() => {
        if (isLoadingLead && !lead && !isMutatingRef.current && !disableTimelineAutoRefresh) {
            console.log('🔄 Detected potential loading loop, resetting...');
            // Force reset the mutation state if we're stuck
            setTimeout(() => {
                if (isMutatingRef.current) {
                    isMutatingRef.current = false;
                    console.log('🔧 Force reset mutation state');
                }
            }, 1000);
        }
    }, [isLoadingLead, lead, isMutatingRef.current, disableTimelineAutoRefresh]);
    ...
    if (!lead && !isLoadingLead && !isFetchingLead) {
        return (
            <div className="text-center p-8">
    ...
    if (!lead) {
        // Fallback: try to find lead in the leads array
        const leadFromArray = leads.find(l => l.id === leadId);
        if (leadFromArray) {
            console.log('✅ Found lead in array as fallback');
            // Use lead from array as fallback
            return (
                <div className="space-y-6">
                    {/* ... rest of the component using leadFromArray instead of lead */}
                    <div className="text-center p-8">
                        <p className="text-subtle">Loading lead data...</p>
                        <p className="text-xs text-subtle mt-2">Using cached data while loading</p>
                    </div>
                </div>
            );
        }
        return null; // Should not happen due to above logic
    }
                    ...
                    <div className="flex-grow overflow-y-auto p-4 space-y-6">
                        {activeTab === 'activity' && (
                            <ActivityTimeline
                                ...
                                onManualRefresh={() => setDisableTimelineAutoRefresh(false)}
