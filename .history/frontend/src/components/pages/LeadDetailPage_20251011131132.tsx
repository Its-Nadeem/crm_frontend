...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ leads, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onAddNote, onScheduleMessage, currentUser, onRefreshLeadById }) => {
    ...
    // Simple lead lookup - find lead in props or fetch if not found
    useEffect(() => {
        if (leadId && currentUser) {
            // First try to find in the leads array
            const foundInArray = leads.find(l => l.id === leadId);
            if (foundInArray) {
                console.log('✅ Found lead in array:', leadId);
                setLead(foundInArray);
            } else if (!lead && !isLoadingLead) {
                console.log('⚠️ Lead not found in array, fetching from backend:', leadId);
                fetchLead(leadId);
            }
        }
    }, [leadId, leads, currentUser]);
