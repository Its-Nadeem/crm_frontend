...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ leads, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onAddNote, onScheduleMessage, currentUser, onRefreshLeadById }) => {
    ...
    // Simple lead state management - NO React Query
    const [lead, setLead] = useState<Lead | null>(null);
    const [isLoadingLead, setIsLoadingLead] = useState(false);
    const [isFetchingLead, setIsFetchingLead] = useState(false);
    const [isRefetchingLead, setIsRefetchingLead] = useState(false);

    // Simple lead fetching function
    const fetchLead = async (leadId: string) => {
        if (!leadId || !currentUser) return;

        try {
            console.log('🔄 Fetching lead from backend:', leadId);
            const result = await apiService.getLeadById(leadId);
            if (result) {
                console.log('✅ Lead fetched successfully, updating state:', result.id);
                setLead(result);
                setIsLoadingLead(false);
                setIsFetchingLead(false);
                setIsRefetchingLead(false);
            } else {
                console.log('❌ No lead data returned from backend');
            }
        } catch (error) {
    ...
    // Fetch lead on mount and when leadId changes
    useEffect(() => {
        if (leadId && currentUser) {
            console.log('🔄 LeadDetailPage mounted/leadId changed, fetching lead:', leadId);
            fetchLead(leadId);
        }
    }, [leadId, currentUser]);

    // Also try to find lead in the leads array prop
    useEffect(() => {
        if (leadId && leads.length > 0) {
            const foundLead = leads.find(l => l.id === leadId);
            if (foundLead && !lead) {
                console.log('✅ Found lead in props array, using it:', foundLead.id);
                setLead(foundLead);
            }
        }
    }, [leadId, leads, lead]);
...
// FIX: Rewrote object creation to help TypeScript compiler and avoid iterator error.
   ...
   const handleEmailSent = (emailData: any) => {
    ...
    if (!lead && !isLoadingLead && !isFetchingLead) {
        // Try to find lead in the leads array as last resort
        const leadFromArray = leads.find(l => l.id === leadId);
        if (leadFromArray) {
            console.log('✅ Using lead from props array as fallback');
            // Don't return here - let the component render with the lead from array
        } else {
            return (
                <div className="text-center p-8">
    ...
    if (!lead) {
        // Final fallback: try to use lead from props array
        const leadFromArray = leads.find(l => l.id === leadId);
        if (leadFromArray) {
            console.log('✅ Using lead from props array as final fallback');
            // Component will render with leadFromArray instead of lead
            // This is handled by the rest of the component logic
        } else {
