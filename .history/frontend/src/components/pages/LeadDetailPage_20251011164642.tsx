   ...
   const handleEmailSent = (emailData: any) => {
    ...
    const handleAddNote = async (noteData: { content: string; leadId: string }) => {
        try {
            ...
            // Refresh from props leads array immediately to ensure lead data is available
            // This is more reliable than API calls and prevents rate limiting
            const updatedLeadFromProps = leads.find(l => l.id === noteData.leadId);
            if (updatedLeadFromProps) {
                console.log('Updating lead data from props after note creation');
                setLead(updatedLeadFromProps);
                setEditableLead(updatedLeadFromProps);
            }

            // Use batched request system for activities refresh only (less critical)
            setTimeout(() => {
                addToRequestBatch('activities');
            }, 1000); // Longer delay for activities
        } catch (error) {
            // Silent error handling - no console logs
            setIsAddNoteModalOpen(false);
        }
