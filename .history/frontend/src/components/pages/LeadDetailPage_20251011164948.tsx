...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ leads, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onAddNote, onScheduleMessage, currentUser, onRefreshLeadById }) => {
    ...
    // Simple lead fetching function - completely silent
    ...
    // Also try to find lead in the leads array prop - with better throttling
    useEffect(() => {
        if (leadId && leads.length > 0) {
            const timeoutId = setTimeout(() => {
                ...
                if (foundLead && !lead) {
                    ...
                    console.log('Lead not found in props array, leads available:', leads.length);
                }
            }, 100); // Reduced delay since this is just props lookup
            return () => clearTimeout(timeoutId);
        }
    }, [leadId, leads, lead]);
...
// FIX: Rewrote object creation to help TypeScript compiler and avoid iterator error.
   ...
   const handleEmailSent = (emailData: any) => {
    ...
    const handleAddNote = async (noteData: { content: string; leadId: string }) => {
        try {
            // Save note silently to backend - no loading states, no optimistic updates
            await createNoteMutation.mutateAsync(noteData);
            setIsAddNoteModalOpen(false);

            // Immediately update local state with optimistic note activity
            if (editableLead) {
                ...
                setLead(updatedLead);
                setEditableLead(updatedLead);
                console.log('Updated lead with note activity optimistically');
            }

            // Also update in parent App.tsx state via props refresh
            const updatedLeadFromProps = leads.find(l => l.id === noteData.leadId);
            if (updatedLeadFromProps) {
                const noteActivity: Activity = {
                    id: crypto.randomUUID(),
                    type: 'NOTE',
                    content: `Note added: <i class="whitespace-pre-wrap">"${noteData.content}"</i>`,
                ...
                const updatedLead = {
                    ...updatedLeadFromProps,
                    activities: [noteActivity, ...(updatedLeadFromProps.activities || [])]
                };

                setLead(updatedLead);
                setEditableLead(updatedLead);
                console.log('Updated lead from props with note activity');
            }

            // Refresh activities after a longer delay to prevent rate limiting
            setTimeout(() => {
                console.log('Refreshing activities after note creation...');
                refetchActivities();
            }, 2000);
        } catch (error) {
            // Silent error handling - no console logs
    ...
    // Simple check - if no lead, show not found
    if (!currentLead) {
        // Try one more time to fetch from API if lead not found in props
        if (leadId && currentUser) {
            console.log('Lead not found in props, trying API fetch...');
            setTimeout(() => {
                fetchLead(leadId);
            }, 500);
        }
