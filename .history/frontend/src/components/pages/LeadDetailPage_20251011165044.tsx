...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ leads, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onAddNote, onScheduleMessage, currentUser, onRefreshLeadById }) => {
    ...
    // Ultra-simple lead state management
    const [lead, setLead] = useState<Lead | null>(null);

    // Simple lead fetching function - with rate limiting protection
    const fetchLead = async (leadId: string) => {
        if (!leadId || !currentUser || lead) return; // Don't fetch if we already have lead data

        try {
            console.log('🔄 Fetching lead from API:', leadId);
            const result = await apiService.getLeadById(leadId);
            if (result) {
                console.log('✅ Lead fetched successfully:', result.id);
                setLead(result);
                setEditableLead(result);
            } else {
                console.log('❌ Lead not found in API');
            }
        } catch (error) {
    ...
    // Debounced lead fetching to prevent rapid fire requests
    const debouncedFetchLead = useCallback(() => {
        if (leadId && currentUser && !lead) { // Only fetch if we don't already have lead data
            console.log('🚀 Scheduling lead fetch for:', leadId);
            const timeoutId = setTimeout(() => {
                fetchLead(leadId);
            }, 500); // Increased delay to 500ms
            return () => clearTimeout(timeoutId);
        }
    }, [leadId, currentUser, lead]);

    // Fetch lead on mount and when leadId changes - with better throttling
    useEffect(() => {
        const cleanup = debouncedFetchLead();
        return cleanup;
    }, [debouncedFetchLead]);

    // Also try to find lead in the leads array prop - immediate lookup
    useEffect(() => {
        if (leadId && leads.length > 0 && !lead) {
            const foundLead = leads.find(l => l.id === leadId);
            if (foundLead) {
                console.log('✅ Found lead in props array:', foundLead.id);
                setLead(foundLead);
                setEditableLead(foundLead);
            } else {
                console.log('❌ Lead not found in props array. Available leads:', leads.length);
    ...
    // No emergency mechanisms needed - everything is simple now

    // Use lead from state, or fallback to props array
    const currentLead = lead || leads.find(l => l.id === leadId);
    const originalLead = useMemo(() => currentLead, [currentLead]);
    const leadTasks = useMemo(() => tasks.filter(t => t.leadId === currentLead?.id), [tasks, currentLead?.id]);
    ...
    // Disable automatic refetching to prevent rate limiting
    React.useEffect(() => {
        // Immediately disable any automatic refetching
        const interval = setInterval(() => {
            console.log('Ensuring no automatic refetching is happening...');
        }, 10000);
    ...
    // Batched request processor to prevent multiple simultaneous API calls
    const processBatchedRequests = useCallback(() => {
        if (requestTimeoutRef.current) {
            clearTimeout(requestTimeoutRef.current);
            requestTimeoutRef.current = null;
        }

        const requests = Array.from(pendingRequestsRef.current);
        ...
        if (uniqueRequests.length > 0) {
            console.log('Processing batched requests:', uniqueRequests);

            uniqueRequests.forEach((requestType, index) => {
                switch (requestType) {
                    case 'activities':
                        // Use much longer delay for activities to prevent rate limiting
                        setTimeout(() => {
                            console.log('Refetching activities...');
                            refetchActivities();
                        }, 2000 + (index * 500)); // 2-3 second delay with stagger
                        break;
                    case 'lead':
                        if (leadId && currentUser) {
                            setTimeout(() => {
                                console.log('Refetching lead...');
                                fetchLead(leadId);
                            }, 1500 + (index * 500)); // 1.5-2 second delay with stagger
                        }
                        break;
                }
            });
        }
    }, [leadId, currentUser, refetchActivities]);

    // Add request to batch queue
    const addToRequestBatch = useCallback((requestType: string) => {
        ...
        // Set new timeout for batched processing (500ms delay)
        requestTimeoutRef.current = setTimeout(processBatchedRequests, 500);
    ...
    // Simple lead lookup - just use the fetch function
...
// FIX: Rewrote object creation to help TypeScript compiler and avoid iterator error.
   ...
   const handleEmailSent = (emailData: any) => {
    ...
    const handleSave = async () => {
        if (!editableLead || !originalLead || isSaving) return;

        setIsSaving(true);
        try {
            await onUpdateLead(editableLead, originalLead);
    ...
    // Debounced save effect - only save after user stops typing for 2 seconds
    useEffect(() => {
        if (!hasUnsavedChanges || !editableLead) return;
    ...
    }, [hasUnsavedChanges, editableLead]);
    ...
    const handleAddNote = async (noteData: { content: string; leadId: string }) => {
        try {
            // Save note silently to backend
            await createNoteMutation.mutateAsync(noteData);
            setIsAddNoteModalOpen(false);

            // Immediately update the UI with optimistic update - no API calls
            if (editableLead) {
                ...
                const updatedLead = {
                    ...editableLead,
                    activities: [noteActivity, ...(editableLead.activities || [])]
                };
                ...
                // Also update the parent App.tsx state
                onUpdateLead(updatedLead, editableLead);

                console.log('✅ Note added and UI updated optimistically');
            }
        } catch (error) {
    ...
    // Removed auto-save useEffect that was causing infinite loop
    // Now using manual save with debouncing

    // Ultra-simple loading logic - NO LOADING STATES
    const showPageSkeleton = false; // NEVER show loading skeleton
    ...
    // Simple check - if no lead, show loading or not found
    if (!currentLead) {
        // Try to fetch from API if lead not found in props
        if (leadId && currentUser) {
            console.log('🔍 Lead not found in props, attempting to fetch from API...');
            // Try immediate fetch
            fetchLead(leadId);
            ...
            setTimeout(() => {
                if (!lead) {
                    console.log('🔄 Retrying lead fetch...');
                    fetchLead(leadId);
                }
            }, 1000);
        }
        ...
        return (
            <div className="text-center p-8">
                ...
                <h2 className="text-xl font-semibold">Loading lead...</h2>
                <p className="text-subtle mt-2">Please wait while we fetch the lead data.</p>
                <p className="text-xs text-subtle mt-4">Lead ID: {leadId}</p>
             ...
             <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                 ...
                 <div className="flex gap-2 w-full sm:w-auto">
                     <button
                         ...
                         className={`font-bold py-2 px-4 rounded-lg text-center flex items-center gap-2 ${
                    ...
                    <div className="flex-grow overflow-y-auto p-4 space-y-6">
                        {activeTab === 'activity' && (
                            <ActivityTimeline
                                ...
                                leadId={leadId}
                                onRefresh={() => {
                                    // Use batched request system for manual refresh
                                    console.log('Manual refresh triggered');
                                    addToRequestBatch('activities');
                                }}
                                onManualRefresh={() => {}} // No-op function since we removed the state management
                                realTime={false} // Disable real-time polling to prevent excessive requests
                                disableAutoRefresh={true} // Disable auto-refresh to prevent rate limiting
                            />
                        )}
                        {activeTab === 'tasks' && (
