...
// FIX: Rewrote object creation to help TypeScript compiler and avoid iterator error.
   ...
   const handleEmailSent = (emailData: any) => {
    ...
    const handleAddNote = async (noteData: { content: string; leadId: string }) => {
        try {
            console.log('💾 Saving note to backend:', noteData.content.substring(0, 50) + '...');

            // Save note to backend first
            await createNoteMutation.mutateAsync(noteData);
            console.log('✅ Note saved to backend successfully');

            setIsAddNoteModalOpen(false);

            // Refresh activities from backend after successful save (with delay to prevent rate limiting)
            setTimeout(() => {
                console.log('🔄 Refreshing activities from backend...');
                refetchActivities();
            }, 1500);
                        ...
                        {activeTab === 'activity' && (
                            <ActivityTimeline
                                ...
                                onRefresh={() => {
                                    ...
                                    // Use a longer delay for manual refresh to prevent rate limiting
                                    setTimeout(() => {
                                        console.log('🔄 Executing manual refresh...');
                                        refetchActivities();
                                    }, 1000);
                                }}
