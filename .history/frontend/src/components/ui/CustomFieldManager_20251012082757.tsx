...
export const CustomFieldManager: React.FC<CustomFieldManagerProps> = ({
        ...
        try {
            const fieldData = {
                ...
                options: formData.type === 'dropdown' && formData.options ?
                    formData.options.split(',').map(opt => opt.trim()) : undefined
            };

            if (editingField) {
                await apiService.updateCustomField(editingField._id!, fieldData);
                showToast('Custom field updated successfully', 'success');
            } else {
                await apiService.createCustomField(fieldData);
    ...
    const handleToggleMappable = async (field: CustomField) => {
        try {
            await apiService.updateCustomField(field._id!, {
                ...field,
                isMappable: !field.isMappable
            });
            loadFields();
            showToast(`Field ${!field.isMappable ? 'enabled' : 'disabled'} for mapping`, 'success');
        } catch (error) {
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-subtle">
                        Manage custom fields for your organization. Mappable fields will be available in lead imports and integrations.
                    </p>
                </div>
                <button
