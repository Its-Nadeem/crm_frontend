...
export const useCreateLeadNote = (leadId: string, currentUser: User, isMutatingRef?: React.MutableRefObject<boolean>) => {
  ...
  return useMutation({
    ...
    onSettled: () => {
        ...
        // Don't invalidate or refetch anything - just update the cache with optimistic updates
        // This prevents any loading states from appearing after note save
        console.log('Note mutation settled - no refetch triggered');
      },
  });
};
