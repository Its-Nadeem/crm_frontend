...
export const useCreateLeadNote = (leadId: string, currentUser: User, isMutatingRef?: React.MutableRefObject<boolean>) => {
  ...
  return useMutation({
    mutationFn: async (noteData: { content: string; leadId: string }) => {
      console.log('🚀 Starting note mutation for lead:', leadId, 'with data:', noteData);
      const result = await apiService.createLeadNote(leadId, noteData);
      console.log('✅ Note mutation successful:', result);
      return result;
    },

    onMutate: async (newNote) => {
      console.log('⚡ Note mutation onMutate - setting mutation flag and canceling queries');
      // Set mutation flag immediately to prevent loading states
      if (isMutatingRef) isMutatingRef.current = true;

      // Cancel any outgoing refetches to prevent loading states
      ...
      await queryClient.cancelQueries({ queryKey: leadQueryKeys.lead(leadId) });

      // Snapshot the previous value
      const previousActivities = queryClient.getQueryData(leadQueryKeys.activities(leadId));
      console.log('📋 Current activities count:', Array.isArray(previousActivities) ? previousActivities.length : 'none');
    ...
    onSuccess: (data, variables, context) => {
      ...
      console.log('💾 Server data integrated into cache');
    },

    onSettled: () => {
      console.log('🏁 Note mutation settled - resetting mutation flag after delay');
      // Reset mutation flag after a short delay to ensure no loading states appear
      setTimeout(() => {
        if (isMutatingRef) isMutatingRef.current = false;
        console.log('🔄 Mutation flag reset - normal state restored');
      }, 100);

      // CRITICAL: Don't invalidate or refetch anything - this prevents loading states
      console.log('🚫 No queries invalidated - preventing any loading states');
    },
  });
};
