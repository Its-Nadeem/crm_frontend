...
// Hook for creating a note - SIMPLE implementation with NO loading states
export const useCreateLeadNote = (leadId: string, currentUser: User, isMutatingRef?: React.MutableRefObject<boolean>) => {
  ...
  return useMutation({
    mutationFn: async (noteData: { content: string; leadId: string }) => {
      console.log('💾 Saving note to backend silently...');
    ...
    onMutate: async () => {
      // Set mutation flag to prevent any loading states
      if (isMutatingRef) isMutatingRef.current = true;
      console.log('🔒 Mutation started - loading states blocked');
    },
    ...
    onSuccess: () => {
      console.log('🎯 Note saved successfully - no cache updates, no loading states');
      // DO NOT update cache here - user will manually refresh to see the note
    },

    onSettled: () => {
      // Reset mutation flag after completion
      setTimeout(() => {
        if (isMutatingRef) isMutatingRef.current = false;
        console.log('🔓 Mutation completed - loading states unblocked');
      }, 100);
    },
  });
};
