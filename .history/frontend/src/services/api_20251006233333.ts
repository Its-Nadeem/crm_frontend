...
class ApiService {
  private async request<T>(endpoint: string, options?: RequestInit): Promise<T> {
    // Try both token storage locations for compatibility
    let token = localStorage.getItem('token') || localStorage.getItem('authToken');

    // If no token found, try to get current user data
    if (!token) {
    ...
    if (!response.ok) {
      const errorMessage = `API request failed: ${response.statusText} (${response.status})`;
      if (response.status === 401) {
        // Clear all stored tokens and user data on authentication error
