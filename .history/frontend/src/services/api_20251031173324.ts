...
class EnhancedApiService {
    ...
    private async request<T>(endpoint: string, options?: RequestInit): Promise<T> {
        ...
        if (!response.ok) {
            ...
            if (response.status === 401 && !endpoint.includes('/super-admin/')) {
                // Clear all stored authentication data on authentication error, but not for super admin routes
    ...
    async requestWithRetry<T>(endpoint: string, options?: RequestInit, maxRetries = 3): Promise<T> {
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (isAuthError && !isLastAttempt && !endpoint.includes('/super-admin/')) {
                    try {
    ...
    async testSuperAdminPaymentGateway(provider: string, amount?: number, currency?: string, description?: string): Promise<any> {
        return this.requestWithRetry<any>('/super-admin/test-payment', {
            method: 'POST',
            body: JSON.stringify({
                provider,
                amount: amount || 1.00,
                currency: currency || 'USD',
                description: description || 'Payment Gateway Test'
            }),
        });
    }
    ...
    async updateSuperAdminPaymentGatewaySettingByProvider(provider: string, settingData: any): Promise<any> {
        console.log('API Service - Updating payment gateway by provider:', provider, settingData);
        return this.requestWithRetry<any>(`/super-admin/payment-gateways/provider?provider=${encodeURIComponent(provider)}`, {
            method: 'PUT',
            body: JSON.stringify(settingData),
        });
    }
...
class ApiService {
     ...
     if (token && token.length > 10) { // Basic validation for token length
         ...
         if (tokenParts.length === 3) {
             try {
                 ...
                 console.warn('Token payload invalid, clearing corrupted token');
                 // Don't clear token for reports endpoint to allow backend validation
                 if (!endpoint.includes('/data/reports')) {
                     this.clearAuthData();
                 } else {
                     // Still send the potentially invalid token for backend to handle
                     (headers as Record<string, string>)['Authorization'] = `Bearer ${token}`;
                     console.log('Sending potentially invalid token for reports endpoint');
                 }
             }
         } else {
             console.warn('Token format invalid, clearing corrupted token');
             // Don't clear token for reports endpoint
             if (!endpoint.includes('/data/reports')) {
                 this.clearAuthData();
             } else {
                 // Still send the malformed token for backend to handle
                 (headers as Record<string, string>)['Authorization'] = `Bearer ${token}`;
                 console.log('Sending malformed token for reports endpoint');
             }
         }
     } else {
         console.warn('No valid token found for API request');
  ...
  async getLeadColumnPreferences(): Promise<any> {
      return this.request<any>('/leads/column-preferences');
  }
