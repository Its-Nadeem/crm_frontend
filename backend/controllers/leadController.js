...
const createLead = async (req, res) => {
   try {
     ...
     if (existingLead) {
       ...
       const lead = new Lead({
         ...leadData,
         assignedToId: leadData.assignedToId || 0, // Default to system (0) if not provided
         organizationId: req.user.organizationId,
         activities: [{
           type: 'LEAD_CREATED',
...
const updateLead = async (req, res) => {
     try {
         ...
         if (fieldChanges.length > 0) {
             ...
             // Check for recent duplicate activities (within last 5 seconds to prevent rapid updates)
             const isRecentDuplicate = (lead.activities || []).some(existing =>
                 existing.content === newActivityContent &&
                 existing.authorId === req.user.id &&
                 Math.abs(new Date(existing.timestamp).getTime() - new Date().getTime()) < 5000 // 5 seconds
             );
         ...
         // Remove duplicate activities (same content and timestamp within 5 seconds)
         const uniqueActivities = [];
         for (const activity of filteredActivities) {
             const isDuplicate = uniqueActivities.some(existing =>
                 existing.content === activity.content &&
                 Math.abs(new Date(existing.timestamp).getTime() - new Date(activity.timestamp).getTime()) < 5000 // 5 seconds
             );
             if (!isDuplicate) {
         ...
         // Trigger webhooks for lead update
         await triggerLeadWebhooks(req.user.organizationId, 'lead.updated', {
             lead: {
                 id: updatedLead.id,
                 ...
                 assignedTo: updatedLead.assignedToId ? {
                     id: updatedLead.assignedToId.id,
                     name: updatedLead.assignedToId.name,
                     email: updatedLead.assignedToId.email
                 } : null,
         ...
         // Trigger stage change webhook if stage was changed
         if (fieldChanges.some(change => change.field === 'Stage')) {
             await triggerLeadWebhooks(req.user.organizationId, 'lead.stage_changed', {
                 lead: {
                     id: updatedLead.id,
                     name: updatedLead.name,
                     email: updatedLead.email,
                 ...
                 oldStage: fieldChanges.find(change => change.field === 'Stage')?.oldValue,
                 newStage: fieldChanges.find(change => change.field === 'Stage')?.newValue
             });
         }
...
const deleteLead = async (req, res) => {
      try {
          ...
          if (lead && lead.organizationId === req.user.organizationId) {
              // Store lead data for webhook before deletion
              const leadData = {
                  id: lead.id,
                  name: lead.name,
                  email: lead.email,
              ...
              // Trigger webhooks for lead deletion
              await triggerLeadWebhooks(req.user.organizationId, 'lead.deleted', {
                  lead: leadData
              });
