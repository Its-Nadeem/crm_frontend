...
export const getSystemMonitoringData = async (req, res) => {
    try {
        ...
        if (healthData.length === 0) {
            // Access systemhealthmetrics collection directly
            const db = AuditLog.db;
            systemHealthMetrics = await db.collection('systemhealthmetrics').find({
        ...
        if (healthData.length > 0) {
            ...
            systemHealthMetrics.forEach((metric, index) => {
                transformedHealthData.push({
                    id: `health-${index}`,
                    timestamp: metric.timestamp instanceof Date ? metric.timestamp.toISOString() : new Date(metric.timestamp).toISOString(),
