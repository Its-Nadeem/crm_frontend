...
router.post('/send', requireAuth, async (req, res) => {
    try {
        ...
        // Find active SMS integration for the organization
        const integration = await SMSIntegration.findActiveByProvider(organizationId, 'twilio'); // Default to twilio for now

        if (!integration) {
            return res.status(400).json({
                success: false,
                message: 'No active SMS integration found for this organization'
            });
        }

        // Check if integration can send messages
        if (!integration.canSendMessage()) {
            return res.status(400).json({
                success: false,
                message: 'SMS sending limit exceeded or integration not connected'
            });
        }
...
router.post('/send-bulk', requireAuth, async (req, res) => {
    try {
        ...
        // Find active SMS integration for the organization
        const integration = await SMSIntegration.findActiveByProvider(organizationId, 'twilio'); // Default to twilio
        ...
        // Check if integration can send messages (rough estimate)
        const estimatedMessages = recipients.length;
        if (integration.usage.messagesSentToday + estimatedMessages > integration.settings.dailyLimit) {
            return res.status(400).json({
                success: false,
                message: 'Bulk SMS would exceed daily sending limit'
            });
        }
...
router.get('/status/:messageId', requireAuth, async (req, res) => {
    try {
        ...
        // Get latest status from provider if possible
        let statusData;
        try {
            // Find the integration
            const integration = await SMSIntegration.findById(smsMessage.integrationId);
            if (integration && integration.isConnected) {
                const smsService = new SMSService(
                    integration.provider,
                    integration.credentials,
                    integration._id.toString(),
                    smsMessage.organizationId
                );

                statusData = await smsService.getDeliveryStatus(messageId);
            } else {
                // Return database status if integration not available
                statusData = {
                    success: true,
                    messageId,
                    ...
                    errorCode: smsMessage.delivery.errorCode,
                    errorMessage: smsMessage.delivery.errorMessage
                };
            }
        } catch (providerError) {
