...
class SMSService {
    ...
    // Twilio SMS sending
    async _sendTwilioSMS(to, message, senderId) {
        ...
        return {
            ...
            cost: parseFloat(twilioMessage.price || 0.05),
            segments: parseInt(twilioMessage.numSegments || 1)
        };
    }
    ...
    // Twilio delivery status check
    async _getTwilioDeliveryStatus(providerMessageId) {
    ...
    // Get analytics data
    static async getAnalytics(organizationId, startDate, endDate) {
        const match = { organizationId };
        if (startDate && endDate) {
            match.createdAt = { $gte: startDate, $lte: endDate };
        }

        const analytics = await SMSMessage.aggregate([
            { $match: match },
            {
                $group: {
                    _id: null,
        ...
        return {
            ...
            deliveryRate: result.totalSent > 0 ? (result.delivered / result.totalSent * 100).toFixed(1) : 0,
            totalCost: result.totalCost.toFixed(2)
        };
    }

    // Get usage statistics
    static async getUsageStats(organizationId) {
        const now = new Date();
        ...
        return {
            monthlyUsage,
            dailyUsage,
            remainingCredits: 10000 - monthlyUsage // Example limit
        };
    }
