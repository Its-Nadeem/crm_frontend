                                ...
                                <ul className="mt-4 space-y-3 text-sm">
                                    {col.links.map(link => {
                                        ...
                                        if (url.startsWith('/#/')) {
                                            ...
                                            return <li key={text}><Link to={path ? `/${path}` : '/'} className="text-subtle hover:text-on-surface transition-colors block py-3 px-2 rounded hover:bg-muted/50 cursor-pointer">{text}</Link></li>;
                                        }
                                        if (url.startsWith('/') && !url.startsWith('//')) {
                                            // Handle internal routes like /privacy, /pricing, etc.
                                            return <li key={text}><Link to={url} className="text-subtle hover:text-on-surface transition-colors block py-3 px-2 rounded hover:bg-muted/50 cursor-pointer">{text}</Link></li>;
                                        }
                                        if (url.startsWith('#') && url.length > 1) {
                                            const path = `/${url}`;
                                            return <li key={text}><Link to={path} className="text-subtle hover:text-on-surface transition-colors block py-3 px-2 rounded hover:bg-muted/50 cursor-pointer">{text}</Link></li>;
                                        }
                                        // For external links or protocol-relative URLs (like //crm.clienn.com/api-docs)
                                        return <li key={text}><a href={url} className="text-subtle hover:text-on-surface transition-colors block py-3 px-2 rounded hover:bg-muted/50 cursor-pointer" target="_blank" rel="noopener noreferrer">{text}</a></li>;
                                    })}
                                </ul>
                            </div>
                        ))}
