...
const CallPage: React.FC<CallPageProps> = ({ users, pipelineStages, customFieldDefs }) => {
        ...
        const fetchData = async () => {
            try {
                ...
                const [scriptsData, campaignsData] = await Promise.all([
                    fetch('/api/marketing/scripts/call', { headers: authHeaders }).then(res => res.json()).then(res => res.data || []).catch(() => []),
                    fetch('/api/marketing/campaigns/call', { headers: authHeaders }).then(res => res.json()).then(res => res.data || []).catch(() => [])
                ]);
    ...
    return (
        <div className="space-y-6">
            {modal.type === 'script' && <ScriptFormModal script={modal.data} onClose={closeModal} onSave={handleSaveScript} />}
            {modal.type === 'campaign' && <CampaignFormModal campaign={modal.data} scripts={scripts} onClose={closeModal} onSave={handleSaveCampaign} users={users} pipelineStages={pipelineStages} customFieldDefs={customFieldDefs} />}
            ...
            {activeTab === 'campaigns' && (
                campaigns.length > 0 ? (
                        ...
                        {campaigns.map(c => (
                            <div key={c.id} className="bg-surface rounded-xl shadow-lg border border-muted/50 p-5 flex flex-col">
                                ...
                                <div className="flex items-center gap-2 pt-4 border-t border-muted">
                                    ...
                                    {c.status === 'DRAFT' && <button onClick={() => handleStartCampaign(c)} className="flex-1 text-center bg-green-500/20 hover:bg-green-500/30 text-green-500 text-sm font-semibold py-2 px-3 rounded-lg">Start</button>}
                    ...
                    <div className="flex items-center justify-center h-64 text-center">
                        <div>
                            <AppIcons.Call className="h-16 w-16 text-subtle mx-auto mb-4" />
                            <h3 className="text-lg font-semibold text-on-surface mb-2">No Call Campaigns Found</h3>
                            <p className="text-subtle mb-4">Create your first call campaign to get started with call marketing.</p>
            ...
            {activeTab === 'scripts' && (
                scripts.length > 0 ? (
                                ...
                                <div key={script.id} className="bg-background p-4 rounded-lg border border-muted">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button onClick={() => handleDeleteScript(script.id)} className="p-1.5 text-subtle hover:text-red-500 hover:bg-red-500/10 rounded-full"><AppIcons.Delete className="h-4 w-4"/></button>
                                        </div>
                                    </div>
                    ...
                    <div className="flex items-center justify-center h-64 text-center">
                        <div>
                            <AppIcons.Call className="h-16 w-16 text-subtle mx-auto mb-4" />
                            <h3 className="text-lg font-semibold text-on-surface mb-2">No Call Scripts Found</h3>
                            <p className="text-subtle mb-4">Create your first call script to start building campaigns.</p>
                            <button onClick={() => openModal('script')} className="bg-primary-600 hover:bg-primary-700 text-white font-bold py-2 px-4 rounded-lg flex items-center mx-auto">
                                <AppIcons.Add className="w-5 h-5 mr-2" /> Create Your First Script
                            </button>
                        </div>
                    </div>
                )
                ...
                campaigns.length > 0 ? (
                    ...
                    <div className="flex items-center justify-center h-64 text-center">
                        <div>
                            ...
                            <p className="text-subtle mb-4">Start some call campaigns to see analytics and performance data here.</p>
