...
const SMSPage: React.FC<SMSPageProps> = ({ users, pipelineStages, customFieldDefs }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch SMS templates and campaigns from backend
                // Get authentication token
                const token = localStorage.getItem('token') || localStorage.getItem('authToken');
                const authHeaders = token ? { 'Authorization': `Bearer ${token}` } : {};

                const [templatesData, campaignsData] = await Promise.all([
                    fetch('/api/marketing/templates/sms', { headers: authHeaders }).then(res => res.json()).then(res => res.data || []).catch(() => []),
                    fetch('/api/marketing/campaigns/sms', { headers: authHeaders }).then(res => res.json()).then(res => res.data || []).catch(() => [])
                ]);

                setTemplates(templatesData || []);
                setCampaigns(campaignsData || []);
            } catch (err) {
                console.error('Failed to fetch SMS marketing data:', err);
    ...
    const handleDeleteTemplate = async (templateId: string) => {
        ...
        try {
            const response = await fetch(`/api/marketing/templates/sms/${templateId}`, {
                method: 'DELETE'
            });
    ...
    const handleDeleteCampaign = async (campaignId: string) => {
        ...
        try {
            const response = await fetch(`/api/marketing/campaigns/sms/${campaignId}`, {
                method: 'DELETE'
            });
    ...
    const handleSendCampaign = async (campaign: SMSCampaign) => {
        try {
            ...
            // TODO: Trigger actual SMS sending via backend
    ...
    const handleScheduleCampaign = async (campaign: SMSCampaign, scheduledDate: string) => {
        try {
    ...
    const handleUnscheduleCampaign = async (campaign: SMSCampaign) => {
        try {
            const updatedCampaign = {
                ...campaign,
        ...
        return (
            <div className="space-y-6">
                <div className="flex items-center justify-center h-64">
                    ...
                    <span className="ml-3 text-lg">Loading SMS marketing data...</span>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            ...
            {modal.type === 'campaign' && <CampaignFormModal campaign={modal.data} templates={templates} onClose={closeModal} onSave={handleSaveCampaign} users={users} pipelineStages={pipelineStages} customFieldDefs={customFieldDefs} />}
            ...
            {activeTab === 'campaigns' && (
                campaigns.length > 0 ? (
                    ...
                    <div className="flex items-center justify-center h-64 text-center">
                        <div>
                            <AppIcons.SMS className="h-16 w-16 text-subtle mx-auto mb-4" />
                            <h3 className="text-lg font-semibold text-on-surface mb-2">No SMS Campaigns Found</h3>
                            <p className="text-subtle mb-4">Create your first SMS campaign to get started with SMS marketing.</p>
            ...
            {activeTab === 'templates' && (
                templates.length > 0 ? (
                    ...
                    <div className="flex items-center justify-center h-64 text-center">
                        <div>
                            <AppIcons.SMS className="h-16 w-16 text-subtle mx-auto mb-4" />
                            <h3 className="text-lg font-semibold text-on-surface mb-2">No SMS Templates Found</h3>
                            <p className="text-subtle mb-4">Create your first SMS template to start building campaigns.</p>
                ...
                campaigns.length > 0 ? (
                    ...
                    <div className="flex items-center justify-center h-64 text-center">
                        <div>
                            ...
                            <p className="text-subtle mb-4">Send some SMS campaigns to see analytics and performance data here.</p>
                            <button onClick={() => setActiveTab('campaigns')} className="bg-primary-600 hover:bg-primary-700 text-white font-bold py-2 px-4 rounded-lg flex items-center mx-auto">
                                <AppIcons.Marketing className="w-5 h-5 mr-2" /> View Campaigns
                            </button>
                        </div>
                    </div>
                )
