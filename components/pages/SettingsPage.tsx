...
const AccountBillingSettings: React.FC<{
    ...
    const handleDownloadMonthInvoices = async () => {
        ...
        try {
            const response = await fetch('/api/billing/download-monthly-invoices', {
                method: 'GET',
                headers: {
            ...
            a.href = url;
            a.download = `invoices-${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, '0')}.zip`;
            ...
            setMessage({ type: 'error', text: 'Failed to download invoices. Please try again.' });
        } finally {
         ...
         <div className="space-y-8">
             ...
             <SettingCard title="Billing History" description="Review your past payments and download invoices.">
                 <div className="mb-4 flex justify-between items-center">
                     ...
                     <button
                         onClick={() => handleDownloadMonthInvoices()}
                         disabled={isDownloading}
                         className="bg-primary-600 hover:bg-primary-700 disabled:bg-primary-600/50 text-white font-semibold py-2 px-4 rounded-lg text-sm disabled:cursor-not-allowed flex items-center gap-2"
                     >
                         {isDownloading ? (
                             <>
                                 <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                 ...
                 <table className="min-w-full divide-y divide-muted">
                    <thead className="bg-muted/50"><tr>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-medium text-subtle uppercase">Transaction ID</th>
                    ...
                    <tbody className="divide-y divide-muted">
                        {history.map(item => (
                            <tr key={item.id}>
                                ...
                                <td className="px-6 py-4 text-sm font-mono text-xs">{item.transactionId || 'N/A'}</td>
...
const LeadScoringSettings: React.FC<{
    ...
    const handleSaveRule = async (ruleData: any) => {
        ...
        try {
            ...
            // If it's a new rule (no valid ID), remove the temporary ID to let backend generate it
            ...
            // The onSave function in App.tsx already updates the state
    ...
    const handleDeleteRule = (rule: LeadScoreRule) => {
        // Use the string id field, or fall back to _id if id is missing (for existing rules)
        const ruleId = rule.id || rule._id;
        if (!ruleId) {
            console.error('Rule missing ID:', rule);
            return;
        }
    ...
    const confirmDeleteRule = async () => {
        ...
        // Use the string id field, or fall back to _id if id is missing (for existing rules)
        const ruleId = ruleToDelete.id || ruleToDelete._id;
        if (!ruleId) {
            console.error('Rule ID is missing:', ruleToDelete);
            return;
        }
        ...
        try {
            ...
            // Update local state immediately for better UX
            // The onDelete function in App.tsx already updates the state
                            ...
                            <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <button
                                        ...
                                        disabled={isLoading !== null}
                                        className="p-1.5 text-subtle hover:text-red-500 hover:bg-red-50 rounded-md transition-colors"
                                        title="Delete rule"
                                    >
                                        <AppIcons.Delete className="w-4 h-4" />
                                    </button>

                                    {/* Hidden Edit Button (on hover) */}
                                    <button
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            handleEditRule(rule);
                                        ...
                                        disabled={isLoading !== null}
                                        className="p-1.5 text-subtle hover:text-on-surface hover:bg-muted rounded-md transition-colors opacity-0 group-hover:opacity-100"
                                        title="Edit rule"
                                    >
...
const SettingsPage: React.FC<SettingsPageProps> = (props) => {
    const { currentUser, hasPermission, currentOrganization, subscriptionPlans, billingHistory, onUpdateProfile, onUpgradeSubscription, onRefreshData, onRefreshLeadScoreRules } = props;
                ...
                {activeTab === 'appearance' && (
                 ...
                 {activeTab === 'scoring' && (
                     <SettingCard title="Lead Scoring" description="Automate lead prioritization by setting up scoring rules." permission={Permission.MANAGE_SETTINGS} hasPermission={hasPermission}>
                       <LeadScoringSettings rules={props.leadScoreRules} onSave={props.onSaveLeadScoreRule} onDelete={props.onDeleteLeadScoreRule} customFieldDefs={props.customFieldDefs} onRefreshRules={props.onRefreshLeadScoreRules} />
                    </SettingCard>
                 )}
            </div>
        </div>
