...
const TasksPage: React.FC<TasksPageProps> = ({ tasks, users, leads, teams, currentUser, updateTask, onAddTask, onDeleteTask, isLoading = false, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onScheduleMessage }) => {
    ...
    // Wrapper function to match LeadDetailModal's expected signature
    const handleAddTaskForModal = (task: Omit<Task, 'id' | 'createdAt' | 'organizationId' | '_id'>) => {
        // For modal usage, we'll use default assignment
        onAddTask(task, { type: 'user', id: currentUser.id });
    };
        ...
        <div className="flex flex-col h-full space-y-6">
            {isAddModalOpen && (
                <AddTaskModal
                    ...
                    onAddTask={handleAddTaskForModal}
                    leads={leads}
                    users={users}
                    teams={teams}
                                        ...
                                        return (
                                            <tr key={task.id} className={`group ${task.isCompleted ? 'bg-surface/50 opacity-70' : 'bg-surface'} hover:bg-muted/30 transition-colors duration-150`}>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm">
                                                    {lead ? (
                                                        <button onClick={() => handleOpenLeadModal(lead)} className="flex items-center gap-2 text-subtle hover:text-primary-500 hover:underline transition-colors">
