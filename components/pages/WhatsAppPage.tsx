...
const WhatsAppPage: React.FC<WhatsAppPageProps> = ({
    ...
    const questionChangeTimerRef = useRef<NodeJS.Timeout | null>(null);
    ...
    const defaultChatbotConfig: ChatbotConfig = {
        ...
        questions: [
            { id: crypto.randomUUID(), question: "Great, {{name}}! What's your email?", crmField: 'email', type: 'email' },
            { id: crypto.randomUUID(), question: "Thanks! And your phone number?", crmField: 'phone', type: 'phone' },
        ],
        keywordRules: [],
    ...
    // Memoized callback for bot config changes (defined before useEffect)
    const handleBotConfigChange = useCallback(async (field: keyof ChatbotConfig, value: any) => {
        if (!currentChatbotConfig) return;

        const updatedConfig = { ...currentChatbotConfig, [field]: value };
        setChatbotConfig(updatedConfig); // Update local state immediately for responsive UI
        ...
        // Set new timer to save after user stops interacting
        ...
        const fetchData = async () => {
            try {
                ...
                const [templatesData, campaignsData, chatbotData] = await Promise.all([
                    fetch('/api/marketing/templates/whatsapp', { headers: authHeaders }).then(res => res.json()).then(res => res.data || []).catch(() => []),
                    fetch('/api/marketing/campaigns/whatsapp', { headers: authHeaders }).then(res => res.json()).then(res => res.data || []).catch(() => []),
                    fetch('/api/settings/chatbot', { headers: authHeaders }).then(res => res.json()).then(res => res.data || null).catch(() => null)
                ]);
    ...
    const handleSaveTemplate = async (template: WhatsAppTemplate) => {
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Failed to save template: ${response.statusText} - ${errorText}`);
            }
    ...
    const handleSaveCampaign = async (campaign: WhatsAppCampaign) => {
        try {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                throw new Error(`Failed to save campaign: ${response.statusText} - ${errorText}`);
            }
    ...
    const handleDeleteCampaign = async (campaignId: string) => {
        ...
        try {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                throw new Error(`Failed to delete campaign: ${response.statusText} - ${errorText}`);
            }
    ...
    const handleSaveChatbotConfig = async (config: ChatbotConfig) => {
        try {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                throw new Error(`Failed to save chatbot config: ${response.statusText} - ${errorText}`);
            }
    ...
    const handleQuestionChange = async (id: string, field: keyof ChatbotQuestion, value: any) => {
        ...
        setChatbotConfig(updatedConfig); // Update local state immediately

        // Debounce API calls
        if (questionChangeTimerRef.current) {
            clearTimeout(questionChangeTimerRef.current);
        }
        questionChangeTimerRef.current = setTimeout(async () => {
            await handleSaveChatbotConfig(updatedConfig);
        }, 1000);
    }
    ...
    const addQuestion = async () => {
        if (!currentChatbotConfig) return;
        const newQuestion: ChatbotQuestion = { id: crypto.randomUUID(), question: '', crmField: 'name', type: 'text' };
    ...
    const addKeywordRule = async () => {
        if (!currentChatbotConfig) return;
        const newRule: ChatbotKeywordRule = { id: crypto.randomUUID(), keyword: '', response: '' };
            ...
            {activeTab === 'campaigns' && (
                campaigns.length > 0 ? (
                    ...
                    <div className="flex items-center justify-center h-64 text-center">
                        <div>
                            <AppIcons.Whatsapp className="h-16 w-16 text-subtle mx-auto mb-4" />
                            <h3 className="text-lg font-semibold text-on-surface mb-2">No WhatsApp Campaigns Found</h3>
                            <p className="text-subtle mb-4">Create your first WhatsApp campaign to get started with WhatsApp marketing.</p>
            ...
            {activeTab === 'templates' && (
                templates.length > 0 ? (
                                ...
                                <div key={template.id} className="bg-background p-4 rounded-lg border border-muted">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button onClick={() => handleDeleteTemplate(template.id)} className="p-1.5 text-subtle hover:text-red-500 hover:bg-red-500/10 rounded-full"><AppIcons.Delete className="h-4 w-4"/></button>
                                        </div>
                                    </div>
                    ...
                    <div className="flex items-center justify-center h-64 text-center">
                        <div>
                            <AppIcons.Whatsapp className="h-16 w-16 text-subtle mx-auto mb-4" />
                            <h3 className="text-lg font-semibold text-on-surface mb-2">No WhatsApp Templates Found</h3>
                            <p className="text-subtle mb-4">Create your first WhatsApp template to start building campaigns.</p>
