...
const testCloudTelephonyIntegration = async (req, res) => {
    try {
        ...
        // Real API connection test based on provider
        ...
        try {
            ...
            if (integration.provider === 'twilio') {
                const twilioService = new TwilioService(integration.accountSid, integration.authToken);
                testResult = await twilioService.testConnection();
            } else {
                // For other providers, implement specific service classes
                throw new Error(`Real API testing not yet implemented for provider: ${integration.provider}`);
            }

            const responseTime = Date.now() - startTime;

            // Update health check result in database
            integration.updateHealthCheck(testResult.status, testResult.responseTime, testResult.error);
            await integration.save();
