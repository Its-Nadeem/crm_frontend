...
class FacebookActivityController {
    ...
    async getActivityLogs(req, res) {
        try {
            ...
            // Handle conditions array for advanced filtering
            const conditions = [];
            let conditionIndex = 0;

            while (req.query[`conditions[${conditionIndex}][field]`]) {
                conditions.push({
                    field: req.query[`conditions[${conditionIndex}][field]`],
                    condition: req.query[`conditions[${conditionIndex}][condition]`],
                    value: req.query[`conditions[${conditionIndex}][value]`]
                });
                conditionIndex++;
            }
            ...
            if (filters.conditions) {
                filters.conditions = filters.conditions.filter(c => c.field && c.value);
            }
            ...
            // Fix: formId is not defined error - add proper error handling
            if (error.message && error.message.includes('formId')) {
                res.status(500).json({
                    success: false,
