...
const createOrUpdateItem = (Model) => async (req, res) => {
    try {
        ...
        if (Model === WebhookConfig && !id && data.url === 'new') {
            data.url = `https://api.Clienn CRM.io/v1/webhooks/leads/${crypto.randomUUID()}`;
        }
...
export const createOrUpdateTeam = async (req, res) => {
    try {
        ...
        // For new teams, generate ID before creating
        if (!id) {
        ...
        const team = id
            ? await Team.findOneAndUpdate({ id: id, organizationId: req.user.organizationId }, data, { new: true, runValidators: true })
...
export const deleteUser = async (req, res) => {
    try {
        ...
        const deleteResult = await User.deleteOne(deleteQuery);

        console.log('DEBUG: User deleted successfully:', id, 'Delete result:', deleteResult);

        // Emit real-time update to all clients in the organization
        if (global.broadcastToOrganization) {
            global.broadcastToOrganization(req.user.organizationId, {
                type: 'users_updated',
