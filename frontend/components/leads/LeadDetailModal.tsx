...
const LeadDetailModal: React.FC<LeadDetailModalProps> = ({ lead, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onScheduleMessage, currentUser, onClose }) => {
    ...
    const handleFieldChange = (field: keyof Lead, value: any) => {
        ...
        if (oldValue !== newValue) {
            ...
            // Create change description for changelog (no immediate activity creation)
   ...
   const handleAddNote = async (noteContent: string) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Trigger timeline refresh for field updates after successful backend save
            console.log('LeadDetailModal: Backend save successful, dispatching timeline refresh event');
            if (window.dispatchEvent) {
    ...
    return (
        <Modal isOpen={true} onClose={() => {}} title="">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
                {/* Left Column: Details */}
                <div className="lg:col-span-1 overflow-y-auto pr-4" onWheel={(e) => { e.stopPropagation(); e.preventDefault(); }}>
                ...
                <div className="lg:col-span-2 flex flex-col overflow-hidden bg-background rounded-lg border border-muted" onWheel={(e) => { e.stopPropagation(); e.preventDefault(); }}>
