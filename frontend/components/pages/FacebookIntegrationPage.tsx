...
import FacebookActivityLogTab from './FacebookActivityLogTab';
...
export const FacebookIntegrationPage: React.FC<FacebookIntegrationPageProps> = ({
    ...
    useEffect(() => {
        ...
        // Fetch users for variable field mapping
        const fetchUsers = async () => {
            try {
                const response = await fetch(`${window.location.origin}/api/users?organizationId=${currentOrganization?.id || 'org-1'}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                if (response.ok) {
                    const data = await response.json();
                    if (data.success) {
                        console.log('Fetched users for variable fields:', data.data);
                        setUsers(data.data);
                    }
                }
            } catch (error) {
    ...
    const handleCreateMapping = async (mappingData: any) => {
        ...
        setMappingForm({ name: '', accountId: '', pageId: '', formId: '' });
    };

    const handleSaveMapping = () => {
        if (!mappingForm.name || !mappingForm.accountId || !mappingForm.pageId || !mappingForm.formId) {
    ...
    const handleSyncLeads = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                const successCount = data.results.filter((r: any) => r.status === 'saved').length;
                ...
                let message = `✅ Sync completed! ${successCount} leads imported successfully.`;
                if (skipCount > 0) message += ` ${skipCount} duplicates skipped.`;
            ...
            if (errorMessage.includes('No leads found')) {
                // Keep error in state for UI display instead of alert
                console.log('No leads found - error will be displayed in UI');
            } else if (errorMessage.includes('access token') || errorMessage.includes('token')) {
                alert('Facebook access token expired or invalid. Please reconnect your Facebook account.');
            } else if (errorMessage.includes('network') || errorMessage.includes('timeout')) {
                alert('Network error. Please check your internet connection and try again.');
            } else if (errorMessage.includes('facebookService.fetchLeadsFromFacebook is not a function')) {
                alert('System error: Facebook service method not found. Please contact support.');
            } else {
                                        ...
                                        <div key={mapping.id} className="px-6 py-4 hover:bg-muted/20 transition-colors">
                                            <div className="grid grid-cols-12 gap-4 items-center text-sm">
                                                ...
                                                <div className="col-span-2">
                                                    <span className={`text-xs px-2 py-1 rounded-full ${mapping.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                    ...
                    <div className="bg-background rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col ring-1 ring-inset ring-muted/50">
                                ...
                                <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                                    ...
                                    <div className="bg-blue-100 rounded-lg p-3">
                                        <p className="text-xs text-blue-700">
                                            ...
                                            1. Select a Facebook form with existing leads<br/>
                                            2. Choose date range (optional)<br/>
                                            3. Click "Start Sync" to import leads<br/>
                                            4. View detailed results below
                                        </p>
                                    </div>
                                </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                                                <svg className="h-4 w-4 text-subtle" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                {syncResults && (
                                    <div className={`border rounded-lg p-4 ${
                                        ...
                                        <h4 className={`font-semibold mb-3 flex items-center gap-2 ${
                                            ...
                                            {syncResults.length === 0 ? 'No Leads Found' : 'Sync Results'}
                                        </h4>

                                        {syncResults.length === 0 ? (
                                            <div className="text-center py-4">
                                                <div className="text-blue-700 mb-2">
                                                    <strong>0 leads found</strong> for the selected form and date range.
                                                </div>
                                                <div className="text-sm text-blue-600">
                                                    Try adjusting your date range or check if the form has received any leads.
                                                </div>
                                            </div>
                                        ) : (
                                            <div className="space-y-3 text-sm">
                                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                                    <div className="text-center">
                                                        <div className="text-2xl font-bold text-green-600">{syncResults.length}</div>
                                                        <div className="text-green-800">Total Processed</div>
                                                    </div>
                                                    <div className="text-center">
                                                        <div className="text-2xl font-bold text-green-600">{syncResults.filter((r: any) => r.status === 'saved').length}</div>
                                                        <div className="text-green-800">Successfully Synced</div>
                                                    </div>
                                                    <div className="text-center">
                                                        <div className="text-2xl font-bold text-amber-600">{syncResults.filter((r: any) => r.status === 'skipped').length}</div>
                                                        <div className="text-amber-800">Skipped (Duplicates)</div>
                                                    </div>
                                                    <div className="text-center">
                                                        <div className="text-2xl font-bold text-red-600">{syncResults.filter((r: any) => r.status === 'error').length}</div>
                                                ...
                                                {/* Detailed Results */}
                                                <div className="mt-4">
                                                    <h5 className="font-medium text-green-900 mb-2">Detailed Results:</h5>
                                                    <div className="max-h-40 overflow-y-auto space-y-1">
                                                        {syncResults.map((result: any, index: number) => (
                                                            <div key={index} className={`text-xs p-2 rounded flex items-center justify-between ${
                                                                result.status === 'saved' ? 'bg-green-100 text-green-800' :
                                                                result.status === 'skipped' ? 'bg-amber-100 text-amber-800' :
                                                                'bg-red-100 text-red-800'
                                                            }`}>
                                                                <span>Lead ID: {result.leadId}</span>
                                        ...
                                        <div className="flex items-start">
                                            ...
                                            <div className="ml-4 flex-1">
                                                <h4 className="text-lg font-semibold text-red-900 mb-2">
                                                    {error.includes('facebookService.fetchLeadsFromFacebook is not a function') ? 'System Configuration Error' :
                                                     error.includes('No leads found') ? 'No Leads Found' :
                                                     'Sync Failed'}
                                                </h4>
                                                <p className="text-sm text-red-700 mb-4 leading-relaxed">
                                                    {error.includes('facebookService.fetchLeadsFromFacebook is not a function') ?
                                                        'There is a system configuration issue preventing the sync. Please contact support or try again later.' :
                                                        error}
                                                </p>
                                                        ...
                                                        {error.includes('facebookService.fetchLeadsFromFacebook is not a function') ? (
                                                            <>
                                                                <li className="flex items-start">
                                                                    ...
                                                                    <span>This is a system error. Please contact support for assistance.</span>
                                                                </li>
                                                                <li className="flex items-start">
                                                                    <span className="inline-block w-2 h-2 bg-red-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                                                                    <span>Try refreshing the page and attempting the sync again.</span>
                                                                </li>
                                                            </>
                                                        ) : error.includes('No leads found') ? (
                                                            <>
                                                                <li className="flex items-start">
                                                                    <span className="inline-block w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                                                                    <span>Check if the selected form has received any leads in the specified date range</span>
                                                                </li>
                                                                <li className="flex items-start">
                                                                    <span className="inline-block w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                                                                    <span>Try expanding the date range or removing date filters</span>
                                                                </li>
                                                                <li className="flex items-start">
                                                                    <span className="inline-block w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                                                                    <span>Verify the form is active and still collecting leads</span>
                                                                </li>
                                                            </>
                                                        ) : (
                                                            <>
                                                                <li className="flex items-start">
                                                                    <span className="inline-block w-2 h-2 bg-red-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                                                                    <span>Check if your Facebook access token is still valid and hasn't expired</span>
                                                                </li>
                                                                <li className="flex items-start">
                                                                    <span className="inline-block w-2 h-2 bg-red-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                                                                    <span>Ensure the selected form has leads in the specified date range</span>
                                                                </li>
                                                                <li className="flex items-start">
                                                                    <span className="inline-block w-2 h-2 bg-red-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                                                                    <span>Verify your Facebook page has the necessary permissions (leads_retrieval)</span>
                                                                </li>
                                                                <li className="flex items-start">
                                                                    <span className="inline-block w-2 h-2 bg-red-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                                                                    <span>Try reconnecting your Facebook account in the integration settings</span>
                                                                </li>
                                                                <li className="flex items-start">
                                                                    <span className="inline-block w-2 h-2 bg-red-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                                                                    <span>Check your internet connection and Facebook API status</span>
                                                                </li>
                                                            </>
                                                        )}
                                                    </ul>
                        ...
                        <div className="flex justify-end gap-3 p-6 border-t border-muted bg-muted/30">
                            ...
                            <button
                                ...
                                className={`font-bold py-2 px-6 rounded-lg transition-all shadow-sm hover:shadow-md flex items-center gap-2 ${
                                    syncLoading || !syncForm.formId || !syncForm.pageId
