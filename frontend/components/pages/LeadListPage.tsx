...
const LeadListPage: React.FC<{
    ...
    useEffect(() => {
        const loadColumnPreferences = async () => {
            try {
                const preferences = await apiService.getLeadColumnPreferences();
                ...
                if (savedPreferences) {
                    try {
                        ...
                        console.error('Failed to parse saved column preferences:', localError);
                    }
                }
            }
        };
    ...
    // Save column preferences to backend when they change
    useEffect(() => {
        const saveColumnPreferences = async () => {
            try {
                await apiService.updateLeadColumnPreferences({
                    columnOrder,
                    visibleColumns
                });
            } catch (error) {
        ...
        const timeoutId = setTimeout(saveColumnPreferences, 500);
        return () => clearTimeout(timeoutId);
    }, [columnOrder, visibleColumns]);

    // Save visibility changes immediately when checkbox is toggled
    const handleVisibilityChange = (columnId: string, isVisible: boolean) => {
        setVisibleColumns(prev => {
            const newVisibleColumns = { ...prev, [columnId]: isVisible };
                        ...
                        <table className="min-w-full divide-y divide-muted">
                            <thead className="bg-muted/50">
                                <tr>
                                    {columnOrder.filter(col => visibleColumns[col] !== false).map(columnId => {
                                        ...
                                        const columnConfig: Record<string, { label: string; sortable: boolean; sortKey?: string }> = {
                                        ...
                                        return (
                                            <th
                                                ...
                                                onClick={config.sortable && config.sortKey ? () => handleSort(config.sortKey as any) : undefined}
                                            >
                                                <div className="flex items-center gap-1">
                                                    {config.label}
                                                    {config.sortable && config.sortKey && sortBy === config.sortKey && (
                            ...
                            <tbody className="bg-surface divide-y divide-muted">
                                {paginatedLeads.length === 0 ? (
                                    <tr>
                                        <td colSpan={columnOrder.filter(col => visibleColumns[col] !== false).length} className="px-6 py-12 text-center text-subtle">
                                            {leads.length === 0 ? (
                                                <div>
                        ...
                        <div className="space-y-3 max-h-60 overflow-y-auto">
                            ...
                            <div>
                                ...
                                <div className="space-y-2 pl-2">
                                    {columnOrder
                                        .filter(col => !col.startsWith('custom_') && !['checkbox', 'actions', 'campaign', 'facebookCampaign', 'facebookAdset', 'facebookAd'].includes(col))
                                        .map(columnId => {
                                            ...
                                            const label = columnConfig[columnId as keyof typeof columnConfig];
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    } ${dragOverColumn === columnId ? 'bg-primary-100' : 'hover:bg-muted/50'}`}
                                                >
                                                    <AppIcons.GripVertical className="h-4 w-4 text-subtle" />
                                                    <input
                                                        type="checkbox"
                                                        checked={visibleColumns[columnId] || false}
                                                        onChange={(e) => handleVisibilityChange(columnId, e.target.checked)}
                                                        className="rounded border-muted"
                                                    />
                            ...
                            <div>
                                ...
                                <div className="space-y-2 pl-2">
                                    {columnOrder
                                        .filter(col => ['campaign', 'facebookCampaign', 'facebookAdset', 'facebookAd'].includes(col))
                                        .map(columnId => {
                                            const columnConfig: Record<string, string> = {
                                                campaign: 'Campaign',
                            ...
                            {customFieldDefs.length > 0 && (
                                <div>
                                    ...
                                    <div className="space-y-2 pl-2">
                                        {columnOrder
                                            .filter(col => col.startsWith('custom_'))
                                            .map(columnId => {
                                                const fieldId = columnId.replace('custom_', '');
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={(e) => handleVisibilityChange(`custom_${field.id}`, e.target.checked)}
                                                            className="rounded border-muted"
                                                        />
