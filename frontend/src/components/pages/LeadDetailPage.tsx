...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ leads, users, tasks, pipelineStages, customFieldDefs, whatsAppTemplates, smsTemplates, onUpdateLead, onUpdateTask, onAddTask, onAddNote, onScheduleMessage, currentUser, onRefreshLeadById }) => {
    ...
    // Lead activities hook - simplified without React Query
    ...
    // Simplified - no batching needed since we're not using React Query
    ...
    const handleSendWhatsApp = async (message: string) => {
        ...
        try {
            ...
            // Trigger timeline refresh to show new activity
            if (window.dispatchEvent) {
                window.dispatchEvent(new CustomEvent('activityUpdate', { detail: { leadId: editableLead.id } }));
            }
            ...
            addToast({
                ...
                message: 'Failed to initiate WhatsApp. Please try again.'
            });
        }
    };
   ...
   const handleEmailSent = (emailData: any) => {
    ...
    const handleAddNote = async (noteData: { content: string; leadId: string }) => {
        try {
            ...
            // Refresh activities after a short delay to ensure backend has processed the note
            setTimeout(async () => {
                console.log('🔄 Refreshing activities after note creation...');
                await refetchActivities();
                console.log('✅ Note creation completed with timeline refresh');
            }, 500);
                    ...
                    <div className="flex-grow overflow-y-auto p-4 space-y-6">
                        {activeTab === 'activity' && (
                            <ActivityTimeline
                                ...
                                realTime={true} // Enable real-time updates for instant results
                                disableAutoRefresh={false} // Enable auto-refresh for better UX
                            />
                        )}
                        {activeTab === 'tasks' && (
