...
webhookConfigSchema.index({ organizationId: 1, createdAt: -1 });
webhookConfigSchema.index({ organizationId: 1, isEnabled: 1 });
...
// Method to regenerate secret
webhookConfigSchema.methods.regenerateSecret = function() {
    this.secret = crypto.randomBytes(64).toString('hex');
    this.updatedAt = new Date();
    return this.secret;
};

// Method to get masked secret for display
webhookConfigSchema.methods.getMaskedSecret = function() {
    if (!this.secret) return null;
    const length = this.secret.length;
    const visibleChars = 4;
    return this.secret.substring(0, visibleChars) + '•'.repeat(length - visibleChars * 2) + this.secret.substring(length - visibleChars);
};
