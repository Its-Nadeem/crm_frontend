...
import facebookActivityController from '../controllers/facebookActivityController.js';
...
router.get('/auth/callback', async (req, res) => {
    try {
        ...
        ongoingOAuthProcesses.delete(`${tenantId}-${code}`);
        res.send(`
            <html>
            <head>
...
router.get('/debug/forms/:pageId', async (req, res) => {
    try {
        const { pageId } = req.params;
        const { tenantId } = req.query;
        ...
        // Test the Facebook API directly
        const facebookService = new FacebookService();
        const forms = await facebookService.getLeadGenForms(pageId, pageAccessToken);
...
router.get('/debug/raw/:pageId', async (req, res) => {
    try {
        const { pageId } = req.params;
        const { tenantId } = req.query;
        ...
        const response = await axios.get(`https://graph.facebook.com/v18.0/${pageId}/leadgen_forms`, {
            params: {
                ...
                fields: 'id,name,leads_count,created_time,questions'
            }
        });
...
// Get custom fields for CRM field dropdown
router.get('/custom-fields', async (req, res) => {
    try {
        ...
        // For now, return standard CRM fields plus custom fields
        // In a real implementation, you would fetch these from your CRM database
        ...
        // Mock custom fields - in real implementation, fetch from database
        const customFields = [
            { key: 'custom_field_1', label: 'Custom Field 1', type: 'text' },
            { key: 'custom_field_2', label: 'Custom Field 2', type: 'dropdown', options: ['Option 1', 'Option 2', 'Option 3'] },
            { key: 'custom_field_3', label: 'Custom Field 3', type: 'number' }
        ];
...
router.get('/activity-logs', facebookActivityController.getActivityLogs);

// Get filter options for activity logs
router.get('/activity-logs/filters', facebookActivityController.getFilterOptions);

// Create new activity log
router.post('/activity-logs', facebookActivityController.createActivityLog);

// Update activity log status
router.put('/activity-logs/:logId/status', facebookActivityController.updateActivityStatus);

// Delete activity logs
router.delete('/activity-logs', facebookActivityController.deleteActivityLogs);

// Get activity statistics
router.get('/activity-logs/stats', facebookActivityController.getActivityStats);
