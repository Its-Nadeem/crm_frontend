...
class FacebookService {
    ...
    getAuthUrl(tenantId, redirectOverride) {
        const scopes = [
            ...
            'email',
            'public_profile',
            'read_page_mailboxes'  // Required to read lead form questions
        ].join(',');
    ...
    async getLeadGenForms(pageId, pageAccessToken) {
        try {
            console.log(`📋 Fetching lead gen forms for page: ${pageId}`);

            // First try with detailed fields
            const params = {
                access_token: pageAccessToken,
                fields: 'id,name,leads_count,created_time,questions,status,locale'
            };
            ...
            const forms = response.data.data.map(form => {
                ...
                console.log(`🔍 Raw questions data:`, form.questions);

                // Facebook API returns questions as a direct array, not nested in data property
                let formFields = [];

                if (form.questions && Array.isArray(form.questions)) {
                    formFields = form.questions.map(q => ({
                        name: q.key,
                        label: q.label,
                        type: q.type
                    }));
                } else if (form.questions && form.questions.data && Array.isArray(form.questions.data)) {
                    // Fallback for different API versions
                    formFields = form.questions.data.map(q => ({
                        name: q.key,
                        label: q.label,
                        type: q.type
                    }));
                } else if (form.questions) {
                    console.log('❓ Questions exists but structure is different:', typeof form.questions, form.questions);
                }

                // Add metadata fields that are commonly available in Facebook leads
                const metadataFields = [
                    { name: 'campaign_name', label: 'Campaign Name', type: 'text' },
                    { name: 'adset_name', label: 'Ad Set Name', type: 'text' },
                    { name: 'ad_name', label: 'Ad Name', type: 'text' },
                    { name: 'form_name', label: 'Form Name', type: 'text' },
                    { name: 'page_name', label: 'Page Name', type: 'text' },
                    { name: 'platform', label: 'Platform', type: 'text' },
                    { name: 'created_time', label: 'Lead Created Time', type: 'datetime' },
                    { name: 'lead_source', label: 'Lead Source', type: 'text' }
                ];

                // Combine form fields with metadata fields
                const allFields = [...formFields, ...metadataFields];

                console.log(`📝 Form "${form.name}" (${form.id}) has ${allFields.length} total fields (${formFields.length} form + ${metadataFields.length} metadata):`,
                          allFields.map(f => `${f.name} (${f.label})`));
                ...
                return {
                    ...
                    locale: form.locale,
                    fields: allFields,
                    formFields: formFields, // Original form fields only
                    metadataFields: metadataFields, // Metadata fields
                    rawQuestions: form.questions // Include raw data for debugging
                };
            });

            return forms;
        } catch (error) {
            console.error('❌ Error fetching lead gen forms:', {
                pageId,
    ...
    /**
     * Fetch leads from Facebook API for sync/backfill
     */
    async fetchLeadsFromFacebook(formId, pageAccessToken, since, until) {
        try {
            ...
            if (leads.length === 0) {
                console.log('No leads found for form:', formId, 'in the specified date range');
            }
