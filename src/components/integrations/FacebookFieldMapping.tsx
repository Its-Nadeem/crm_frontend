...
interface CRMField {
    ...
    options?: string[];
    isCustom?: boolean;
    variableType?: 'pipeline_status' | 'users';
}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            return (
                                <div key={crmField.key} className="bg-background rounded-lg border border-muted p-3 shadow-sm min-h-[60px] flex items-center justify-start">
                                    <select
                                        ...
                                        disabled={!!currentMapping?.customValue}
                                    >
                    ...
                    <div className="space-y-3">
                        {crmFields.map(crmField => {
                            ...
                            return (
                                <div key={crmField.key} className="bg-background rounded-lg border border-muted p-3 shadow-sm min-h-[60px] flex items-center justify-start">
                                    {hasFacebookFieldMapped && currentMapping?.facebookField !== 'Use Custom Value' ? (
                                    ...
                                    ) : crmField.type === 'variable' ? (
                                        // Variable field - show searchable input like course field
                                        <div className="relative w-full searchable-dropdown">
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    const allOptions = crmField.variableType === 'pipeline_status' ? salesPipelineStages :
                                                                          crmField.variableType === 'users' ? users.map((user: any) => user.name) : [];
                                                ...
                                                disabled={hasFacebookFieldMapped}
                                            />
                                            {/* Dropdown with filtered options */}
                                            {searchTerms[crmField.key] && !hasFacebookFieldMapped && (
                                                <div className="absolute z-50 w-full mt-1 bg-background border border-muted rounded-lg shadow-xl max-h-48 overflow-y-auto">
                                                    {(() => {
                                                        const allOptions = crmField.variableType === 'pipeline_status' ? salesPipelineStages :
                                                                              crmField.variableType === 'users' ? users.map((user: any) => user.name || user.email || user.id || user._id).filter(Boolean) : [];
                                                        const filteredOptions = allOptions.filter((option: string) =>
                                                            option.toLowerCase().includes((searchTerms[crmField.key] || '').toLowerCase())
                                                        );
                                                        console.log('Variable field options for', crmField.label, ':', allOptions);
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                    ...
                                    <div>
                                        ...
                                        <span className="font-medium text-green-600 ml-2">{mappings.filter(m => m.facebookField || m.customValue).length}</span>
                                    </div>
                                </div>
                            </div>
                        ) : (
